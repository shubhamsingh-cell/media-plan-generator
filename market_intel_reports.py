...
_PLATFORM_BENCHMARKS_FALLBACK: Dict[str, Dict[str, float]] = {
    ...
    "ziprecruiter": {"cpc": 1.50, "cpa": 35.0, "ctr": 0.035, "conv_rate": 0.028},
    "glassdoor": {"cpc": 1.20, "cpa": 40.0, "ctr": 0.030, "conv_rate": 0.032},
...
if _HAS_BENCHMARK_REGISTRY:
    ...
    for _plat, _fb in _PLATFORM_BENCHMARKS_FALLBACK.items():
        ...
        PLATFORM_BENCHMARKS[_plat] = {
            ...
            "ctr": _rb.get("ctr", _fb["ctr"]),
            "conv_rate": _fb.get("conv_rate", 0.03),
        }
else:
    PLATFORM_BENCHMARKS: Dict[str, Dict[str, float]] = _PLATFORM_BENCHMARKS_FALLBACK
...
def _collect_market_overview(industry: str, locations: List[str]) -> Dict[str, Any]:
    ...
    if _research:
        lmi = _safe_get(
            _research.get_labour_market_intelligence, industry, locations, default={}
        )
        if lmi:
    ...
    if _api_enrichment:
        jolts = _safe_get(
            _api_enrichment.get_jolts_hiring_difficulty, industry, default={}
        )
        if jolts:
            data["jolts"] = jolts
...
def _collect_salary_benchmarks(
    ...
    data["aggregate"] = {
        "median": (
            round(statistics.mean(all_medians))
            if all_medians
            else base.get("median", 58000)
        ),
        "min_p10": min((v["p10"] for v in data["by_location"].values()), default=35000),
        "max_p90": max(
            (v["p90"] for v in data["by_location"].values()), default=105000
        ),
    }
    return data
...
def _collect_channel_performance(industry: str, collar_type: str) -> Dict[str, Any]:
    ...
    if _trend_engine:
        benchmarks = _safe_get(
            _trend_engine.get_all_platform_benchmarks, industry, collar_type, default={}
        )
        if benchmarks:
    ...
    data["ranked_by_cpa"] = sorted(
        data["channels"].items(), key=lambda x: x[1].get("cpa") or 999
    )
    return data


def _collect_competitor_analysis(
...
def _collect_talent_supply(
    ...
    pool_mult = {"tight": 0.6, "moderate": 1.0, "adequate": 1.3, "surplus": 1.8}.get(
        supply_level, 1.0
    )
    base_pool = 5000
    ...
    data: Dict[str, Any] = {
        ...
        "remote_eligible_pct": (
            0.65 if kb.get("collar", "white_collar") == "white_collar" else 0.08
        ),
...
def _collect_seasonal_trends(industry: str) -> Dict[str, Any]:
    ...
    if _trend_engine:
        for m in range(1, 13):
            adj = _safe_get(
                _trend_engine.get_seasonal_adjustment, "mixed", m, default={}
            )
...
def _build_executive_summary(report: Dict[str, Any]) -> str:
    ...
    diff_text = (
        "highly competitive"
        if diff >= 70
        else ("moderately competitive" if diff >= 50 else "relatively accessible")
    )
    paras.append(
...
def generate_report(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with ThreadPoolExecutor(max_workers=5) as pool:
        futures = {
            "market_overview": pool.submit(
                _collect_market_overview, industry, locations
            ),
            "salary_benchmarks": pool.submit(
                _collect_salary_benchmarks, role_category, industry, locations
            ),
            "channel_performance": pool.submit(
                _collect_channel_performance, industry, collar_type
            ),
            "talent_supply": pool.submit(
                _collect_talent_supply, role_category, industry, locations
            ),
            "seasonal_trends": pool.submit(_collect_seasonal_trends, industry),
        }
        if competitors:
            futures["competitor_analysis"] = pool.submit(
                _collect_competitor_analysis, competitors, industry
            )
        ...
        for key, fut in futures.items():
            try:
                ...
                confidence_scores[key] = (
                    "high"
                    if "api" in src
                    else ("medium" if src != "estimate" else "low")
                )
            except Exception as exc:
...
def generate_intel_excel(report_data: Dict[str, Any]) -> bytes:
    ...
    info = [
        ...
        (
            "Collar Type",
            report_data.get("collar_type", "N/A").replace("_", " ").title(),
        ),
        ("Generated", meta.get("generated_at", datetime.now().isoformat())[:16]),
        (
            "Difficulty Score",
            f"{report_data.get('market_overview', {}).get('difficulty_score', 'N/A')}/100",
        ),
    ]
    ...
    if summary:
        ...
        ws.cell(row=12, column=2, value="Executive Summary").font = Font(
            name=_XL_FONT, size=13, bold=True, color=_XL_DARK
        )
    ...
    ws2.cell(row=2, column=2, value="Channel Performance Benchmarks").font = Font(
        name=_XL_FONT, size=14, bold=True, color=_XL_WHITE
    )
    for c in range(2, 9):
    ...
    ws3.cell(row=2, column=2, value="Salary Benchmarks by Location").font = Font(
        name=_XL_FONT, size=14, bold=True, color=_XL_WHITE
    )
    for c in range(2, 9):
    ...
    ws4.cell(row=2, column=2, value="Seasonal Hiring Patterns").font = Font(
        name=_XL_FONT, size=14, bold=True, color=_XL_WHITE
    )
    for c in range(2, 5):
    ...
    for idx in range(12):
        ...
        interp = (
            "Above average (higher competition)"
            if mult > 1.05
            else ("Below average (opportunity)" if mult < 0.95 else "Average")
        )
    ...
    ws5.cell(row=2, column=2, value="Strategic Recommendations").font = Font(
        name=_XL_FONT, size=14, bold=True, color=_XL_WHITE
    )
    for c in range(2, 7):
    ...
    if comps:
        ...
        ws6.cell(row=2, column=2, value="Competitor Analysis").font = Font(
            name=_XL_FONT, size=14, bold=True, color=_XL_WHITE
        )
        for c in range(2, 6):
        ...
        for i, h in enumerate(
            ["Company", "Est. Openings", "Primary Channels", "Brand Strength"]
        ):
...
def generate_intel_ppt(report_data: Dict[str, Any]) -> bytes:
    ...
    _tb(
        ...
        11,
        0.5,
        f"Generated {datetime.now().strftime('%B %d, %Y')}  |  Created by Shubham Singh Chandel",
        sz=12,
        color=white,
        align=PP_ALIGN.CENTER,
    )
    ...
    for idx, (ch, m) in enumerate(channels.items()):
        ...
        _tb(
            ...
            card_w - 0.2,
            0.4,
            f"Trend: {m.get('trend', 'stable').title()}",
            sz=10,
            color=port_gore,
            align=PP_ALIGN.CENTER,
        )
    ...
    _tb(
        s5, 0.8, 0.2, 11, 0.7, "Seasonal Hiring Patterns", sz=24, bold=True, color=white
    )
    ...
    max_mult = max(
        (
            info.get("multiplier", 1.0) if isinstance(info, dict) else 1.0
            for info in mults.values()
        ),
        default=1.2,
    )
    ...
    for idx in range(12):
        ...
        color = (
            downy
            if mult < 0.97
            else (blue_violet if mult > 1.03 else RGBColor(0x94, 0xA3, 0xB8))
        )
    ...
    for idx, rec in enumerate(recs[:6]):
        ...
        badge_c = (
            blue_violet
            if impact == "high"
            else (downy if impact == "medium" else RGBColor(0x94, 0xA3, 0xB8))
        )
    ...
    _tb(
        ...
        11,
        0.6,
        "Created by Shubham Singh Chandel  |  Intelligent Recruitment Advertising",
        sz=14,
        color=downy,
        align=PP_ALIGN.CENTER,
    )
...
if __name__ == "__main__":
    ...
    locs = (
        _sys.argv[3].split(",") if len(_sys.argv) > 3 else ["San Francisco", "New York"]
    )
