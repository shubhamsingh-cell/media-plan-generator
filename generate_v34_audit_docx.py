...
def build_report():
    ...
    metrics = [
        ...
        ("API Integrations", "30 live data sources (salary, demand, location, ad platforms)"),
        ("QC Tests", "76 total, 73 passing (96.1%), 3 env-only failures"),
        ("Codebase", "25 Python modules, 61,219 lines of code"),
        ("Cost Reduction", "Tool-calling queries moved from ~$0.003-0.005/query to $0/query"),
    ...
    modules = [
        ("app.py", "9,789", "HTTP server, all API endpoints, rate limiting, CORS, static files"),
        ("nova.py", "5,213", "AI chatbot engine, 26 tools, Claude/free LLM integration"),
        ("llm_router.py", "1,095", "12-provider LLM router, circuit breaker, task classification"),
        ("api_enrichment.py", "10,502", "30 API integrations, concurrent fetching, caching"),
        ("auto_qc.py", "3,064", "76-test autonomous QC engine, self-healing, self-upgrading"),
        ("monitoring.py", "940", "Health checks, SLO tracking, metrics, audit trails"),
        ("ppt_generator.py", "4,845", "PowerPoint media plan generation"),
        ("research.py", "3,244", "Market research, knowledge base, 42 industry sources"),
        ("data_orchestrator.py", "2,815", "Unified data fetching, tier-aware fallback"),
        ("budget_engine.py", "2,679", "Budget allocation, ROI projections, channel optimization"),
        ("data_synthesizer.py", "2,532", "Multi-source data fusion, confidence scoring"),
        ("nova_slack.py", "1,556", "Slack bot event handling, message formatting"),
        ("collar_intelligence.py", "1,620", "Blue/white collar classification and strategy"),
    ...
    add_bullet(doc, "Path A: Simple/conversational queries -> Free LLM providers (no tools, $0)")
    add_bullet(doc, "Path B: Tool-use queries -> Free LLM providers WITH tools ($0)")
    add_bullet(doc, "Path C: Claude API -> Last-resort paid fallback (only if Path A/B fail)")
    add_bullet(doc, "Path D: Rule-based fallback -> keyword matching + curated knowledge base")

    add_body(doc, "Media Plan Generation Pipeline:")
    add_bullet(doc, "POST /api/generate -> async job creation -> data enrichment (30 APIs) -> "
    ...
    changes_router = [
        ("_convert_tools_anthropic_to_openai()", "New helper function that converts Anthropic "
        ...
        ("_build_openai_request()", "Extended to accept tools parameter. Handles tool result "
         "messages (role=tool), assistant messages with tool_calls, and adds converted tools "
         "to payload with tool_choice=auto."),
        ("_parse_openai_response()", "Extended to detect and return tool_calls from response. "
         "Preserves raw_message for conversation threading."),
        ("call_llm() routing", "Removed forced Claude routing for tools (was: if tools: "
    ...
    changes_nova = [
        ("_chat_with_free_llm_tools()", "New method (~150 lines). Multi-turn tool iteration "
        ...
        ("_tool_handler_map() / _get_tool_handler_names()", "Centralized tool handler registry. "
         "Eliminates duplicated tool name validation. Used by both execute_tool() and "
         "free LLM tool path."),
        ("chat() routing", "Updated 4-tier routing: simple -> free+tools -> Claude -> rule-based. "
         "Free LLM with tools is now Path B, tried before Claude (Path C)."),
        ("_FREE_TOOL_PROVIDERS", "Constant listing 8 free providers that support OpenAI-compatible "
    ...
    make_table(doc,
        ...
        [
            ("Wrapper", "{ name, description, input_schema }", '{ type: "function", function: { name, description, parameters } }'),
    ...
    make_table(doc,
        ...
        [
            ...
            ("Tool-calling (3 iterations)", "~$0.009 (Claude)", "$0 (free LLM)", "100%"),
            ("Complex tool chains (8 iter)", "~$0.024 (Claude)", "$0 free / $0.024 fallback", "~90%"),
    ...
    providers = [
        ("1", "Gemini 2.0 Flash", "gemini-2.0-flash", "Free", "15 RPM", "Structured, JSON, verification"),
        ("2", "Groq", "llama-3.3-70b-versatile", "Free", "30 RPM", "Conversational, tool calling"),
        ("3", "Cerebras", "llama-3.3-70b", "Free", "30 RPM", "Hot spare (same model, different infra)"),
        ("4", "Mistral Small", "mistral-small-latest", "Free", "30 RPM", "JSON, multilingual"),
        ("5", "OpenRouter", "meta-llama/llama-4-maverick", "Free", "20 RPM", "Free model gateway"),
        ("6", "xAI Grok", "grok-3-mini-fast", "Free*", "30 RPM", "Strong reasoning, 128K context"),
        ("7", "SambaNova", "Meta-Llama-3.1-405B", "Free", "10 RPM", "Largest open model (405B)"),
        ("8", "NVIDIA NIM", "meta/llama-3.1-70b-instruct", "Free", "15 RPM", "NVIDIA-optimized inference"),
        ("9", "Cloudflare", "@cf/meta/llama-3.3-70b", "Free", "50 RPM", "Edge-distributed, 10K neurons/day"),
        ("10", "GPT-4o", "gpt-4o", "Paid", "60 RPM", "Structured + general reasoning"),
        ("11", "Claude Sonnet", "claude-sonnet-4-20250514", "Paid", "50 RPM", "Tool chains, last resort"),
        ("12", "Claude Opus", "claude-opus-4-20250514", "Paid", "20 RPM", "Emergency only, highest quality"),
    ]
    ...
    tasks = [
        ("STRUCTURED", "Benchmark lookups, CPC/CPA queries, JSON output", "Gemini > Groq > Cerebras"),
        ("CONVERSATIONAL", "Strategy advice, general Q&A, advisory", "Groq > Cerebras > Gemini"),
        ("COMPLEX", "What-if scenarios, role decomposition, multi-step", "Groq > Cerebras > SambaNova"),
        ("CODE", "Formula generation, calculations, transforms", "Gemini > Groq > Cerebras"),
        ("VERIFICATION", "Fact-checking, grounding validation", "Gemini > Groq > Cerebras"),
        ("RESEARCH", "Market research, geopolitical analysis", "Groq > Cerebras > SambaNova"),
        ("NARRATIVE", "Long-form text, executive summaries", "Groq > Cerebras > SambaNova"),
    ...
    bugs = [
        ("CRITICAL", "call_llm() treated tool_calls as failures",
         ...
         "llm_router.py"),
        ("HIGH", "Free tool path could fall through to paid providers",
         ...
         "nova.py"),
        ("MEDIUM", "active_provider set even on failure (provider lock-in bug)",
         ...
         "nova.py"),
        ("MEDIUM", "Hardcoded tool name validation set could drift",
         ...
         "nova.py"),
        ("LOW", "No per-response tool_call count limit",
    ...
    tools = [
        ("1", "query_global_supply", "Joveo global supply data by role/location/channel"),
        ...
        ("18", "suggest_smart_defaults", "Auto-detect budget, channel split, CPA from partial info"),
        ("19", "query_employer_brand", "Company-specific brand intelligence (30+ employers)"),
        ("20", "query_ad_benchmarks", "CPC/CPM/CTR by platform and industry"),
        ("21", "query_hiring_insights", "Hiring difficulty, salary competitiveness, peak windows"),
        ("22", "query_collar_strategy", "Blue vs white collar hiring strategy comparison"),
        ("23", "query_market_trends", "CPC/CPA trends, seasonal patterns, forecasting"),
        ("24", "query_role_decomposition", "Seniority-level breakdowns and hiring splits"),
        ("25", "simulate_what_if", "Budget/channel what-if scenario simulation"),
        ("26", "query_skills_gap", "Skills availability and hiring difficulty analysis"),
    ]
    ...
    test_cats = [
        ("Core Functionality", "API endpoints, media plan generation, data enrichment", "15"),
        ("Nova Chatbot", "Tool execution, response quality, conversation handling", "12"),
        ("LLM Router", "Provider routing, fallback, circuit breaker, task classification", "10"),
        ...
        ("Dynamic (auto-generated)", "Weekly auto-generated from user interactions", "4"),
    ]
    ...
    add_bullet(doc, "Global rate limit on /api/chat to prevent distributed API cost abuse")
    ...
    add_bullet(doc, "JSON parse safety for free LLM tool arguments (try/except with retry)")
    add_bullet(doc, "Per-response tool_call count cap (max 5) prevents hallucinated tool floods")

    add_heading_num(doc, "10.3", "Paid API Protection", level=2)
    add_bullet(doc, "Paid-provider guard in free tool path prevents accidental Claude/GPT-4o usage")
    add_bullet(doc, "Claude is hardcoded as last-resort only (Path C, after all free providers)")
    add_bullet(doc, "Max 3 tool iterations for free providers (vs 8 for Claude) to respect rate limits")
    add_bullet(doc, "Force-provider failure bail: returns None immediately instead of cascading")
    ...
    versions = [
        ("3.4.0", "2026-03-10", "Free LLM tool calling, paid-provider guard, 5 bug fixes"),
        ("3.3.0", "2026-03-08", "Deep QC audit, 8-task routing matrix, 76 tests, critical fixes"),
        ("3.2.0", "2026-03-06", "12-provider LLM router, Resend/Grafana/Supabase integrations"),
        ("3.1.0", "2026-03-04", "Production hardening, intelligence upgrades, LLM router"),
        ("3.0.0", "2026-02-28", "AI Intelligence Engine, trend engine, collar intelligence"),
    ]
    ...
    commits = [
        ...
        ("9ee1c1c", "feat: v3.4 -- move Nova tool calling from Claude (paid) to free LLM providers"),
        ("bea4e71", "feat: v3.3 -- deep QC audit, 8-task LLM routing matrix, 76 tests, critical fixes"),
        ("85dc768", "feat: expand LLM router to 12 providers (9 free + 3 paid)"),
        ("c916bfc", "feat: v3.2 -- 9-provider LLM router, Resend/Grafana/Supabase integrations"),
        ("5fd31b3", "fix: Nova chatbot accuracy, verbosity, and hallucination issues"),
        ("8b61f88", "docs: add v3.1 system state document (MD + DOCX)"),
        ("0afc021", "feat: v3.1 production hardening + intelligence upgrades + LLM router"),
        ("ec5568b", "chore: bump to v3.0.0, delete joveo_iq.py, fix DataUSA latency waste"),
        ("16afc76", "feat: v3 AI Intelligence Engine -- trend engine, collar intelligence"),
    ]
