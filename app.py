...
def generate_excel(data):
    ...
    if isinstance(data.get("locations"), list):
        data["locations"] = [
            _shared_standardize_location(loc) if isinstance(loc, str) else loc
    ...
    for role_key in ("target_roles", "roles"):
        if isinstance(data.get(role_key), list):
            data[role_key] = [
                r.strip().title() if isinstance(r, str) else r for r in data[role_key]
            ]
    # Standardize company name (title case if all lower/upper)
    _cn = data.get("client_name") or ""
    if (
        isinstance(_cn, str)
        and (_cn == _cn.lower() or _cn == _cn.upper())
        and len(_cn) > 3
    ):
    ...
    wb.properties.title = (
        f"Recruitment Media Plan - {data.get('client_name', 'Client')}"
    )
    wb.properties.creator = "Nova AI by Joveo"
    wb.properties.subject = (
        f"AI-generated recruitment media plan for {data.get('client_name', 'Client')}"
    )
    wb.properties.keywords = f"recruitment media plan, {data.get('industry') or ''.replace('_', ' ').title()}, job advertising"
    wb.properties.description = (
        f"Generated by Nova AI Media Plan Generator. Data from 25 APIs, 91+ platforms."
    )
    ...
    accent_light_fill = PatternFill(
        start_color=ACCENT_LIGHT, end_color=ACCENT_LIGHT, fill_type="solid"
    )
    accent_pale_fill = PatternFill(
        start_color=ACCENT_PALE, end_color=ACCENT_PALE, fill_type="solid"
    )
    off_white_fill = PatternFill(
        start_color=OFF_WHITE, end_color=OFF_WHITE, fill_type="solid"
    )
    warm_gray_fill = PatternFill(
        start_color=WARM_GRAY, end_color=WARM_GRAY, fill_type="solid"
    )
    ...
    niche_key = (
        db.get("industries", {}).get(industry, {}).get("niche_channel_key") or ""
    )
    ...
    title_cell.value = (
        ...
        if _overview_client
        else "AI Media Planner — Overview"
    )
    ...
    logo_data, logo_src = (
        fetch_client_logo(client_name, client_website)
        if (client_name or client_website)
        else (None, None)
    )
    if logo_data:
        try:
    ...
    if _company_name_for_filter and client_competitors:
        client_competitors = [
            c for c in client_competitors if _company_name_for_filter not in c.lower()
        ]

    overview_items = [
        ("Client Name", data.get("client_name") or ""),
        ("Client Website", data.get("client_website") or "" or "Not specified"),
        ("Client's Use Case", data.get("use_case") or "" or "Not specified"),
        (
            "Industry",
            data.get("industry_label", industry_label_map.get(industry, industry)),
        ),
        (
            "NAICS Sector",
            f"{data.get('bls_sector', 'General')} (NAICS {data.get('naics_code', '00')})",
        ),
        ("Talent Profile", data.get("talent_profile", "General / Mixed Workforce")),
        (
            "Job Categories",
            ", ".join(job_cat_labels) if job_cat_labels else "Not specified",
        ),
        ("Target Locations", ", ".join(locations)),
        ("Target Roles", ", ".join(roles)),
        ("Target Demographic", data.get("target_demographic") or ""),
        (
            "Budget Range",
            data.get("budget_range") or "" or data.get("budget", "Not specified"),
        ),
        ("Campaign Duration", data.get("campaign_duration") or "" or "Not specified"),
        ("Hire Volume", data.get("hire_volume") or "" or "Not specified"),
        (
            "Key Competitors",
            ", ".join(client_competitors) if client_competitors else "Not specified",
        ),
    ]

    row = 4
    for label, value in overview_items:
        ws_overview.cell(row=row, column=2, value=label).font = Font(
    ...
    row += 1
    ws_overview.cell(row=row, column=2, value="Joveo Supply Network").font = Font(
        name="Calibri", bold=True, size=12, color="2E75B6"
    )
    row += 1
    total_pubs = joveo_pubs.get("total_active_publishers", 1238)
    _total_pubs_str = (
        f"{total_pubs:,}" if isinstance(total_pubs, (int, float)) else str(total_pubs)
    )
    ws_overview.cell(
        row=row, column=2, value=f"  Active Supply Partners: {_total_pubs_str}+"
    ).font = body_font
    row += 1
    ws_overview.cell(row=row, column=2, value=f"  Countries Covered: 200+").font = (
        body_font
    )
    row += 1
    ws_overview.cell(
        row=row,
        column=2,
        value=f"  Regions: Americas, Europe, APAC, LATAM, MEA, Africa",
    ).font = body_font

    # Geopolitical Risk Assessment (if risk_level is not "low")
    _synth_data = data.get("_synthesized", {}) if isinstance(data, dict) else {}
    _geo_ctx = (
        _synth_data.get("geopolitical_context", {})
        if isinstance(_synth_data, dict)
        else {}
    )
    if isinstance(_geo_ctx, dict) and _geo_ctx.get("risk_level", "low") != "low":
        row += 2
        _risk_color = (
            "C0392B" if _geo_ctx.get("risk_level") in ("high", "critical") else "D47A1A"
        )
        ws_overview.cell(
            row=row, column=2, value="Geopolitical Risk Assessment"
        ).font = Font(name="Calibri", bold=True, size=12, color=_risk_color)
        row += 1
        _risk_level = _geo_ctx.get("risk_level", "moderate").upper()
        _risk_score = _geo_ctx.get("overall_risk_score") or 0
        ws_overview.cell(row=row, column=2, value=f"  Overall Risk Level:").font = Font(
            name="Calibri", bold=True, size=11, color="1B2A4A"
        )
        ws_overview.cell(
            row=row, column=3, value=f"{_risk_level} ({_risk_score:.1f}/10)"
        ).font = Font(name="Calibri", bold=True, size=11, color=_risk_color)
        row += 1
        _geo_summary = _geo_ctx.get("summary") or ""
        if _geo_summary:
            ws_overview.cell(row=row, column=2, value=f"  Summary:").font = Font(
                name="Calibri", bold=True, size=11, color="1B2A4A"
            )
            ws_overview.cell(row=row, column=3, value=str(_geo_summary)[:200]).font = (
                body_font
            )
            ws_overview.cell(row=row, column=3).alignment = wrap_alignment
            row += 1
        ...
        if _geo_recs:
            ...
            ws_overview.cell(
                row=row, column=3, value="; ".join(str(r)[:80] for r in _geo_recs[:3])
            ).font = body_font
            ws_overview.cell(row=row, column=3).alignment = wrap_alignment
            row += 1

    row += 2
    ws_overview.cell(row=row, column=2, value="Plan Sections").font = Font(
        name="Calibri", bold=True, size=12, color="2E75B6"
    )
    row += 1
    ...
    ws_overview.cell(
        row=row + 1,
        column=2,
        value=f"Generated on: {datetime.datetime.now().strftime('%B %d, %Y')}",
    ).font = Font(name="Calibri", italic=True, size=9, color="888888")
    ws_overview.cell(
        row=row + 2,
        column=2,
    ...
    ws_exec["B3"].value = (
        f"{industry_label_val}  |  Generated {datetime.datetime.now().strftime('%B %d, %Y')}"
    )
    ...
    campaign_duration_val = data.get("campaign_duration") or "" or "Not specified"
    loc_count = len(locations)
    role_count = len(roles)
    hire_volume_val = data.get("hire_volume") or "" or "Not specified"
    ...
    metric_grid = [
        ...
        (
            ...
            "Target Roles",
            f"{role_count} role(s)" if role_count > 0 else "Roles not specified",
        ),
        (
            "Industry",
            industry_label_val,
            "Talent Profile",
            data.get("talent_profile", "General / Mixed Workforce"),
        ),
    ]
    ...
    for sn in all_sheet_names:
        ws_exec.cell(row=exec_row, column=2, value=f"  \u2022  {sn}").font = Font(
    ...
    exec_row += 1
    regional_count = len(
        data.get("selected_regional", db["traditional_channels"]["regional_local"][:25])
    )
    niche_count = len(
        data.get(
            "selected_niche",
            db["traditional_channels"]["niche_by_industry"].get(niche_key, [])[:25],
        )
    )
    global_count = len(
        data.get("selected_global", db["traditional_channels"]["global_reach"][:25])
    )
    ws_exec.cell(
        row=exec_row,
        column=2,
        value=f"{regional_count} Regional  +  {niche_count} Niche  +  {global_count} Global channels",
    ).font = Font(name="Calibri", size=11, color="1B2A4A")
    exec_row += 2
    ...
    if company_intel.get("matched"):
        ...
        for label, value in intel_items:
            ws_exec.cell(row=exec_row, column=2, value=f"  {label}:").font = Font(
                name="Calibri", bold=True, size=10, color="1B2A4A"
            )
            ws_exec.cell(row=exec_row, column=4, value=value).font = Font(
                name="Calibri", size=10, color="333333"
            )
            ws_exec.merge_cells(f"B{exec_row}:C{exec_row}")
    ...
    exec_row += 1
    lm_summary_items = [
        f"Sector: {lm_ind.get('sector_name') or ''}",
        f"Employment Growth: {lm_ind.get('projected_growth_2024_2034') or ''}",
        f"Talent Shortage: {lm_ind.get('talent_shortage_severity') or ''}",
        f"JOLTS Openings Rate: {lm_ind.get('job_openings_rate_jolts') or ''}",
        f"Avg Time to Fill: {lm_ind.get('vacancy_fill_time_avg') or ''}",
        f"Wage Growth: {lm_ind.get('wage_growth_yoy') or ''}",
    ]
    for item in lm_summary_items:
        ws_exec.cell(row=exec_row, column=2, value=f"  {item}").font = Font(
            name="Calibri", size=10, color="333333"
        )
    ...
    if industry_emp:
        ...
        _total_emp_str = (
            f"{_total_emp:,}"
            if isinstance(_total_emp, (int, float))
            else str(_total_emp)
        )
        _avg_wage = (
            industry_emp.get("avg_annual_wage") or industry_emp.get("avg_wage") or 0
        )
        _avg_wage_str = (
            f"${_avg_wage:,.0f}"
            if isinstance(_avg_wage, (int, float))
            else str(_avg_wage)
        )
        _estabs = industry_emp.get("establishments") or ""
        _estabs_str = (
            f", {_estabs:,} establishments"
            if isinstance(_estabs, (int, float)) and _estabs
            else ""
        )
        ws_exec.cell(
            row=exec_row,
            column=2,
            value=f"Live Data: {industry_emp.get('sector_name', 'Industry')} — {_total_emp_str} employed, Avg annual wage {_avg_wage_str}{_estabs_str} ({industry_emp.get('source', 'BLS QCEW')})",
    ...
    if sec_data and sec_data.get("is_public"):
        ...
        ws_exec.cell(
            row=exec_row,
            column=2,
            value=f"Public Company: {sec_data.get('company_name') or ''} (Ticker: {sec_data.get('ticker', 'N/A')}) — Source: SEC EDGAR",
    ...
    ws_exec.cell(
        row=exec_row, column=2, value=f"  Peak Hiring Months: {peak_months_str}"
    ).font = Font(name="Calibri", bold=True, size=10, color="1B2A4A")
    ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
    exec_row += 1
    ws_exec.cell(
        row=exec_row, column=2, value=f"  Campaign Ramp-Up Start: {ramp_start}"
    ).font = Font(name="Calibri", bold=True, size=10, color="2E75B6")
    ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
    exec_row += 1
    ws_exec.cell(row=exec_row, column=2, value=f"  {seasonal_note}").font = Font(
        name="Calibri", italic=True, size=10, color="596780"
    )
    ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
    exec_row += 1
    exec_row += 1
    ...
    if client_competitors:
        ...
        ws_exec.cell(
            row=exec_row,
            column=2,
            value=f"Key Competitors: {', '.join(client_competitors)}",
        ).font = Font(name="Calibri", size=11, color="333333")
        exec_row += 1
        ws_exec.cell(
            row=exec_row,
            column=2,
            value="Detailed per-competitor intelligence (hiring channels, employer brand, strategies, and recommendations) included in the Market Trends sheet.",
        ).font = Font(name="Calibri", italic=True, size=10, color="596780")
        ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
        exec_row += 2
    ...
    _exec_total_proj = (
        _exec_budget.get("total_projected", {})
        if isinstance(_exec_budget, dict)
        else {}
    )
    _exec_proj_hires = (
        _exec_total_proj.get("hires") or 0 if isinstance(_exec_total_proj, dict) else 0
    )
    _exec_proj_cph = (
        _exec_total_proj.get("cost_per_hire") or 0
        if isinstance(_exec_total_proj, dict)
        else 0
    )
    _exec_proj_clicks = (
        _exec_total_proj.get("clicks") or 0 if isinstance(_exec_total_proj, dict) else 0
    )
    _exec_proj_apps = (
        _exec_total_proj.get("applications") or 0
        if isinstance(_exec_total_proj, dict)
        else 0
    )
    _has_budget_data = any(
        isinstance(v, (int, float)) and v > 0
        for v in [_exec_proj_hires, _exec_proj_clicks, _exec_proj_apps]
    )
    if _has_budget_data:
        ...
        _ba_summary = [
            (
                "Projected Hires",
                (
                    f"{_exec_proj_hires:,.0f}"
                    if isinstance(_exec_proj_hires, (int, float))
                    else str(_exec_proj_hires)
                ),
                "Projected Cost/Hire",
                (
                    f"${_exec_proj_cph:,.0f}"
                    if isinstance(_exec_proj_cph, (int, float)) and _exec_proj_cph > 0
                    else "N/A"
                ),
            ),
            (
                "Projected Clicks",
                (
                    f"{_exec_proj_clicks:,.0f}"
                    if isinstance(_exec_proj_clicks, (int, float))
                    else str(_exec_proj_clicks)
                ),
                "Projected Applications",
                (
                    f"{_exec_proj_apps:,.0f}"
                    if isinstance(_exec_proj_apps, (int, float))
                    else str(_exec_proj_apps)
                ),
            ),
        ]
        for label1, val1, label2, val2 in _ba_summary:
            ws_exec.cell(row=exec_row, column=2, value=f"  {label1}:").font = Font(
                name="Calibri", bold=True, size=10, color=NAVY
            )
            ws_exec.cell(row=exec_row, column=3, value=val1).font = Font(
                name="Calibri", size=10, color="333333"
            )
            ws_exec.cell(row=exec_row, column=4, value=f"  {label2}:").font = Font(
                name="Calibri", bold=True, size=10, color=NAVY
            )
            ws_exec.merge_cells(f"E{exec_row}:G{exec_row}")
            ws_exec.cell(row=exec_row, column=5, value=val2).font = Font(
                name="Calibri", size=10, color="333333"
            )
            exec_row += 1
        ...
        if isinstance(_exec_ch_allocs, dict) and _exec_ch_allocs:
            exec_row += 1
            ws_exec.cell(
                row=exec_row, column=2, value="  Top Channel Recommendations:"
            ).font = Font(name="Calibri", bold=True, size=10, color=NAVY)
            ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
            exec_row += 1
            ...
            _sorted_chs = sorted(
                [
                    (ch_name, ch_data)
                    for ch_name, ch_data in _exec_ch_allocs.items()
                    if isinstance(ch_data, dict)
                ],
                key=lambda x: x[1].get("dollar_amount", x[1].get("amount") or 0),
                reverse=True,
            )
            for _ch_name, _ch_data in _sorted_chs[:3]:
                _ch_amt = _ch_data.get("dollar_amount", _ch_data.get("amount") or 0)
                _ch_pct = _ch_data.get("percentage") or 0
                _ch_hires = _ch_data.get("projected_hires") or 0
                _ch_display = _ch_name.replace("_", " ").title()
                ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
                _ch_text = (
                    f"    {_ch_display}: ${_ch_amt:,.0f} ({_ch_pct:.0f}%)"
                    if isinstance(_ch_amt, (int, float))
                    and isinstance(_ch_pct, (int, float))
                    else f"    {_ch_display}"
                )
                if isinstance(_ch_hires, (int, float)) and _ch_hires > 0:
                    _ch_text += f" — ~{_ch_hires:.1f} projected hires"
                ws_exec.cell(row=exec_row, column=2, value=_ch_text).font = Font(
                    name="Calibri", size=10, color="333333"
                )
                exec_row += 1
    ...
    if has_international:
        recommendations.append(
            "International recruitment strategy recommended with local job boards"
        )
    budget_str = str(data.get("budget_range") or "" or data.get("budget") or "")
    # Check if budget exceeds $500K
    if any(
        x in budget_str.lower()
        for x in ["500k", "500,000", "1m", "1,000,000", "million"]
    ):
        recommendations.append(
            "Multi-channel programmatic approach with performance tracking"
        )
    ...
    if roles:
        ...
        recommendations.append(
            f"Role-targeted strategy: Focus channels and messaging on {role_str} talent pools for maximum relevance"
        )
        ...
        for _tname, _tg in _tier_groups.items():
            ...
            recommendations.append(
                f"{_tname} ({_tier_roles}): Recommended channels — {_tier_channels}. Est. time-to-fill: {_ti['time_to_fill_days']} days. CPA multiplier: {_ti['cpa_multiplier']}x"
            )
        # Additional pattern-based recommendations (complementary to tier system)
        role_lower = " ".join(r.lower() for r in roles)
        if any(
            x in role_lower
            for x in ["nurse", "physician", "rn", "medical", "clinical", "therapist"]
        ):
            recommendations.append(
                "Clinical talent: Prioritize health-specific boards (Health eCareers, Vivian Health) and hospital system career pages"
            )
        if any(
            x in role_lower
            for x in ["engineer", "developer", "software", "devops", "data scientist"]
        ):
            recommendations.append(
                "Tech talent: Invest in developer communities (GitHub, Stack Overflow Talent, Wellfound) and technical assessment platforms"
            )
        if any(
            x in role_lower
            for x in ["driver", "warehouse", "mechanic", "technician", "operator"]
        ):
            recommendations.append(
                "High-volume roles: Deploy programmatic CPA-based campaigns for rapid scaling with performance optimization"
            )
        if any(
            x in role_lower
            for x in ["executive", "director", "vp", "chief", "president", "c-suite"]
        ):
            recommendations.append(
                "Executive search: Supplement with retained/contingency search firms and premium professional networks"
            )
        if any(x in role_lower for x in ["sales", "account", "business development"]):
            recommendations.append(
                "Sales talent: Leverage LinkedIn Recruiter, sales communities, and performance-based sourcing channels"
            )

    # Always have at least 3 recommendations
    if len(recommendations) < 3:
        recommendations.append(
            "Programmatic job advertising recommended for optimized cost-per-applicant"
        )
    if len(recommendations) < 3:
        recommendations.append(
            "Employer branding investment will improve long-term talent pipeline"
        )
    ...
    if enrichment_summary.get("apis_succeeded"):
        ...
        ws_exec.cell(
            row=exec_row,
            column=2,
            value=f"Live API data sourced from: {apis_used} (fetched {enrichment_summary.get('total_time_seconds') or 0:.1f}s ago)",
        ...
        "healthcare_medical": {
            ...
            "benchmarks": {
                "North America": {
                    ...
                    "cpc": "$0.90 - $3.50",
                    "cph": "$9,000 - $12,000",
                    "notes": "Hardest to fill; travel nursing $85+ CPA; clinical shortages persist; CPA exceeds all other sectors",
                },
                "Europe (UK/DE/FR)": {
                    "cpa": "$28 - $65",
                    "cpc": "$0.70 - $2.80",
                    "cph": "$7,000 - $10,000",
                    "notes": "NHS high volume lower CPA; specialist/surgeon roles premium; burnout-driven churn",
                },
                "APAC (IN/AU/SG)": {
                    "cpa": "$10 - $40",
                    "cpc": "$0.30 - $1.50",
                    "cph": "$3,000 - $7,000",
                    "notes": "India lowest; Australia/Singapore premium; immigration constraints limiting supply",
                },
                "LATAM": {
                    "cpa": "$8 - $30",
                    "cpc": "$0.20 - $1.00",
                    "cph": "$2,000 - $5,000",
                    "notes": "Growing market; lower competition; telemedicine roles emerging",
                },
            },
        },
        "blue_collar_trades": {
            ...
            "benchmarks": {
                "North America": {
                    ...
                    "cpc": "$0.40 - $1.60",
                    "cph": "$3,500 - $5,600",
                    "notes": "High volume; skilled trades (electricians, plumbers) facing persistent shortages; CDL roles $52+ CPA",
                },
                "Europe (UK/DE/FR)": {
                    "cpa": "$10 - $30",
                    "cpc": "$0.35 - $1.30",
                    "cph": "$3,000 - $5,000",
                    "notes": "Trades apprenticeships lower CPA; Germany skilled worker visa driving demand",
                },
                "APAC (IN/AU/SG)": {
                    "cpa": "$4 - $18",
                    "cpc": "$0.10 - $0.70",
                    "cph": "$1,200 - $3,000",
                    "notes": "India high volume, very low CPA; Australia mining/construction premium",
                },
                "LATAM": {
                    "cpa": "$3 - $15",
                    "cpc": "$0.08 - $0.55",
                    "cph": "$800 - $2,500",
                    "notes": "Manufacturing hubs competitive; Mexico nearshoring driving demand",
                },
            },
        },
        "tech_engineering": {
            ...
            "benchmarks": {
                "North America": {
                    ...
                    "cpc": "$1.20 - $4.50",
                    "cph": "$6,000 - $22,000",
                    "notes": "CPAs falling due to white-collar recession; senior/AI roles still $100+; CPCs remain high despite reduced competition",
                },
                "Europe (UK/DE/FR)": {
                    "cpa": "$20 - $60",
                    "cpc": "$0.90 - $3.50",
                    "cph": "$5,000 - $15,000",
                    "notes": "Berlin/London hotspots; remote-first lowering CPA; AI/ML roles carry premium",
                },
                "APAC (IN/AU/SG)": {
                    "cpa": "$8 - $35",
                    "cpc": "$0.25 - $1.80",
                    "cph": "$2,000 - $8,000",
                    "notes": "India tech hubs competitive; Singapore premium; GCC market expanding",
                },
                "LATAM": {
                    "cpa": "$6 - $28",
                    "cpc": "$0.20 - $1.40",
                    "cph": "$1,500 - $6,000",
                    "notes": "Nearshore tech hubs booming; Brazil/Mexico/Colombia key markets",
                },
            },
        },
        "general_entry_level": {
            ...
            "benchmarks": {
                "North America": {
                    ...
                    "cpc": "$0.35 - $1.30",
                    "cph": "$2,000 - $4,700",
                    "notes": "Highest volume; CPCs up 27% in 2024; Q4 seasonal spikes; every US state saw lower CPA than 2023",
                },
                "Europe (UK/DE/FR)": {
                    "cpa": "$8 - $22",
                    "cpc": "$0.28 - $1.10",
                    "cph": "$1,800 - $4,000",
                    "notes": "Retail/hospitality most common; UK high street volume roles",
                },
                "APAC (IN/AU/SG)": {
                    "cpa": "$3 - $14",
                    "cpc": "$0.08 - $0.50",
                    "cph": "$500 - $2,500",
                    "notes": "Massive volume in India; Sun Belt-equivalent regions deliver lower CPA",
                },
                "LATAM": {
                    "cpa": "$2 - $10",
                    "cpc": "$0.05 - $0.40",
                    "cph": "$400 - $2,000",
                    "notes": "Lowest CPAs globally; scaling opportunity; quality screening critical",
                },
            },
        },
        "finance_banking": {
            ...
            "benchmarks": {
                "North America": {
                    ...
                    "cpc": "$0.90 - $3.50",
                    "cph": "$5,000 - $12,000",
                    "notes": "Compliance roles premium; fintech competitive; extensive background checks inflate CPH",
                },
                "Europe (UK/DE/FR)": {
                    "cpa": "$18 - $55",
                    "cpc": "$0.75 - $2.80",
                    "cph": "$4,000 - $10,000",
                    "notes": "London financial district highest CPA; regulatory hiring surging",
                },
                "APAC (IN/AU/SG)": {
                    "cpa": "$8 - $32",
                    "cpc": "$0.30 - $1.50",
                    "cph": "$2,000 - $6,000",
                    "notes": "Singapore/HK premium; India BPO/fintech ops lower CPA",
                },
                "LATAM": {
                    "cpa": "$6 - $25",
                    "cpc": "$0.18 - $0.90",
                    "cph": "$1,500 - $4,500",
                    "notes": "Banking sector growing in Brazil/Mexico; digital banking roles emerging",
                },
            },
        },
        "retail_consumer": {
            ...
            "benchmarks": {
                "North America": {
                    ...
                    "cpc": "$0.25 - $1.00",
                    "cph": "$2,700 - $4,000",
                    "notes": "High volume; seasonal Q4 peak; historically ~$21 avg CPA; 64,000 retail jobs shed in 2025",
                },
                "Europe (UK/DE/FR)": {
                    "cpa": "$7 - $18",
                    "cpc": "$0.20 - $0.85",
                    "cph": "$2,200 - $3,500",
                    "notes": "High street retail competitive; e-commerce fulfillment roles growing",
                },
                "APAC (IN/AU/SG)": {
                    "cpa": "$3 - $10",
                    "cpc": "$0.08 - $0.40",
                    "cph": "$800 - $2,000",
                    "notes": "E-commerce driving demand in India/SE Asia; quick commerce roles new category",
                },
                "LATAM": {
                    "cpa": "$2 - $8",
                    "cpc": "$0.05 - $0.35",
                    "cph": "$600 - $1,800",
                    "notes": "Retail expansion across region; large candidate pools keep CPA low",
                },
            },
        },
        "pharma_biotech": {
            ...
            "benchmarks": {
                "North America": {
                    ...
                    "cpc": "$1.50 - $5.00",
                    "cph": "$8,000 - $18,000",
                    "notes": "Highly specialized; clinical/regulatory roles most expensive; credentialing adds to CPH",
                },
                "Europe (UK/DE/FR)": {
                    "cpa": "$32 - $85",
                    "cpc": "$1.20 - $4.00",
                    "cph": "$6,000 - $14,000",
                    "notes": "Basel/Cambridge/Dublin clusters premium; EMA regulatory talent scarce",
                },
                "APAC (IN/AU/SG)": {
                    "cpa": "$12 - $45",
                    "cpc": "$0.40 - $2.00",
                    "cph": "$3,000 - $8,000",
                    "notes": "India pharma hub (Hyderabad/Mumbai); R&D roles growing; CDMO expansion",
                },
                "LATAM": {
                    "cpa": "$8 - $35",
                    "cpc": "$0.25 - $1.40",
                    "cph": "$2,000 - $6,000",
                    "notes": "Clinical trials driving demand in Brazil; biosimilar manufacturing growth",
                },
            },
        },
        "energy_utilities": {
            ...
            "benchmarks": {
                "North America": {
                    ...
                    "cpc": "$0.90 - $3.00",
                    "cph": "$5,000 - $10,000",
                    "notes": "Oil/gas premium; renewables/solar growing fast; energy transition creating new role categories",
                },
                "Europe (UK/DE/FR)": {
                    "cpa": "$22 - $58",
                    "cpc": "$0.75 - $2.50",
                    "cph": "$4,000 - $8,500",
                    "notes": "Green energy hubs in Germany/Nordics; wind/solar technicians in demand",
                },
                "APAC (IN/AU/SG)": {
                    "cpa": "$10 - $38",
                    "cpc": "$0.30 - $1.50",
                    "cph": "$2,500 - $6,000",
                    "notes": "Mining and energy in Australia premium; India solar manufacturing scale-up",
                },
                "LATAM": {
                    "cpa": "$7 - $28",
                    "cpc": "$0.20 - $1.00",
                    "cph": "$1,800 - $5,000",
                    "notes": "Oil/gas hubs in Brazil/Mexico; lithium mining in Chile/Argentina",
                },
            },
        },
        "logistics_supply_chain": {
            ...
            "benchmarks": {
                "North America": {
                    ...
                    "cpc": "$0.40 - $1.80",
                    "cph": "$4,500 - $8,000",
                    "notes": "Transportation roles ~$52 CPA; CDL/last-mile most expensive; warehouse automation roles emerging",
                },
                "Europe (UK/DE/FR)": {
                    "cpa": "$10 - $35",
                    "cpc": "$0.30 - $1.30",
                    "cph": "$3,500 - $6,500",
                    "notes": "Distribution hubs Germany/Netherlands; Brexit-driven UK driver shortages",
                },
                "APAC (IN/AU/SG)": {
                    "cpa": "$4 - $18",
                    "cpc": "$0.10 - $0.60",
                    "cph": "$1,200 - $3,500",
                    "notes": "India massive volume; very low CPA; e-commerce logistics booming",
                },
                "LATAM": {
                    "cpa": "$3 - $14",
                    "cpc": "$0.08 - $0.45",
                    "cph": "$800 - $2,800",
                    "notes": "Manufacturing and nearshoring logistics expansion; Mexico/Colombia key hubs",
                },
            },
        },
        "automotive": {
            ...
            "benchmarks": {
                "North America": {
                    ...
                    "cpc": "$0.60 - $2.20",
                    "cph": "$5,600 - $9,000",
                    "notes": "EV sector most competitive; skilled trades premium; battery/EV plant buildouts driving demand",
                },
                "Europe (UK/DE/FR)": {
                    "cpa": "$15 - $42",
                    "cpc": "$0.50 - $1.80",
                    "cph": "$4,500 - $7,500",
                    "notes": "Germany auto hub; EV battery gigafactories scaling; automation technician roles growing",
                },
                "APAC (IN/AU/SG)": {
                    "cpa": "$5 - $22",
                    "cpc": "$0.15 - $0.85",
                    "cph": "$1,500 - $4,000",
                    "notes": "Japan/Korea OEMs; India manufacturing growth; EV supply chain expansion",
                },
                "LATAM": {
                    "cpa": "$4 - $18",
                    "cpc": "$0.12 - $0.65",
                    "cph": "$1,000 - $3,500",
                    "notes": "Mexico auto corridor competitive; nearshoring accelerating manufacturing demand",
                },
            },
        },
        "insurance": {
            ...
            "benchmarks": {
                "North America": {
                    ...
                    "cpc": "$0.85 - $3.20",
                    "cph": "$5,000 - $10,000",
                    "notes": "Actuarial/underwriting most expensive; insurtech creating new role demand",
                },
                "Europe (UK/DE/FR)": {
                    "cpa": "$20 - $52",
                    "cpc": "$0.70 - $2.60",
                    "cph": "$4,000 - $8,000",
                    "notes": "London/Zurich insurance markets premium; Solvency II compliance roles",
                },
                "APAC (IN/AU/SG)": {
                    "cpa": "$8 - $30",
                    "cpc": "$0.25 - $1.30",
                    "cph": "$2,000 - $5,000",
                    "notes": "India BPO insurance ops lower CPA; digital insurance roles growing",
                },
                "LATAM": {
                    "cpa": "$6 - $22",
                    "cpc": "$0.18 - $0.85",
                    "cph": "$1,500 - $4,000",
                    "notes": "Growing insurance market in Brazil; microinsurance roles emerging",
                },
            },
        },
        "hospitality_travel": {
            ...
            "benchmarks": {
                "North America": {
                    ...
                    "cpc": "$0.22 - $1.00",
                    "cph": "$2,500 - $4,000",
                    "notes": "CPA surging +225% YoY despite high volume; seasonal peaks extreme; turnover-driven churn",
                },
                "Europe (UK/DE/FR)": {
                    "cpa": "$7 - $20",
                    "cpc": "$0.18 - $0.80",
                    "cph": "$2,000 - $3,500",
                    "notes": "Tourism hubs seasonal demand; Mediterranean summer surges; visa-dependent workforce",
                },
                "APAC (IN/AU/SG)": {
                    "cpa": "$3 - $12",
                    "cpc": "$0.06 - $0.40",
                    "cph": "$800 - $2,200",
                    "notes": "Massive hospitality sector in SE Asia; Bali/Thailand/Vietnam growth markets",
                },
                "LATAM": {
                    "cpa": "$2 - $10",
                    "cpc": "$0.04 - $0.35",
                    "cph": "$600 - $1,800",
                    "notes": "Tourism-driven in Caribbean/Mexico; all-inclusive resort hiring surges",
                },
            },
        },
        "telecommunications": {
            ...
            "benchmarks": {
                "North America": {
                    ...
                    "cpc": "$0.75 - $2.80",
                    "cph": "$5,000 - $10,000",
                    "notes": "5G/fiber technicians in demand; network engineer roles premium; field roles hard to fill",
                },
                "Europe (UK/DE/FR)": {
                    "cpa": "$18 - $48",
                    "cpc": "$0.60 - $2.20",
                    "cph": "$4,000 - $8,000",
                    "notes": "Fibre rollout in UK/DE driving demand; regulatory/spectrum roles niche",
                },
                "APAC (IN/AU/SG)": {
                    "cpa": "$6 - $25",
                    "cpc": "$0.18 - $1.00",
                    "cph": "$1,500 - $4,500",
                    "notes": "India Jio/Airtel hiring at scale; APAC 5G rollout accelerating",
                },
                "LATAM": {
                    "cpa": "$5 - $20",
                    "cpc": "$0.14 - $0.75",
                    "cph": "$1,200 - $3,500",
                    "notes": "Infrastructure buildout across Brazil/Mexico; rural connectivity roles emerging",
                },
            },
        },
        "food_beverage": {
            ...
            "benchmarks": {
                "North America": {
                    ...
                    "cpc": "$0.18 - $0.75",
                    "cph": "$2,000 - $3,500",
                    "notes": "CPA down 25.8% YoY — steepest decline across all sectors; improved part-time labor availability",
                },
                "Europe (UK/DE/FR)": {
                    "cpa": "$5 - $15",
                    "cpc": "$0.15 - $0.65",
                    "cph": "$1,800 - $3,000",
                    "notes": "QSR/fast casual competitive; seasonal tourist-area demand spikes",
                },
                "APAC (IN/AU/SG)": {
                    "cpa": "$2 - $8",
                    "cpc": "$0.05 - $0.30",
                    "cph": "$500 - $1,500",
                    "notes": "Massive food delivery workforce in India/SE Asia; gig economy roles",
                },
                "LATAM": {
                    "cpa": "$2 - $7",
                    "cpc": "$0.04 - $0.25",
                    "cph": "$400 - $1,200",
                    "notes": "Large informal workforce transitioning to formal; QSR expansion rapid",
                },
            },
        },
        "media_entertainment": {
            ...
            "benchmarks": {
                "North America": {
                    ...
                    "cpc": "$0.70 - $2.80",
                    "cph": "$5,000 - $12,000",
                    "notes": "Content creation/streaming roles competitive; marketing at 6.31% apply rate; AI content roles emerging",
                },
                "Europe (UK/DE/FR)": {
                    "cpa": "$16 - $45",
                    "cpc": "$0.55 - $2.20",
                    "cph": "$4,000 - $9,000",
                    "notes": "London/Berlin creative hubs; localization roles growing across EU",
                },
                "APAC (IN/AU/SG)": {
                    "cpa": "$5 - $22",
                    "cpc": "$0.15 - $0.90",
                    "cph": "$1,500 - $5,000",
                    "notes": "India Bollywood/OTT content scale; gaming industry growth in Japan/Korea",
                },
                "LATAM": {
                    "cpa": "$4 - $18",
                    "cpc": "$0.12 - $0.70",
                    "cph": "$1,200 - $4,000",
                    "notes": "Spanish-language content production growing; streaming localization demand",
                },
            },
        },
        "construction_real_estate": {
            ...
            "benchmarks": {
                "North America": {
                    ...
                    "cpc": "$0.65 - $2.50",
                    "cph": "$4,500 - $8,000",
                    "notes": "Skilled trades shortage persists; infrastructure bill driving demand; among hardest to hire alongside healthcare",
                },
                "Europe (UK/DE/FR)": {
                    "cpa": "$16 - $42",
                    "cpc": "$0.50 - $2.00",
                    "cph": "$3,500 - $6,500",
                    "notes": "Green building/retrofit demand; housing crisis driving construction hiring",
                },
                "APAC (IN/AU/SG)": {
                    "cpa": "$5 - $20",
                    "cpc": "$0.15 - $0.80",
                    "cph": "$1,200 - $3,500",
                    "notes": "India infrastructure megaprojects; Australia mining/construction premium",
                },
                "LATAM": {
                    "cpa": "$4 - $16",
                    "cpc": "$0.10 - $0.60",
                    "cph": "$800 - $2,800",
                    "notes": "Infrastructure development across region; skilled labor migration common",
                },
            },
        },
        "education": {
            ...
            "benchmarks": {
                "North America": {
                    ...
                    "cpc": "$0.55 - $2.20",
                    "cph": "$3,500 - $7,000",
                    "notes": "Teacher shortages driving higher CPA; STEM/special ed most competitive; edtech roles blending with tech CPAs",
                },
                "Europe (UK/DE/FR)": {
                    "cpa": "$14 - $38",
                    "cpc": "$0.45 - $1.80",
                    "cph": "$3,000 - $6,000",
                    "notes": "UK teacher recruitment crisis; university research roles competitive",
                },
                "APAC (IN/AU/SG)": {
                    "cpa": "$5 - $20",
                    "cpc": "$0.12 - $0.75",
                    "cph": "$1,000 - $3,500",
                    "notes": "India edtech boom; international school hiring in Singapore/HK",
                },
                "LATAM": {
                    "cpa": "$3 - $14",
                    "cpc": "$0.08 - $0.50",
                    "cph": "$800 - $2,500",
                    "notes": "Public education hiring constrained; private/international school growth",
                },
            },
        },
    }
    ...
    if (
        client_industry != "general_entry_level"
        and "general_entry_level" in cpa_cpc_benchmarks
    ):
        relevant_benchmarks["general_entry_level"] = cpa_cpc_benchmarks[
            "general_entry_level"
        ]
    ...
    for ind_key, ind_data in relevant_benchmarks.items():
        ...
        ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
        ws_exec.cell(row=exec_row, column=2, value=ind_data["label"]).font = Font(
            name="Calibri", bold=True, size=12, color="2E75B6"
        )
        exec_row += 1
        ...
        ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
        ws_exec.cell(
            row=exec_row, column=2, value=f"2025 Trend: {ind_data['yoy_trend']}"
        ).font = Font(name="Calibri", italic=True, size=9, color="1B6B3A")
        exec_row += 1
        ...
        ws_exec.cell(
            row=exec_row,
            column=2,
            value=f"Avg Apply Rate: {ind_data['apply_rate']}  |  Avg CPH (Total): {ind_data['cph_range']}",
        ).font = Font(name="Calibri", bold=True, size=10, color="1B2A4A")
        exec_row += 1
        ...
        bench_headers = [
            "Region",
            f"Avg CPA ({display_currency_code})",
            f"Avg CPC ({display_currency_code})",
            f"Est. CPH ({display_currency_code})",
            f"{_bench_year_label} Market Intelligence",
        ]
    ...
    if _appcast_bm and _app_occ_key:
        ...
        if any([_occ_cpa, _occ_cph, _occ_ar]):
            ...
            ws_exec.cell(
                row=exec_row,
                column=2,
                value=f"Full-Funnel Recruitment Costs: {_app_occ_key.replace('_', ' ').title()} (2025 Actuals)",
            ).font = Font(name="Calibri", bold=True, size=12, color="1B2A4A")
            ws_exec.cell(row=exec_row, column=2).border = Border(
            ...
            ws_exec.cell(
                row=exec_row,
                column=2,
                value="Source: 10th Annual Recruitment Marketing Benchmark Report (302M clicks, 27.4M applies, ~1,200 employers)",
            ).font = Font(name="Calibri", italic=True, size=9, color="596780")
            exec_row += 1
            ...
            _overall_cpc = _appcast_bm.get("overall_median_cpc") or ""
            _overall_cpa = _appcast_bm.get("overall_median_cpa") or ""
            _overall_cph = _appcast_bm.get("overall_median_cph") or ""
            _overall_ar = _appcast_bm.get("overall_apply_rate") or ""

            _funnel_rows = [
                (
                    "Median CPA (This Occupation)",
                    _occ_cpa or "N/A",
                    f"Overall median: {_overall_cpa}" if _overall_cpa else "",
                ),
                (
                    "Cost-Per-Hire (This Occupation)",
                    _occ_cph or "N/A",
                    f"Overall median: {_overall_cph}" if _overall_cph else "",
                ),
                (
                    "Apply Rate (This Occupation)",
                    _occ_ar or "N/A",
                    f"Overall: {_overall_ar}" if _overall_ar else "",
                ),
                (
                    "Cost-Per-Screen",
                    _occ_cps or "N/A",
                    "Screened = qualified applicants",
                ),
                (
                    "Cost-Per-Interview",
                    _occ_cpi or "N/A",
                    f"Screen rate: {_appcast_bm.get('screen_rate', 'N/A')}; Interview rate: {_appcast_bm.get('interview_rate', 'N/A')}",
                ),
                (
                    "Cost-Per-Offer",
                    _occ_cpo or "N/A",
                    f"Offer rate: {_appcast_bm.get('offer_rate', 'N/A')}; Hire rate: {_appcast_bm.get('hire_rate', 'N/A')}",
                ),
            ]
            for _metric, _val, _ctx in _funnel_rows:
                ws_exec.cell(row=exec_row, column=2, value=_metric).font = Font(
                    name="Calibri", bold=True, size=10, color="1B2A4A"
                )
                ws_exec.cell(row=exec_row, column=2).border = thin_border
                ws_exec.cell(row=exec_row, column=3, value=_val).font = Font(
                ...
                ws_exec.cell(row=exec_row, column=4, value=_ctx).font = Font(
                    name="Calibri", italic=True, size=9, color="596780"
                )
            ...
            if _jad_insights:
                ...
                ws_exec.cell(
                    row=exec_row,
                    column=2,
                    value="Job Ad Optimization Insights (2025 Data)",
                ).font = Font(name="Calibri", bold=True, size=11, color="1B2A4A")
                ws_exec.cell(row=exec_row, column=2).border = Border(
                ...
                for _ins in _insight_items:
                    ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
                    ws_exec.cell(
                        row=exec_row, column=2, value=f"  \u2022  {_ins}"
                    ).font = Font(name="Calibri", size=9, color="1B2A4A")
                    exec_row += 1
    ...
    if _gads_cat:
        ...
        ws_exec.cell(
            row=exec_row,
            column=2,
            value=f"Google Ads Recruitment Benchmarks: {_gads_cat.get('category_name', _gads_cat_key)} (Joveo 2025 First-Party Data)",
        ).font = Font(name="Calibri", bold=True, size=12, color="1B2A4A")
        ws_exec.cell(row=exec_row, column=2).border = Border(
            bottom=Side(style="medium", color="2E75B6")
        )
        exec_row += 1
        ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
        _gads_total_kw = _gads_bm.get("total_keywords_analyzed") or 0
        _gads_total_spend = _gads_bm.get("total_spend") or 0
        ws_exec.cell(
            row=exec_row,
            column=2,
            value=f"Source: Joveo's own Google Ads campaign data — {_gads_total_kw:,} keywords, ${_gads_total_spend:,.0f} total spend analyzed",
        ).font = Font(name="Calibri", italic=True, size=9, color="596780")
        exec_row += 1
        ...
        _gads_rows = [
            (
                "Blended CPC",
                f"${_gads_cat.get('blended_cpc', 'N/A')}",
                "Weighted by spend across all keywords in category",
            ),
            (
                "Blended CTR",
                f"{_gads_cat.get('blended_ctr', 'N/A')}%",
                "Click-through rate (clicks / impressions)",
            ),
            (
                "Median CPC",
                f"${_gads_cat.get('cpc_stats', {}).get('median', 'N/A')}",
                f"Range: ${_gads_cat.get('cpc_stats', {}).get('p25', 'N/A')} (P25) to ${_gads_cat.get('cpc_stats', {}).get('p75', 'N/A')} (P75)",
            ),
            (
                "Keywords Analyzed",
                f"{_gads_cat.get('total_keywords') or 0:,}",
                f"Total spend in category: ${_gads_cat.get('total_spend') or 0:,.0f}",
            ),
        ]
        for _m, _v, _n in _gads_rows:
            ws_exec.cell(row=exec_row, column=2, value=_m).font = Font(
                name="Calibri", bold=True, size=10, color="1B2A4A"
            )
            ws_exec.cell(row=exec_row, column=2).border = thin_border
            ws_exec.cell(row=exec_row, column=3, value=_v).font = Font(
                name="Calibri", bold=True, size=10
            )
            ws_exec.cell(row=exec_row, column=3).border = thin_border
            ws_exec.cell(row=exec_row, column=3).alignment = center_alignment
            ws_exec.cell(row=exec_row, column=4, value=_n).font = Font(
                name="Calibri", italic=True, size=9, color="596780"
            )
            ws_exec.cell(row=exec_row, column=4).border = thin_border
            exec_row += 1
        ...
        if _top_kw:
            ...
            ws_exec.cell(
                row=exec_row,
                column=2,
                value="Top Performing Google Ads Keywords (by spend)",
            ).font = Font(name="Calibri", bold=True, size=10, color="1B2A4A")
            exec_row += 1
            ...
            for i, h in enumerate(_kw_headers):
                ...
                cell.fill = PatternFill(
                    start_color="0A66C9", end_color="0A66C9", fill_type="solid"
                )
                cell.border = thin_border
            exec_row += 1
            for kw in _top_kw[:5]:
                ws_exec.cell(
                    row=exec_row, column=2, value=kw.get("keyword") or ""
                ).font = Font(name="Calibri", size=9)
                ws_exec.cell(row=exec_row, column=2).border = thin_border
                ws_exec.cell(
                    row=exec_row, column=3, value=f"${kw.get('cpc', 'N/A')}"
                ).font = Font(name="Calibri", size=9)
                ws_exec.cell(row=exec_row, column=3).border = thin_border
                ws_exec.cell(row=exec_row, column=3).alignment = center_alignment
                ws_exec.cell(
                    row=exec_row, column=4, value=f"{kw.get('ctr_pct', 'N/A')}%"
                ).font = Font(name="Calibri", size=9)
                ws_exec.cell(row=exec_row, column=4).border = thin_border
                ws_exec.cell(row=exec_row, column=4).alignment = center_alignment
                ws_exec.cell(
                    row=exec_row, column=5, value=f"{kw.get('clicks') or 0:,}"
                ).font = Font(name="Calibri", size=9)
    ...
    exec_row += 1
    ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
    cpqa_cell = ws_exec.cell(
        row=exec_row,
        column=2,
        value="Beyond CPA: Cost Per Qualified Applicant (CPQA) — Joveo's Recommended Metric",
    )
    ...
    ws_exec.cell(
        row=exec_row,
        column=2,
        value="Unlike CPC (cost per click) and CPA (cost per application), CPQA measures the cost of attracting candidates who meet predefined qualification standards — shifting focus from volume to value.",
    ).font = Font(name="Calibri", size=10, color="596780")
    exec_row += 1
    ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
    ws_exec.cell(
        row=exec_row,
        column=2,
        value="True CPQA evaluates candidate fit, intent, and potential — not just proximity to a zip code or surface-level criteria. This is the metric that directly correlates with hiring outcomes.",
    ).font = Font(name="Calibri", size=10, color="596780")
    exec_row += 1
    ...
    for point in cpqa_points:
        ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
        ws_exec.cell(row=exec_row, column=2, value=f"  \u2022  {point}").font = Font(
            name="Calibri", size=9, color="1B2A4A"
        )
        exec_row += 1
    ...
    exec_row += 2
    ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
    ws_exec.cell(row=exec_row, column=2, value="Data Sources & Citations").font = Font(
        name="Calibri", bold=True, size=11, color="1B2A4A"
    )
    ws_exec.cell(row=exec_row, column=2).border = Border(
        bottom=Side(style="thin", color="2E75B6")
    ...
    for src in sources:
        ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
        ws_exec.cell(row=exec_row, column=2, value=src).font = Font(
            name="Calibri", italic=True, size=8, color="777777"
        )
        exec_row += 1
    ...
    ws_exec.cell(
        row=exec_row,
        column=2,
        value="Rates shown for your target regions. Actual rates may vary based on job specificity, seasonality, competition, and programmatic optimization level.",
    ...
    if tier_groups and roles:
        ...
        ws_exec.cell(
            row=exec_row,
            column=2,
            value="Each role is classified into a hiring tier with tailored funnel weights, recommended channels, and expected time-to-fill. This ensures differentiated strategies across executive, professional, clinical, trades, hourly, gig, and education roles.",
        ).font = Font(name="Calibri", italic=True, size=9, color="596780")
        exec_row += 2
        ...
        for tier_name, tg in tier_groups.items():
            ...
            funnel_display = (
                f"{dominant_stage.title()} ({int(funnel_info[dominant_stage]*100)}%)"
            )
            ...
            c5 = ws_exec.cell(
                row=exec_row, column=6, value=f"{ti['time_to_fill_days']} days"
            )
        ...
        ws_exec.cell(
            row=exec_row,
            column=2,
            value="CPA Multiplier reflects relative cost compared to baseline. Time-to-fill estimates are industry medians. Funnel emphasis shows where budget should be concentrated.",
        ).font = Font(name="Calibri", italic=True, size=8, color="999999")
    ...
    ws_exec.cell(
        row=exec_row,
        column=2,
        value="Estimated pipeline volumes based on industry conversion benchmarks and selected channel mix. Actual results depend on job specificity, employer brand, and optimization.",
    ).font = Font(name="Calibri", italic=True, size=9, color="596780")
    exec_row += 2
    ...
    funnel_benchmarks = {
        "healthcare_medical": {
            "impression_to_click": 0.028,
            "click_to_apply": 0.035,
            "apply_to_qualified": 0.22,
            "qualified_to_interview": 0.35,
            "interview_to_offer": 0.28,
            "offer_to_hire": 0.82,
        },
        "tech_engineering": {
            "impression_to_click": 0.035,
            "click_to_apply": 0.064,
            "apply_to_qualified": 0.30,
            "qualified_to_interview": 0.40,
            "interview_to_offer": 0.25,
            "offer_to_hire": 0.78,
        },
        "blue_collar_trades": {
            "impression_to_click": 0.030,
            "click_to_apply": 0.045,
            "apply_to_qualified": 0.25,
            "qualified_to_interview": 0.38,
            "interview_to_offer": 0.32,
            "offer_to_hire": 0.85,
        },
        "general_entry_level": {
            "impression_to_click": 0.032,
            "click_to_apply": 0.061,
            "apply_to_qualified": 0.28,
            "qualified_to_interview": 0.35,
        ...
        "finance_banking": {
            "impression_to_click": 0.030,
            "click_to_apply": 0.050,
            "apply_to_qualified": 0.26,
            "qualified_to_interview": 0.38,
            "interview_to_offer": 0.27,
            "offer_to_hire": 0.80,
        },
        "retail_consumer": {
            "impression_to_click": 0.033,
            "click_to_apply": 0.055,
            "apply_to_qualified": 0.30,
            "qualified_to_interview": 0.32,
            "interview_to_offer": 0.35,
            "offer_to_hire": 0.85,
        },
        "pharma_biotech": {
            "impression_to_click": 0.025,
            "click_to_apply": 0.040,
            "apply_to_qualified": 0.20,
            "qualified_to_interview": 0.35,
            "interview_to_offer": 0.25,
            "offer_to_hire": 0.80,
        },
        "hospitality_travel": {
            "impression_to_click": 0.035,
            "click_to_apply": 0.050,
            "apply_to_qualified": 0.32,
            "qualified_to_interview": 0.30,
            "interview_to_offer": 0.38,
            "offer_to_hire": 0.88,
        },
        "logistics_supply_chain": {
            "impression_to_click": 0.030,
            "click_to_apply": 0.048,
            "apply_to_qualified": 0.25,
            "qualified_to_interview": 0.35,
            "interview_to_offer": 0.30,
            "offer_to_hire": 0.83,
        ...
        "energy_utilities": {
            "impression_to_click": 0.028,
            "click_to_apply": 0.045,
            "apply_to_qualified": 0.24,
            "qualified_to_interview": 0.36,
            "interview_to_offer": 0.28,
            "offer_to_hire": 0.82,
        },
        "automotive": {
        ...
        "insurance": {
            "impression_to_click": 0.029,
            "click_to_apply": 0.050,
            "apply_to_qualified": 0.25,
            "qualified_to_interview": 0.37,
            "interview_to_offer": 0.27,
            "offer_to_hire": 0.80,
        },
        "food_beverage": {
            "impression_to_click": 0.034,
            "click_to_apply": 0.055,
            "apply_to_qualified": 0.32,
            "qualified_to_interview": 0.30,
            "interview_to_offer": 0.36,
            "offer_to_hire": 0.87,
        },
        "construction_real_estate": {
            "impression_to_click": 0.027,
            "click_to_apply": 0.040,
            "apply_to_qualified": 0.22,
            "qualified_to_interview": 0.34,
            "interview_to_offer": 0.30,
            "offer_to_hire": 0.83,
        },
        "education": {
    ...
    fb = funnel_benchmarks.get(
        client_industry, funnel_benchmarks["general_entry_level"]
    )

    # ── Budget-driven funnel calculation ──
    # Parse budget from the budget_range string (e.g. "$50,000 - $250,000", "< $50,000")
    budget_range_str = str(data.get("budget_range") or "" or "")
    ...
    if role_tiers_data:
        ...
        avg_cph = avg_cph * (
            avg_tier_multiplier / 1.5
        )  # 1.5 is the professional baseline
    ...
    funnel_stages = [
        (
            "Impressions",
            est_impressions,
            "Job ad views across all selected channels",
            None,
        ),
        (
            "Clicks",
            est_clicks,
            "Candidates who clicked through to job details",
            f"{fb['impression_to_click']*100:.1f}%",
        ),
        (
            "Applications",
            est_applications,
            "Completed applications submitted",
            f"{fb['click_to_apply']*100:.1f}%",
        ),
        (
            "Qualified Applicants",
            est_qualified,
            "Applicants meeting minimum qualifications (CPQA target)",
            f"{fb['apply_to_qualified']*100:.0f}%",
        ),
        (
            "Interviews",
            est_interviews_final,
            "Candidates advancing to interview stage",
            f"{fb['qualified_to_interview']*100:.0f}%",
        ),
        (
            "Offers",
            est_offers,
            "Offers extended to qualified candidates",
            f"{fb['interview_to_offer']*100:.0f}%",
        ),
        (
            "Hires",
            est_hires_val,
            f"Projected hires at {display_currency}{avg_cph:,.0f} avg CPH",
            f"{fb['offer_to_hire']*100:.0f}%",
        ),
    ]
    ...
    ws_exec.cell(
        row=exec_row,
        column=2,
        value=f"Conversion rates based on {_bench_year_label} industry benchmarks (Appcast, Recruitics). CPQA stage is where Joveo's ML optimization delivers highest impact.",
    ).font = Font(name="Calibri", italic=True, size=8, color="999999")

    # ── Budget Sufficiency Warning ──
    budget_warning = ""
    hire_volume_raw = data.get("hire_volume") or ""
    ...
    if budget_warning:
        ...
        for _wc in range(3, 8):
            ws_exec.cell(row=exec_row, column=_wc).fill = PatternFill(
                start_color="C00000", end_color="C00000", fill_type="solid"
            )
    ...
    funnel_data_ref = Reference(
        ws_exec, min_col=3, min_row=funnel_table_start_row, max_row=funnel_table_end_row
    )
    funnel_cats_ref = Reference(
        ws_exec, min_col=2, min_row=funnel_table_start_row, max_row=funnel_table_end_row
    )
    ...
    ws_exec.cell(
        row=exec_row,
        column=2,
        value="Estimated % of qualified pipeline contributed by each selected channel category, based on programmatic recruitment benchmarks.",
    ).font = Font(name="Calibri", italic=True, size=9, color="596780")
    exec_row += 2
    ...
    if isinstance(ch_cats2_raw, list):
        ...
        for item in ch_cats2_raw:
            if isinstance(item, dict):
                ch_cats2[item.get("name") or ""] = item.get("enabled", True)
            else:
    ...
    _bstr = str(data.get("budget") or "" or "")
    try:
    ...
    if _bval is not None:
        if _bval < 50000:
            ...
            _ap["programmatic_dsp"] += 4
            _ap["global_boards"] += 2
        elif _bval > 500000:
            _ap["employer_branding"] += 4
            _ap["regional_boards"] += 2
            _ap["social_media"] += 2
    ...
    if _rl:
        ...
        if _sr > _jr:
            _ap["niche_boards"] += 4
            _ap["employer_branding"] += 3
            _ap["social_media"] = max(2, _ap["social_media"] - 3)
            _ap["programmatic_dsp"] = max(5, _ap["programmatic_dsp"] - 4)
        elif _jr > _sr:
            _ap["social_media"] += 5
            _ap["global_boards"] += 3
            _ap["niche_boards"] = max(2, _ap["niche_boards"] - 4)
            _ap["programmatic_dsp"] = max(5, _ap["programmatic_dsp"] - 2)
    for _k2 in _ap:
    ...
    _niche_d = _NICHE_DESC.get(
        _ind_key,
        "Specialized boards with higher quality match; lower volume but better CPQA",
    )
    _SOCIAL_DESC = {
    ...
    _social_d = _SOCIAL_DESC.get(
        _ind_key,
        "Facebook Jobs, Instagram, TikTok — passive candidate reach; employer brand amplification",
    )
    ...
    if ch_cats2.get("programmatic_dsp", True):
        channel_allocations.append(
            (
                "Programmatic & DSP",
                _ap["programmatic_dsp"],
                "ML-optimized bidding; highest volume driver; real-time publisher optimization",
                "#2E75B6",
                "Highest",
            )
        )
    if ch_cats2.get("global_boards", True):
        channel_allocations.append(
            (
                "Global Job Boards",
                _ap["global_boards"],
                "Indeed, ZipRecruiter, Glassdoor — broad reach; consistent applicant flow",
                "#1B6B3A",
                "High",
            )
        )
    ...
    if ch_cats2.get("regional_boards", True):
        channel_allocations.append(
            (
                ...
                _ap["regional_boards"],
                "Geo-targeted local reach; strong for hourly & blue-collar roles",
                "#4472C4",
                "Medium",
            )
        )
    if ch_cats2.get("employer_branding", False):
        channel_allocations.append(
            (
                "Employer Branding",
                _ap["employer_branding"],
                "Glassdoor, Comparably — long-term brand building; 1.7x higher InMail acceptance",
                "#00B0F0",
                "Long-term",
            )
        )
    if ch_cats2.get("apac_regional", False):
        channel_allocations.append(
            (
                "APAC Regional",
                _ap["apac_regional"],
                "JobStreet, Naukri, Seek — APAC market-specific reach",
                "#FFC000",
                "Regional",
            )
        )
    if ch_cats2.get("emea_regional", False):
        channel_allocations.append(
            (
                "EMEA Regional",
                _ap["emea_regional"],
                "StepStone, Totaljobs, Reed — EMEA market-specific reach",
                "#FF6B6B",
                "Regional",
            )
        )
    ...
    normalized = [
        (name, round(pct / total_alloc * 100), desc, color, impact)
        for name, pct, desc, color, impact in channel_allocations
    ]
    ...
    ch_headers = [
        ...
        "Budget Allocation",
        "Impact Level",
        "Strategy & Expected Contribution",
    ]
    ...
    if normalized:
        ...
        ws_exec.cell(row=exec_row, column=8, value="Channel").font = Font(
            name="Calibri", bold=True, size=9, color="999999"
        )
        ws_exec.cell(row=exec_row, column=9, value="Allocation %").font = Font(
            name="Calibri", bold=True, size=9, color="999999"
        )
        exec_row += 1
        donut_colors = [
        ...
        for ch_idx, (ch_name, ch_pct, _desc, _color, _impact) in enumerate(normalized):
            ws_exec.cell(row=exec_row, column=8, value=ch_name).font = Font(
                name="Calibri", size=8, color="999999"
            )
            ws_exec.cell(row=exec_row, column=9, value=ch_pct).font = Font(
                name="Calibri", size=8, color="999999"
            )
            exec_row += 1
        ...
        donut_data_ref = Reference(
            ws_exec, min_col=9, min_row=donut_data_start, max_row=donut_data_end
        )
        donut_cats_ref = Reference(
            ws_exec, min_col=8, min_row=donut_data_start + 1, max_row=donut_data_end
        )
    ...
    style_section_header(
        ...
        2,
        7,
        "Employer Branding ROI — Why Brand Investment Multiplies Hiring Outcomes",
    )
    exec_row += 1
    ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
    ws_exec.cell(
        row=exec_row,
        column=2,
        value=f"Data from LinkedIn's {_bench_year_label} Hiring Value analysis shows that candidates who engage with an employer's brand before applying deliver significantly better hiring outcomes.",
    ).font = Font(name="Calibri", italic=True, size=9, color="596780")
    exec_row += 2
    ...
    eb_metrics = [
        (
            "1.7x",
            "Higher Outreach Acceptance",
            "Candidates exposed to employer branding are 1.7x more likely to accept recruiter InMails and respond to outreach messages.",
            "#2E75B6",
        ),
        (
            "5.9x",
            "Higher Conversion to Hire",
            "Brand-engaged applicants are 5.9x more likely to convert from application to hire compared to non-engaged candidates.",
            "#00B050",
        ),
        (
            "2.2x",
            "Faster Promotion Rate",
            "Employer brand-influenced hires show 2.2x higher promotion rates, indicating better role fit and long-term alignment.",
            "#0A66C9",
        ),
        (
            "1.4x",
            "Higher Demand Talent",
            "Brand-influenced hires are 1.4x more likely to be in-demand candidates (higher InMail volume), signaling you're attracting competitive talent.",
            "#ED7D31",
        ),
        (
            "82%",
            "First-Year Retention",
            "Brand-engaged hires show stronger first-year retention rates, reducing costly early-stage turnover and rehiring costs.",
            "#1B6B3A",
        ),
    ]
    ...
    for idx, (metric, label, desc, color) in enumerate(eb_metrics):
        ...
        if idx % 2 == 0:
            ...
            for c in range(3, 7):
                ws_exec.cell(row=exec_row, column=c).fill = PatternFill(
    ...
    exec_row += 1
    ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
    rec_cell = ws_exec.cell(
        row=exec_row,
        column=2,
        value="  RECOMMENDATION: Allocate 5-10% of total recruitment marketing budget to employer branding for sustained pipeline quality improvement.",
    )
    ...
    rec_cell.border = Border(
        left=Side(style="thick", color=ACCENT), bottom=Side(style="thin", color=ACCENT)
    )
    for c in range(3, 8):
    ...
    exec_row += 3
    style_section_header(
        ws_exec, exec_row, 2, 7, "Quality of Hire — Expected Outcomes by Channel Type"
    )
    exec_row += 1
    ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
    ws_exec.cell(
        row=exec_row,
        column=2,
        value="Not all channels deliver equal quality. This framework shows expected quality-of-hire outcomes by sourcing channel, helping optimize spend toward highest-quality pipelines.",
    ).font = Font(name="Calibri", italic=True, size=9, color="596780")
    exec_row += 2
    ...
    _CHANNEL_QUALITY_META = {
        "programmatic": (
            ...
            "78-85%",
            4,
            "High-volume hiring; ML optimization improves quality over time",
        ),
        "programmatic_dsp": (
            "18-25 days",
            "78-85%",
        ...
        "job_board": (
            ...
            "72-80%",
            3,
            "Broad reach; consistent pipeline for standard roles",
        ),
        "niche_board": (
            "25-35 days",
            "82-90%",
            5,
            "Specialized roles; highest quality match; lower volume",
        ),
        "social_media": (
            "20-30 days",
            "70-78%",
            3,
            "Passive candidates; employer brand-driven; younger demographics",
        ),
        "social": (
            "20-30 days",
            "70-78%",
        ...
        "referral": (
            ...
            "88-95%",
            5,
            "Highest retention; fastest fill; limited scale",
        ),
        "employer_branding": (
            "30-45 days",
            "85-92%",
            5,
            "Long-term pipeline; highest quality but slower ROI",
        ),
        "events": (
            "25-40 days",
            "75-82%",
            3,
            "Face-to-face engagement; strong for campus/entry-level",
        ),
        "staffing_agency": (
            "10-15 days",
            "60-70%",
            2,
            "Fastest fill; lowest retention; highest per-hire cost",
        ),
        "search_engine": (
            "20-28 days",
            "74-82%",
            3,
            "Intent-driven candidates via Google/Bing job ads",
        ),
        "display": (
            "25-35 days",
            "65-75%",
            2,
            "Brand awareness and passive candidate re-engagement",
        ),
    }
    _qoh_budget = data.get("_budget_allocation", {})
    _qoh_ch_allocs = (
        _qoh_budget.get("channel_allocations", {})
        if isinstance(_qoh_budget, dict)
        else {}
    )
    if _qoh_ch_allocs and isinstance(_qoh_ch_allocs, dict):
        ...
        for ch_name, ch_data in _qoh_ch_allocs.items():
            ...
            meta = _CHANNEL_QUALITY_META.get(
                category, _CHANNEL_QUALITY_META.get("job_board")
            )
            label = ch_name.replace("_", " ").title()
            if "joveo" in ch_name.lower() or category in (
                "programmatic",
                "programmatic_dsp",
            ):
    ...
    for idx, (ch_type, ttf, retention, quality, best_for) in enumerate(qoh_data):
        ...
        c1.font = Font(
            name="Calibri", bold=True, size=10, color="2E75B6" if is_joveo else "1B2A4A"
        )
        c1.border = thin_border
    ...
    exec_row += 1
    ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
    joveo_rec = ws_exec.cell(
        row=exec_row,
        column=2,
        value="  Joveo's programmatic approach combines the speed of automation with quality optimization (CPQA), delivering the best balance of time-to-fill, retention, and cost efficiency.",
    )
    ...
    ws_exec.cell(
        row=exec_row,
        column=2,
        value="Key performance indicators estimated from your channel mix, industry benchmarks, and campaign parameters.",
    ).font = Font(name="Calibri", italic=True, size=9, color="596780")
    exec_row += 2
    ...
    if isinstance(_qm_total_proj, dict) and _qm_total_proj.get("hires") or 0 > 0:
        ...
        est_applies = int(_qm_total_proj.get("applications") or 0)
        _qm_cpa = _qm_total_proj.get("cost_per_application")
        est_cpa = (
            round(float(_qm_cpa), 2)
            if isinstance(_qm_cpa, (int, float)) and _qm_cpa > 0
            else round(budget_midpoint / max(est_applies, 1), 2)
        )
        _qm_cph = _qm_total_proj.get("cost_per_hire")
        est_cph_display = (
            round(float(_qm_cph), 0)
            if isinstance(_qm_cph, (int, float)) and _qm_cph > 0
            else round(budget_midpoint / max(est_hires, 1), 0)
        )
        ...
        num_channels = (
            ...
            else (regional_count + niche_count + global_count)
        )
    else:
        ...
        est_cpa = (
            round(budget_midpoint / max(est_applies, 1), 2) if est_applies > 0 else 0
        )
        est_cph_display = (
            round(budget_midpoint / max(est_hires, 1), 0) if est_hires > 0 else avg_cph
        )
    channel_diversity = min(round(num_channels / 10 * 100, 0), 100)  # score out of 100

    quality_metrics = [
        (
            f"{est_total_reach:,.0f}",
            "Estimated Total Reach",
            "Total impressions across all selected channels",
        ),
        (
            f"{display_currency}{est_cpa:,.2f}",
            "Est. Cost Per Application",
            "Budget-calibrated CPA based on industry benchmarks",
        ),
        (
            f"{display_currency}{est_cph_display:,.0f}",
            "Est. Cost Per Hire",
            f"Based on {client_industry.replace('_', ' ').title()} industry benchmarks",
        ),
        (
            f"{est_hires:,}",
            "Projected Hires",
            f"Across {num_channels} channels in {len(normalized) if normalized else 0} categories",
        ),
    ]
    ...
    style_section_header(
        ...
        2,
        7,
        "Peer Industry Benchmark Comparison — How Your Industry Compares",
    )
    exec_row += 1
    ws_exec.merge_cells(f"B{exec_row}:G{exec_row}")
    ws_exec.cell(
    ...
    ws_exec.cell(row=exec_row, column=2, value="ALL-INDUSTRY AVG").font = Font(
        name="Calibri", bold=True, size=10, color="FFFFFF"
    )
    ...
    ws_exec.cell(
        row=exec_row,
        column=2,
        value="Your industry row is highlighted in blue. \u25b2 = easier than average (green), \u25bc = harder than average (amber). Use this to calibrate budget expectations.",
    ).font = Font(name="Calibri", italic=True, size=8, color="999999")
    ...
    ws_trends.merge_cells(
        start_row=2, start_column=2, end_row=2, end_column=2 + len(locations)
    )
    ...
    market_trends = research.get_market_trends(
        locations, industry, roles, enrichment_salary_data=enrichment_salary_data
    )

    for trend in market_trends:
        row += 1
        style_body_cell(ws_trends, row, 2, trend.get("factor") or "")
        ws_trends.cell(row=row, column=2).font = Font(
            name="Calibri", bold=True, size=10
        )
        for i, loc in enumerate(locations):
    ...
    company_name = data.get("company_name") or "" or data.get("client_name") or ""
    competitors = research.get_competitors(
        industry, locations, company_name=company_name
    )

    for comp in competitors:
        row += 1
        style_body_cell(ws_trends, row, 2, comp.get("category") or "")
        ws_trends.cell(row=row, column=2).font = Font(
            name="Calibri", bold=True, size=10
        )
        style_body_cell(ws_trends, row, 3, comp.get("competitors") or "")
        style_body_cell(ws_trends, row, 4, comp.get("threat") or "")
    ...
    _client_name_lower = (data.get("client_name") or "" or "").lower().strip()
    if _client_name_lower and client_competitors:
        client_competitors = [
            c for c in client_competitors if _client_name_lower not in c.lower()
        ]
    if client_competitors:
        comp_intel = research.get_client_competitor_intelligence(
            client_competitors, industry
        )

        row += 3
        style_section_header(
            ws_trends, row, 2, 5, "Client-Identified Competitor Intelligence"
        )
        row += 1
        ws_trends.cell(
            row=row,
            column=2,
            value="Detailed competitive intelligence for each client-specified competitor, including hiring channels, employer brand analysis, and strategic recommendations.",
        ).font = Font(name="Calibri", italic=True, size=10, color="596780")
        ...
        for i, h in enumerate(ci_headers):
            ...
            cell.font = header_font
            cell.fill = PatternFill(
                start_color="CE9047", end_color="CE9047", fill_type="solid"
            )
            cell.alignment = center_alignment
            cell.border = thin_border
        ...
        for ci in comp_intel:
            ...
            # Recruitment strategies
            style_body_cell(
                ws_trends, row, 4, ci.get("known_recruitment_strategies") or ""
            )

            # Strategic recommendation
            style_body_cell(ws_trends, row, 5, ci.get("strategic_recommendation") or "")
    ...
    ws_labour.cell(
        row=lm_row,
        column=2,
        value="BLS/JOLTS-style curated data for the selected industry and target locations. Use this intelligence to inform recruitment strategy, budget allocation, and candidate messaging.",
    ).font = Font(name="Calibri", italic=True, size=10, color="596780")
    ...
    ws_labour.merge_cells(f"B{lm_row}:D{lm_row}")
    ws_labour.cell(
        row=lm_row, column=2, value="National Economic Snapshot (US)"
    ).font = Font(name="Calibri", bold=True, size=14, color="FFFFFF")
    ws_labour.cell(row=lm_row, column=2).fill = PatternFill(
        start_color="1B2A4A", end_color="1B2A4A", fill_type="solid"
    )
    lm_row += 1
    ...
    national_items = [
        ("Total Nonfarm Job Openings", national.get("total_nonfarm_openings") or ""),
        (
            "National Unemployment Rate",
            national.get("national_unemployment_rate") or "",
        ),
        (
            "Labour Force Participation Rate",
            national.get("labour_force_participation") or "",
        ),
        ("U-6 Underemployment Rate", national.get("u6_underemployment") or ""),
        ("National Quits Rate (JOLTS)", national.get("national_quits_rate") or ""),
        (
            "National Openings Rate (JOLTS)",
            national.get("national_openings_rate") or "",
        ),
        (
            "Avg Hourly Earnings (All Workers)",
            national.get("avg_hourly_earnings_all") or "",
        ),
        ("Wage Growth YoY", national.get("avg_hourly_earnings_yoy_change") or ""),
        ("Jobs-to-Unemployed Ratio", national.get("jobs_to_unemployed_ratio") or ""),
    ]
    for label, value in national_items:
        ws_labour.cell(row=lm_row, column=2, value=label).font = Font(
            name="Calibri", bold=True, size=10, color="1B2A4A"
        )
    ...
    sector_name = ind_metrics.get(
        "sector_name", industry_label_map.get(industry, industry)
    )
    ws_labour.merge_cells(f"B{lm_row}:D{lm_row}")
    ws_labour.cell(
        row=lm_row, column=2, value=f"Industry Focus: {sector_name}"
    ).font = Font(name="Calibri", bold=True, size=14, color="FFFFFF")
    ws_labour.cell(row=lm_row, column=2).fill = PatternFill(
        start_color="2E75B6", end_color="2E75B6", fill_type="solid"
    )
    ...
    ind_items = [
        ("BLS Sector Code", ind_metrics.get("bls_sector_code") or ""),
        ("Total US Employment", ind_metrics.get("total_employment_us") or ""),
        (
            "Projected Growth (2024-2034)",
            ind_metrics.get("projected_growth_2024_2034") or "",
        ),
        ("Annual Job Openings", ind_metrics.get("annual_openings") or ""),
        ("Median Annual Wage", ind_metrics.get("median_annual_wage") or ""),
        ("JOLTS Openings Rate", ind_metrics.get("job_openings_rate_jolts") or ""),
        ("JOLTS Quits Rate", ind_metrics.get("quits_rate_jolts") or ""),
        ("JOLTS Hires Rate", ind_metrics.get("hires_rate_jolts") or ""),
        ("JOLTS Layoffs Rate", ind_metrics.get("layoffs_rate_jolts") or ""),
        ("Avg Time to Fill Vacancy", ind_metrics.get("vacancy_fill_time_avg") or ""),
        ("Talent Shortage Severity", ind_metrics.get("talent_shortage_severity") or ""),
        ("Wage Growth YoY", ind_metrics.get("wage_growth_yoy") or ""),
        ("Unionization Rate", ind_metrics.get("unionization_rate") or ""),
        ("Remote Work %", ind_metrics.get("remote_work_pct") or ""),
    ]
    ...
    ws_labour.merge_cells(f"B{lm_row}:D{lm_row}")
    ws_labour.cell(
        row=lm_row, column=2, value="Key Industry Trends & Hiring Implications"
    ).font = Font(name="Calibri", bold=True, size=14, color="FFFFFF")
    ws_labour.cell(row=lm_row, column=2).fill = PatternFill(
        start_color="438765", end_color="438765", fill_type="solid"
    )
    lm_row += 1
    ...
    if loc_contexts:
        ...
        ws_labour.merge_cells(f"B{lm_row}:D{lm_row}")
        ws_labour.cell(
            row=lm_row, column=2, value="Location-Specific Labour Market Context"
        ).font = Font(name="Calibri", bold=True, size=14, color="FFFFFF")
        ws_labour.cell(row=lm_row, column=2).fill = PatternFill(
            start_color="CE9047", end_color="CE9047", fill_type="solid"
        )
        ...
        for lctx in loc_contexts:
            style_body_cell(ws_labour, lm_row, 2, lctx.get("location") or "")
            ws_labour.cell(row=lm_row, column=2).font = Font(
                name="Calibri", bold=True, size=10
            )
            style_body_cell(ws_labour, lm_row, 3, lctx.get("unemployment_rate") or "")
            style_body_cell(ws_labour, lm_row, 4, lctx.get("median_salary") or "")
            style_body_cell(ws_labour, lm_row, 5, lctx.get("population") or "")
            style_body_cell(ws_labour, lm_row, 6, str(lctx.get("coli") or ""))
            style_body_cell(ws_labour, lm_row, 7, lctx.get("context_note") or "")
            ws_labour.row_dimensions[lm_row].height = 35
            lm_row += 1
    ...
    if salary_data:
        lm_row += 2
        style_section_header(
            ws_labour, lm_row, 2, 7, "Real-Time Salary Benchmarks (BLS)"
        )
        lm_row += 1
        # Headers
        for ci, hdr in enumerate(
            ["Role", "Mean / Median Salary", "10th %ile", "90th %ile", "Source"],
            start=2,
        ):
        ...
        for role_name, sdata in salary_data.items():
            ...
            _med = sdata.get("median") or sdata.get("mean") or 0
            ...
            ws_labour.cell(
                ...
                value=(
                    f"${_med:,.0f} ({_label})"
                    if isinstance(_med, (int, float))
                    else str(_med)
                ),
            ).font = Font(name="Calibri", size=10, bold=True, color="2E7D32")
            _p10 = sdata.get("p10") or 0
            ws_labour.cell(
                row=lm_row,
                column=4,
                value=f"${_p10:,.0f}" if isinstance(_p10, (int, float)) else str(_p10),
            ).font = body_font
            _p90 = sdata.get("p90") or 0
            ws_labour.cell(
                row=lm_row,
                column=5,
                value=f"${_p90:,.0f}" if isinstance(_p90, (int, float)) else str(_p90),
            ).font = body_font
            ws_labour.cell(
                row=lm_row, column=6, value=sdata.get("source", "BLS")
            ).font = Font(name="Calibri", italic=True, size=9, color="596780")
            for ci in range(2, 7):
    ...
    if location_demos:
        lm_row += 2
        style_section_header(
            ws_labour, lm_row, 2, 7, "Location Demographics (Live Data)"
        )
        lm_row += 1
        for ci, hdr in enumerate(
            [
        ...
        for loc_name, ldata in location_demos.items():
            ...
            ws_labour.cell(
                ...
                value=(
                    ...
                    else ("N/A" if not pop else str(pop))
                ),
            ).font = body_font
            ...
            ws_labour.cell(
                ...
                value=(
                    ...
                    else ("N/A" if not inc else str(inc))
                ),
            ).font = body_font
            ...
            matched_place = (
                ldata.get("matched_place")
                or ""
                or ldata.get("state_name")
                or ""
                or ldata.get("country")
                or ""
            )
            ws_labour.cell(row=lm_row, column=6, value=matched_place).font = body_font
            ws_labour.cell(
                row=lm_row, column=7, value=ldata.get("source") or ""
            ).font = Font(name="Calibri", italic=True, size=9, color="596780")
            for ci in range(2, 8):
                ws_labour.cell(row=lm_row, column=ci).border = thin_border
                ws_labour.cell(row=lm_row, column=ci).alignment = center_alignment
    ...
    if global_indicators:
        lm_row += 2
        style_section_header(
            ws_labour, lm_row, 2, 7, "Global Economic Indicators (World Bank)"
        )
        lm_row += 1
        for ci, hdr in enumerate(
            ["Country", "Unemployment Rate", "GDP Growth", "Labor Force", "Source"],
            start=2,
        ):
            c = ws_labour.cell(row=lm_row, column=ci, value=hdr)
            c.font = Font(name="Calibri", bold=True, size=10, color="FFFFFF")
        ...
        for country, gdata in global_indicators.items():
            ...
            ws_labour.cell(
                row=lm_row, column=3, value=f"{unemp}%" if unemp else "N/A"
            ).font = body_font
            gdp = gdata.get("gdp_growth")
            ws_labour.cell(
                row=lm_row, column=4, value=f"{gdp}%" if gdp else "N/A"
            ).font = body_font
            ...
            ws_labour.cell(
                ...
                value=(
                    ...
                    else ("N/A" if not lf else str(lf))
                ),
            ).font = body_font
            ws_labour.cell(
                row=lm_row, column=6, value=gdata.get("source", "World Bank")
            ).font = Font(name="Calibri", italic=True, size=9, color="596780")
            for ci in range(2, 7):
                ws_labour.cell(row=lm_row, column=ci).border = thin_border
                ws_labour.cell(row=lm_row, column=ci).alignment = center_alignment
    ...
    if hiring_regs:
        ...
        ws_labour.merge_cells(f"B{lm_row}:D{lm_row}")
        ws_labour.cell(
            row=lm_row, column=2, value="Hiring Compliance & Regulatory Notes"
        ).font = Font(name="Calibri", bold=True, size=14, color="FFFFFF")
        ws_labour.cell(row=lm_row, column=2).fill = PatternFill(
        ...
        ws_labour.cell(
            row=lm_row,
            column=2,
            value="Applicable hiring regulations by state/jurisdiction. Ensure job postings and hiring processes comply with all listed requirements.",
        ).font = Font(name="Calibri", italic=True, size=9, color="596780")
        ws_labour.merge_cells(f"B{lm_row}:D{lm_row}")
        lm_row += 1
        ...
        for reg_entry in hiring_regs:
            ...
            ws_labour.cell(
                row=lm_row, column=2, value=f"{reg_state} ({reg_loc})"
            ).font = Font(name="Calibri", bold=True, size=11, color="CC0000")
            ws_labour.merge_cells(f"B{lm_row}:D{lm_row}")
            lm_row += 1

            for reg in reg_list:
                ws_labour.cell(row=lm_row, column=2, value=f"  \u2022  {reg}").font = (
                    Font(name="Calibri", size=10, color="333333")
                )
                ws_labour.merge_cells(f"B{lm_row}:D{lm_row}")
                lm_row += 1

            ws_labour.cell(row=lm_row, column=2, value=compliance_note).font = Font(
                name="Calibri", italic=True, size=9, color="596780"
            )
            ws_labour.merge_cells(f"B{lm_row}:D{lm_row}")
            lm_row += 1
    ...
    ws_labour.cell(
        row=lm_row,
        column=2,
        value="Data Sources: BLS Occupational Employment & Wage Statistics, JOLTS (Job Openings & Labor Turnover Survey), BLS Employment Projections, industry reports. Curated reference data as of 2024.",
    ).font = Font(name="Calibri", italic=True, size=9, color="888888")
    ws_labour.merge_cells(f"B{lm_row}:D{lm_row}")
    ...
    strat_headers = [
        ...
        "How to Use",
        "KPIs / Metrics",
        "Niche / Non-Traditional Channels",
    ]
    ...
    for strat in selected_strategies:
        row += 1
        style_body_cell(ws_strategy, row, 2, strat.get("channel") or "")
        ws_strategy.cell(row=row, column=2).font = Font(
            name="Calibri", bold=True, size=10
        )
        style_body_cell(ws_strategy, row, 3, strat.get("reasoning") or "")
        style_body_cell(ws_strategy, row, 4, strat.get("usage") or "")
        style_body_cell(
            ws_strategy,
            row,
            5,
            strat.get("kpis", "Reach, Engagement, CTR, Conversions"),
        )
        # Populate Niche / Non-Traditional Channels column with 2-3 industry-specific niche channels per row
        if _niche_channels_for_industry and _niche_idx < len(
            _niche_channels_for_industry
        ):
            ...
            ws_strategy.cell(row=row, column=6).font = Font(
                name="Calibri", size=10, color="0A66C9"
            )
            _niche_idx = _end_idx
    ...
    row += 3
    style_section_header(
        ws_strategy, row, 2, 6, "Industry-Specific Niche Channel Recommendations"
    )
    row += 1
    ...
    ws_strategy.cell(
        row=row,
        column=2,
        value=f"Recommended niche and non-traditional channels for the {data.get('industry_label', industry.replace('_', ' ').title())} industry. These specialized platforms offer higher-quality candidates for targeted roles.",
    ).font = Font(name="Calibri", italic=True, size=10, color="596780")
    row += 1
    ...
    for _nch in _niche_channels_for_industry:
        ...
        ws_strategy.cell(row=row, column=2).font = Font(
            name="Calibri", bold=True, size=10, color="0A66C9"
        )
        ...
        style_body_cell(
            ...
            row,
            4,
            f"Specialized {data.get('industry_label', industry.replace('_', ' ').title())} talent",
        )
        row += 1
    ...
    _eff_ch_allocs = (
        _eff_budget.get("channel_allocations", {})
        if isinstance(_eff_budget, dict)
        else {}
    )
    if _eff_ch_allocs and isinstance(_eff_ch_allocs, dict):
        ...
        for _eff_ch_name, _eff_ch_data in _eff_ch_allocs.items():
            if isinstance(_eff_ch_data, dict):
                ...
                _eff_score = (
                    min(int(round(float(_eff_roi) * 10)), 100)
                    if isinstance(_eff_roi, (int, float))
                    else 50
                )
    ...
    bar_data_ref = Reference(
        ws_strategy, min_col=3, min_row=bar_data_start, max_row=bar_data_end
    )
    bar_cats = Reference(
        ws_strategy, min_col=2, min_row=bar_data_start + 1, max_row=bar_data_end
    )
    ...
    campus_recs = research.get_campus_recruiting_recommendations(
        locations, roles, industry
    )
    if campus_recs:
        row += 18  # Space for the bar chart above
        style_section_header(
            ws_strategy, row, 2, 5, "Campus Recruiting Recommendations"
        )
        row += 1
        ws_strategy.cell(
            row=row,
            column=2,
            value="Recommended universities based on target locations. Leverage campus career fairs, on-campus events, and university job boards.",
        ).font = Font(name="Calibri", italic=True, size=9, color="596780")
        ...
        for rec in campus_recs:
            uni_name = rec.get("university") or ""
            ...
            style_body_cell(ws_strategy, row, 3, rec.get("programs") or "")
            style_body_cell(ws_strategy, row, 4, rec.get("enrollment") or "")
            style_body_cell(ws_strategy, row, 5, rec.get("recruiting_channel") or "")
    ...
    _trad_pubs_str = (
        f"{_trad_pubs:,}" if isinstance(_trad_pubs, (int, float)) else str(_trad_pubs)
    )
    ws_trad["B3"].value = (
        f"Target: {', '.join(locations)}{roles_str} | Joveo Supply Network: {_trad_pubs_str}+ active publishers"
    )
    ...
    max_rows = max(
        len(regional), len(niche_channels), len(global_channels), len(location_boards)
    )
    ...
    ws_trad.cell(
        row=row, column=2, value="Additional Joveo Supply Partners by Category"
    ).font = Font(name="Calibri", bold=True, size=12, color="2E75B6")
    row += 1
    ...
    if _ind_nt_channels:
        _ind_label = (
            db.get("industries", {})
            .get(_ind_key_nt, {})
            .get("label", _ind_key_nt.replace("_", " ").title())
        )
        categories.append(
            (f"Industry-Specific Channels ({_ind_label})", _ind_nt_channels)
        )

    # Enrich with Joveo supply partner data
    joveo_nt_cats = {
        "Staffing Partners": joveo_pubs.get("by_category", {}).get(
            "Staffing Partner", []
        )[:10],
        "AI-Powered Sourcing Tools": joveo_pubs.get("by_category", {}).get(
            "AI tool", []
        )[:10],
        "Influencer Marketing Platforms": joveo_pubs.get("by_category", {}).get(
            "Influencer Marketing", []
        )[:10],
        "Programmatic Audio Partners": joveo_pubs.get("by_category", {}).get(
            "Programmatic Audio", []
        )[:10],
    }
    if include_programmatic:
    ...
    if include_apac and (apac_social or apac_classifieds):
        ...
        if has_international or has_apac_data:
            if apac_social:
                cell = ws_nontrad.cell(
                    row=row, column=2, value="APAC Local Social Platforms"
                )
                cell.font = section_font
                cell.fill = section_fill
                cell.border = thin_border
                ...
                for country, platforms in apac_social.items():
                    style_body_cell(
                        ws_nontrad, row, 2, f"  {country.replace('_', ' ').title()}"
                    )
            ...
            if apac_classifieds:
                cell = ws_nontrad.cell(
                    row=row, column=2, value="APAC Local Classifieds"
                )
                cell.font = section_font
                cell.fill = section_fill
                cell.border = thin_border
    ...
    if has_international or data.get("include_global_supply"):
        ...
        for c in range(3, 8):
            ws_global.cell(row=row, column=c).fill = PatternFill(
                start_color="1F4E79", end_color="1F4E79", fill_type="solid"
            )
        ...
        for cb in country_boards:
            ...
            for idx, board in enumerate(boards):
                ...
                style_body_cell(ws_global, row, 3, board.get("name") or "")
                style_body_cell(ws_global, row, 4, board.get("billing") or "")
                style_body_cell(ws_global, row, 5, board.get("category") or "")
                style_body_cell(ws_global, row, 6, board.get("tier") or "")
        ...
        row += 1
        cell = ws_global.cell(
            row=row, column=2, value="Push vs Pull Strategy Recommendations"
        )
        cell.font = Font(name="Calibri", bold=True, size=12, color="FFFFFF")
        cell.fill = PatternFill(
            start_color="1F4E79", end_color="1F4E79", fill_type="solid"
        ...
        for strategy_type in ["pull_advertising", "push_advertising"]:
            ...
            if strat:
                cell = ws_global.cell(
                    row=row, column=2, value=strategy_type.replace("_", " ").title()
                )
                ...
                style_body_cell(ws_global, row, 3, strat.get("description") or "")
                ws_global.merge_cells(
                ...
                style_body_cell(ws_global, row, 3, strat.get("best_for") or "")
                ws_global.merge_cells(
                    start_row=row, start_column=3, end_row=row, end_column=7
                )
                row += 1
    ...
    if data.get("include_dei"):
        ...
        for c in range(3, 6):
            ws_dei.cell(row=row, column=c).fill = PatternFill(
                start_color="1F4E79", end_color="1F4E79", fill_type="solid"
            )
        ...
        for region_name, boards in dei_by_country.items():
            ...
            board_list = (
                ...
                else boards.get("boards", boards) if isinstance(boards, dict) else []
            )
            if isinstance(board_list, list):
                for board in board_list:
                    if isinstance(board, dict):
                        ...
                        style_body_cell(
                            ws_dei, row, 4, board.get("regions", region_name)
                        )
                        if row % 2 == 0:
                            for c in range(2, 5):
        ...
        for board in women_boards:
            style_body_cell(ws_dei, row, 2, board.get("name") or "")
            style_body_cell(ws_dei, row, 3, board.get("focus") or "")
            style_body_cell(ws_dei, row, 4, board.get("regions") or "")
            if row % 2 == 0:
                for c in range(2, 5):
                    ws_dei.cell(row=row, column=c).fill = accent_fill
            row += 1
        ...
        row += 2
        cell = ws_dei.cell(
            row=row, column=2, value="Industry-Specific Diversity Channels"
        )
        cell.font = Font(name="Calibri", bold=True, size=12, color="FFFFFF")
        cell.fill = PatternFill(
            start_color="1F4E79", end_color="1F4E79", fill_type="solid"
    ...
    if data.get("include_innovative"):
        ...
        ws_innov["B3"].value = (
            "Emerging recruitment channels: CTV, DOOH, Retail Media, Gaming, Podcasts, Messaging Apps & more"
        )
        ...
        for channel_key, channel_data in innovative.items():
            if isinstance(channel_data, dict):
                ...
                ws_innov.cell(row=row, column=2).font = Font(
                    name="Calibri", bold=True, size=10
                )
        ...
        for c in range(3, 7):
            ws_innov.cell(row=row, column=c).fill = PatternFill(
                start_color="1F4E79", end_color="1F4E79", fill_type="solid"
            )
    ...
    if data.get("include_budget_guide"):
        ...
        for c in range(3, 6):
            ws_budget.cell(row=row, column=c).fill = PatternFill(
                start_color="1F4E79", end_color="1F4E79", fill_type="solid"
            )
        ...
        for model_name, model_data in billing_models.items():
            ...
            ws_budget.cell(row=row, column=2).font = Font(
                name="Calibri", bold=True, size=10
            )
            style_body_cell(ws_budget, row, 3, model_data.get("description") or "")
            style_body_cell(ws_budget, row, 4, model_data.get("typical_range") or "")
            if row % 2 == 0:
                for c in range(2, 5):
        ...
        row += 2
        cell = ws_budget.cell(
            row=row, column=2, value="CPA Rate Benchmarks (Typical Ranges)"
        )
        cell.font = Font(name="Calibri", bold=True, size=12, color="FFFFFF")
        cell.fill = PatternFill(
            start_color="1F4E79", end_color="1F4E79", fill_type="solid"
        ...
        if isinstance(cpa_from_db, dict) and cpa_from_db:
            ...
            for region_key, region_data in cpa_from_db.items():
                ...
                region_label = _region_labels.get(
                    region_key.lower(), region_key.replace("_", " ").title()
                )
                ...
                notes_str = (
                    f"Key publishers: {', '.join(tier_publishers[:5])}"
                    if tier_publishers
                    else ""
                )
        ...
        if isinstance(_synth_ad, dict) and _synth_ad:
            ...
            if _plat_cpas:
                cpa_benchmarks.append(
                    (
                        "Platform-Specific (Synthesized)",
                        " | ".join(_plat_cpas[:6]),
                        "CPA from ad platform analysis synthesis",
                    )
                )
        ...
        if not cpa_benchmarks:
            cpa_benchmarks = [
                (
                    ...
                    "$15 - $45",
                    "High competition, strong CPC performance on Indeed/ZipRecruiter",
                ),
                (
                    "Europe (UK/DE/FR/NL)",
                    "$12 - $40",
                    "Mixed CPC/Posting models; StepStone, Reed, Totaljobs common",
                ),
                (
                    "APAC (India/Japan/AU/SG)",
                    "$5 - $30",
                    "Lower CPAs in India; premium in Japan/AU/Singapore",
                ),
                (
                    "LATAM (Brazil/Mexico/Argentina)",
                    "$3 - $20",
                    "Cost-effective; CompuTrabajo, OCC Mundial popular",
                ),
                (
                    "MEA (UAE/South Africa/Kenya)",
                    "$5 - $25",
                    "Growing market; Bayt.com, CareerJunction dominant",
                ),
            ]
        for i, h in enumerate(
            ["Region", f"CPA Range ({display_currency_code})", "Notes"]
        ):
            cell = ws_budget.cell(row=row, column=2 + i, value=h)
            cell.font = subheader_font
            cell.fill = subheader_fill
        ...
        for region, cpa_range, notes in cpa_benchmarks:
            ...
            if region_key.startswith("Platform") or any(
                kw in location_str for kw in keywords
            ):
        ...
        if not filtered_benchmarks:
            filtered_benchmarks = [
                b
                for b in cpa_benchmarks
                if "North America" in b[0] or "Platform" in b[0]
            ]
        ...
        row += 2
        cell = ws_budget.cell(
            row=row, column=2, value="Recommended Budget Allocation by Channel Type"
        )
        cell.font = Font(name="Calibri", bold=True, size=12, color="FFFFFF")
        cell.fill = PatternFill(
            start_color="1F4E79", end_color="1F4E79", fill_type="solid"
        ...
        _pie_ch_allocs = (
            _pie_budget.get("channel_allocations", {})
            if isinstance(_pie_budget, dict)
            else {}
        )
        if _pie_ch_allocs and isinstance(_pie_ch_allocs, dict):
            ...
            for _pie_ch_name, _pie_ch_data in _pie_ch_allocs.items():
                if isinstance(_pie_ch_data, dict):
                    _pie_pct = _pie_ch_data.get(
                        "percentage", _pie_ch_data.get("pct") or 0
                    )
        ...
        pie_labels = Reference(
            ws_budget, min_col=2, min_row=pie_data_start + 1, max_row=pie_data_end
        )
        pie_values = Reference(
            ws_budget, min_col=3, min_row=pie_data_start, max_row=pie_data_end
        )
        ...
        _bp_ch_allocs = (
            _bp_budget_alloc.get("channel_allocations", {})
            if isinstance(_bp_budget_alloc, dict)
            else {}
        )
        if isinstance(_bp_ch_allocs, dict) and _bp_ch_allocs:
            row += 2
            cell = ws_budget.cell(
                row=row, column=2, value="Calculated Budget Breakdown (Projected)"
            )
            cell.font = Font(name="Calibri", bold=True, size=12, color="FFFFFF")
            cell.fill = PatternFill(
                start_color="1F4E79", end_color="1F4E79", fill_type="solid"
            ...
            _bp_headers = [
                "Channel",
                f"$ Amount ({display_currency_code})",
                "Projected Hires",
                "Cost/Hire",
            ]
            ...
            for ch_name, ch_data in _bp_ch_allocs.items():
                if not isinstance(ch_data, dict):
                    continue
                _bp_amt = ch_data.get("dollar_amount", ch_data.get("amount") or 0)
                _bp_hires = ch_data.get("projected_hires") or 0
                _bp_cph = ch_data.get("cost_per_hire") or 0
                ...
                style_body_cell(
                    ...
                    (
                        f"${_bp_amt:,.0f}"
                        if isinstance(_bp_amt, (int, float))
                        else str(_bp_amt)
                    ),
                )
                style_body_cell(
                    ...
                    (
                        f"{_bp_hires:,.1f}"
                        if isinstance(_bp_hires, (int, float))
                        else str(_bp_hires)
                    ),
                )
                style_body_cell(
                    ...
                    (
                        f"${_bp_cph:,.0f}"
                        if isinstance(_bp_cph, (int, float)) and _bp_cph > 0
                        else "N/A"
                    ),
                )
                if row % 2 == 0:
            ...
            ws_budget.cell(
                row=row,
                column=2,
                value="Dollar amounts computed by the budget engine based on industry allocation percentages, role complexity, and location cost multipliers.",
            ).font = Font(name="Calibri", italic=True, size=8, color="999999")
    ...
    if job_categories and job_cat_db:
        ...
        jc_title.fill = PatternFill(
            start_color="1B2A4A", end_color="1B2A4A", fill_type="solid"
        )
        jc_title.alignment = Alignment(horizontal="center", vertical="center")
        for c in range(3, 7):
            ws_jc.cell(row=2, column=c).fill = PatternFill(
                start_color="1B2A4A", end_color="1B2A4A", fill_type="solid"
            )
        ...
        for c in range(3, 7):
            ws_jc.cell(row=3, column=c).fill = PatternFill(
                start_color="1B2A4A", end_color="1B2A4A", fill_type="solid"
            )
        ...
        for cat_key in job_categories:
            ...
            cell = ws_jc.cell(
                row=jc_row,
                column=2,
                value=f"{cat.get('icon') or ''} {cat.get('label', cat_key)}",
            )
            ...
            for c in range(3, 7):
                ws_jc.cell(row=jc_row, column=c).fill = PatternFill(
                    start_color="2E75B6", end_color="2E75B6", fill_type="solid"
                )
            ...
            ws_jc.merge_cells(f"B{jc_row}:F{jc_row}")
            ws_jc.cell(
                row=jc_row, column=2, value=cat.get("description") or ""
            ...
            metric_items = [
                ...
                (
                    "Awareness %",
                    (
                        str(cat.get("strategy_emphasis", {}).get("awareness") or "")
                        + "%"
                        if cat.get("strategy_emphasis")
                        else "N/A"
                    ),
                ),
                (
                    "Hiring %",
                    (
                        str(cat.get("strategy_emphasis", {}).get("hiring") or "") + "%"
                        if cat.get("strategy_emphasis")
                        else "N/A"
                    ),
                ),
                ...
                if ch_list:
                    ...
                    if "Primary" in ch_label:
                        ...
                        style_body_cell(
                            ...
                            jc_row,
                            4,
                            f"High-volume, proven ROI — allocate {_pri_range} of budget",
                        )
                        ...
                        style_body_cell(
                            ...
                            jc_row,
                            4,
                            f"Supplementary reach — allocate {_sec_range} of budget",
                        )
                        ...
                        style_body_cell(
                            ...
                            jc_row,
                            4,
                            f"Brand awareness + retargeting — allocate {_soc_range} of budget",
                        )
                        ...
                        style_body_cell(
                            ...
                            jc_row,
                            4,
                            f"Targeted specialists — allocate {_niche_range} of budget",
                        )
            ...
            # Joveo Supply Fit
            jc_row += 1
            ws_jc.cell(row=jc_row, column=2, value="Joveo Supply Fit").font = Font(
            ...
            # Competitor Channels
            ws_jc.cell(row=jc_row, column=2, value="Competitor Channels").font = Font(
    ...
    ws_timeline["B3"].value = (
        f"Client: {data.get('client_name') or ''}  |  Duration: {duration_display}"
    )
    ...
    phase_colors = {
        ...
        "Phase 4": PatternFill(
            start_color="ECFEFF", end_color="ECFEFF", fill_type="solid"
        ),  # pale teal
    }
    ...
    if cw <= 12:
        phases = [
            (
                ...
                "Research & Setup",
                "Weeks 1-2",
                "Market analysis, account setup, creative development",
                "N/A",
                "Setup completion",
                "5%",
            ),
            (
                ...
                "Launch & Optimize",
                f"Weeks 3-{min(6, cw)}",
                "Initial campaign launch, A/B testing, bid optimization",
                "All programmatic",
                "CPC, CPA, Apply rate",
                "30%",
            ),
            (
                ...
                "Scale & Expand",
                f"Weeks {min(7, cw)}-{cw}",
                "Increase spend on top performers, add new channels",
                "Top 5 channels",
            ...
            (
                ...
                "Sustain & Refine",
                "Ongoing",
                "Maintain performance, quarterly reviews, seasonal adjustments",
                "Proven channels",
                "ROI, Time to fill",
                "25%",
            ),
        ...
        phases = [
            (
                ...
                "Research & Setup",
                "Weeks 1-3",
                "Market analysis, account setup, creative development, employer brand audit",
                "N/A",
                "Setup completion",
                "5%",
            ),
            ...
            (
                "Phase 3",
                "Scale & Expand",
                f"Weeks {cw // 3 + 1}-{2 * cw // 3}",
                "Increase spend on top performers, add new channels, expand geo-targeting",
                "Top 5 channels",
                "Cost per hire, Quality of hire",
                "40%",
            ),
            (
                ...
                "Sustain & Refine",
                f"Weeks {2 * cw // 3 + 1}-{cw}",
                "Maintain performance, quarterly reviews, seasonal adjustments, ROI reporting",
                "Proven channels",
                "ROI, Time to fill",
                "30%",
            ),
        ...
        phases = [
            ...
            (
                ...
                "Launch & Optimize",
                f"Weeks 5-{cw // 4}",
                "Initial campaign launch, A/B testing, bid optimization, talent pipeline building",
                "All programmatic",
                "CPC, CPA, Apply rate",
                "20%",
            ),
            ...
            (
                ...
                "Sustain & Refine",
                f"Weeks {cw // 2 + 1}-{cw}",
                "Maintain performance, quarterly reviews, seasonal adjustments, advanced ROI reporting",
                "Proven channels",
                "ROI, Time to fill, Retention",
                "35%",
            ),
        ]
    ...
    for (
        ...
        ws_timeline.cell(row=tl_row, column=2).font = Font(
            name="Calibri", bold=True, size=10
        )
    ...
    if data.get("include_educational"):
        ...
        for p in partners:
            row += 1
            style_body_cell(ws_edu, row, 2, p.get("institution") or "")
            ws_edu.cell(row=row, column=2).font = Font(
                name="Calibri", bold=True, size=10
            )
            style_body_cell(ws_edu, row, 3, p.get("fit") or "")
        ...
        if uni_pubs:
            ...
            ws_edu.cell(
                row=row,
                column=2,
                value="Joveo University Job Board Partners (for campus recruitment)",
            ).font = Font(name="Calibri", bold=True, size=12, color="2E75B6")
            row += 1
            ...
            for pub in uni_pubs[:20]:
                ...
                style_body_cell(
                    ws_edu, row, 3, "University Job Board — Campus Recruiting Pipeline"
                )
                row += 1
    ...
    if data.get("include_events"):
        ...
        for evt in events:
            row += 1
            style_body_cell(ws_events, row, 2, evt.get("partner") or "")
            ws_events.cell(row=row, column=2).font = Font(
                name="Calibri", bold=True, size=10
            )
            style_body_cell(ws_events, row, 3, evt.get("location") or "")
            style_body_cell(ws_events, row, 4, evt.get("type") or "")
            style_body_cell(ws_events, row, 5, evt.get("impact") or "")
            style_body_cell(ws_events, row, 6, evt.get("reach") or "")
            style_body_cell(ws_events, row, 7, evt.get("budget") or "")
    ...
    if data.get("include_radio_podcasts"):
        ...
        if local_stations:
            ...
            for station in local_stations:
                row += 1
                style_body_cell(ws_radio, row, 2, station.get("name") or "")
                ws_radio.cell(row=row, column=2).font = Font(
                    name="Calibri", bold=True, size=10
                )
                style_body_cell(ws_radio, row, 3, station.get("listeners") or "")
                style_body_cell(ws_radio, row, 4, station.get("genre") or "")
                style_body_cell(ws_radio, row, 5, station.get("audience") or "")
        ...
        if podcasts:
            ...
            for pod in podcasts:
                row += 1
                style_body_cell(ws_radio, row, 2, pod.get("name") or "")
                ws_radio.cell(row=row, column=2).font = Font(
                    name="Calibri", bold=True, size=10
                )
                style_body_cell(ws_radio, row, 3, pod.get("listeners") or "")
                style_body_cell(ws_radio, row, 4, pod.get("genre") or "")
                style_body_cell(ws_radio, row, 5, pod.get("audience") or "")
    ...
    if data.get("include_media_platforms"):
        ...
        media_platforms_db = {
            "healthcare_medical": {
                ...
                "trade_publications": [
                    (
                        "JAMA Network",
                        "Physicians, Surgeons, Researchers",
                        "US + Global",
                        "Employer profiles, classified ads, sponsored features",
                    ),
                    (
                        "New England Journal of Medicine (NEJM)",
                        "Physicians, Hospital Leaders",
                        "Global",
                        "Career center listings, display ads",
                    ),
                    (
                        "American Journal of Nursing (AJN)",
                        "RNs, NPs, Nurse Leaders",
                        "US",
                        "Print + digital job ads, career guides",
                    ),
                    (
                        "Modern Healthcare",
                        "Hospital Administrators, C-Suite",
                        "US",
                        "Sponsored content, job board integration",
                    ),
                    (
                        "Becker's Hospital Review",
                        "Health System Executives",
                        "US",
                        "Job listings, executive recruitment features",
                    ),
                    (
                        "Nursing Times",
                        "Nurses, Midwives, HCAs",
                        "UK + Europe",
                        "Recruitment supplements, career events",
                    ),
                ],
                "digital_media": [
                    (
                        "Medscape",
                        "1M+ Physicians, NPs, PAs (verified)",
                        "Global",
                        "Targeted recruitment ads by specialty & geography",
                    ),
                    (
                        "Doximity",
                        "80%+ of US Physicians",
                        "US",
                        "InMail recruiting, employer profiles, telehealth talent",
                    ),
                    (
                        "Nurse.com",
                        "RNs, LPNs, Travel Nurses",
                        "US",
                        "Job alerts, continuing education + recruitment combos",
                    ),
                    (
                        "Health eCareers",
                        "Allied Health, Clinical, Admin",
                        "US",
                        "Niche job board, resume database access",
                    ),
                    (
                        "PracticeLink",
                        "Physicians in active job search",
                        "US",
                        "Physician-only recruitment platform",
                    ),
                ],
                "ooh_print": [
                    (
                        "Hospital Cafeteria / Break Room Flyers",
                        "Current staff referrals",
                        "Local",
                        "Low-cost, high-trust referral channel",
                    ),
                    (
                        "Medical Conference Programs (AAMC, AACN)",
                        "Conference attendees by specialty",
                        "US + Global",
                        "Event program ads, booth placements",
                    ),
                    (
                        "Nursing School Campus Boards",
                        "New grad nurses, students",
                        "Local",
                        "Career fair flyers, campus partnerships",
                    ),
                    (
                        "Transit Ads in Hospital Districts",
                        "Local healthcare workers",
                        "Regional",
                        "Bus wraps, metro ads near medical centers",
                    ),
                ],
                "broadcast_audio": [
                    (
                        "Healthcare NOW Radio",
                        "Healthcare professionals, HIT",
                        "US",
                        "Sponsored segments, recruitment spots",
                    ),
                    (
                        "The Nurses You Know Podcast",
                        "Nursing professionals",
                        "US",
                        "Sponsorship, employer spotlight episodes",
                    ),
                    (
                        "Spotify/Pandora Geo-targeted",
                        "Commuting healthcare workers",
                        "Regional",
                        "Audio ads targeting healthcare interest segments",
                    ),
                ],
            },
            "technology_it": {
                ...
                "trade_publications": [
                    (
                        ...
                        "Global",
                        "Career ads, employer sponsored features",
                    ),
                    (
                        "Communications of the ACM",
                        "Software Engineers, CS Researchers",
                        "Global",
                        "Career center, recruitment display ads",
                    ),
                    (
                        "Wired Magazine",
                        "Tech professionals, early adopters",
                        "US + Global",
                        "Employer brand features, career content",
                    ),
                    (
                        "MIT Technology Review",
                        "Tech leaders, AI/ML researchers",
                        "Global",
                        "Sponsored recruitment content, job listings",
                    ),
                    (
                        "InfoWorld / Computerworld",
                        "IT Managers, Developers, DevOps",
                        "US",
                        "Tech career guides, recruitment ads",
                    ),
                ],
                "digital_media": [
                    (
                        "Stack Overflow Talent",
                        "10M+ Monthly developers",
                        "Global",
                        "Targeted developer recruitment, company profiles, skills-based matching",
                    ),
                    (
                        "GitHub (Employer Branding)",
                        "100M+ Developer accounts",
                        "Global",
                        "Repository sponsorships, team pages, open-source community engagement",
                    ),
                    (
                        "Hacker News (Who's Hiring)",
                        "Senior engineers, startup talent",
                        "Global",
                        "Monthly hiring threads, YC company pages",
                    ),
                    (
                        "Dev.to",
                        "2M+ Developer community",
                        "Global",
                        "Employer listings, sponsored posts, community engagement",
                    ),
                    (
                        "TechCrunch / The Verge Careers",
                        "Tech industry professionals",
                        "Global",
                        "Sponsored job placements, brand articles",
                    ),
                    (
                        "Dice.com",
                        "IT professionals, contractors",
                        "US",
                        "Tech-specific job board, skills matching",
                    ),
                ],
                "ooh_print": [
                    (
                        "Tech Conference Programs (CES, AWS re:Invent, Google I/O)",
                        "Developers, CTOs, Architects",
                        "Global",
                        "Event sponsorships, booth recruitment",
                    ),
                    (
                        "Tech Hub Billboard / Transit (SF, NYC, Austin, Seattle)",
                        "Local tech workers",
                        "Regional",
                        "High-visibility employer brand ads",
                    ),
                    (
                        "Hackathon Sponsorships",
                        "Junior to mid developers",
                        "Regional",
                        "Direct talent pipeline + brand building",
                    ),
                    (
                        "University CS Department Boards",
                        "CS/Engineering students",
                        "Local",
                        "Campus recruiting, internship postings",
                    ),
                ],
                "broadcast_audio": [
                    (
                        ...
                        "Global",
                        "Recruitment sponsorships, employer spotlights",
                    ),
                    (
                        "Changelog Podcast Network",
                        "Open source developers",
                        "Global",
                        "Targeted dev recruitment, sponsor reads",
                    ),
                    (
                        "Spotify Tech Playlist Targeting",
                        "Tech workers by interest",
                        "US + Global",
                        "Audio ads targeting tech/coding playlists",
                    ),
                ],
            },
            "tech_engineering": {
                ...
                "digital_media": [
                    ...
                    (
                        ...
                        "100M+ Developer accounts",
                        "Global",
                        "Repository sponsorships, team pages, open-source engagement",
                    ),
                    (
                        "Hacker News (Who's Hiring)",
                        "Senior engineers, startup talent",
                ...
                "ooh_print": [
                    (
                        "Tech Conference Programs (CES, AWS re:Invent)",
                        "Developers, CTOs, Architects",
                        "Global",
                        "Event sponsorships, booth recruitment",
                    ),
                ...
                "broadcast_audio": [
                    ...
                    (
                        ...
                        "Tech workers by interest",
                        "Global",
                        "Audio ads targeting tech playlists",
                    ),
                ],
            },
            "blue_collar_trades": {
                ...
                "trade_publications": [
                    (
                        ...
                        "US",
                        "Classified job ads, workforce features",
                    ),
                    (
                        "Electrical Contractor (EC&M)",
                        "Electricians, Electrical contractors",
                        "US",
                        "Career section, recruitment ads",
                    ),
                    (
                        "Welding Journal (AWS)",
                        "Welders, fabricators, inspectors",
                        "US + Global",
                        "Certified welder recruitment ads",
                    ),
                    (
                        "Fine Homebuilding / JLC",
                        "Carpenters, builders, remodelers",
                        "US",
                        "Help wanted ads, workforce content",
                    ),
                    (
                        "Plumbing & Mechanical Magazine",
                        "Plumbers, pipefitters",
                        "US",
                        "Trade recruitment, apprenticeship ads",
                    ),
                ],
                "digital_media": [
                    (
                        ...
                        "All skilled trades",
                        "US",
                        "Skilled labor marketplace, staffing",
                    ),
                    (
                        "iHireConstruction / iHireMaintenance",
                        "Construction & maintenance workers",
                        "US",
                        "Niche job boards for trades",
                    ),
                    (
                        "SkillsUSA / Trade School Job Boards",
                        "Vocational students, apprentices",
                        "US",
                        "Early career pipeline, school partnerships",
                    ),
                    (
                        "Facebook Local Groups (Trade Work)",
                        "Local tradespeople",
                        "Regional",
                        "Community-based recruiting, local job posts",
                    ),
                    (
                        "NextDoor Neighborhood Ads",
                        "Local residents seeking work",
                        "Regional",
                        "Hyperlocal recruitment for trades",
                    ),
                ],
                "ooh_print": [
                    (
                        "Home Depot / Lowe's Community Boards",
                        "DIY-ers, handymen, trade workers",
                        "Local",
                        "Flyer postings, partnership programs",
                    ),
                    (
                        "Union Halls & Trade School Bulletin Boards",
                        "Apprentices, journeymen",
                        "Local",
                        "Direct trade community access",
                    ),
                    (
                        "Truck Stop / Gas Station Postings",
                        "CDL drivers, field workers",
                        "Regional",
                        "Highway corridor reach",
                    ),
                    (
                        "Local Newspaper Classifieds",
                        "Blue collar job seekers",
                        "Local",
                        "Traditional but trusted channel",
                    ),
                    (
                        "Community Recreation Centers",
                        "Local job seekers",
                        "Local",
                        "Flyers, job fair postings",
                    ),
                ],
                "broadcast_audio": [
                    (
                        "Local AM/FM Radio (Drive Time)",
                        "Commuting tradespeople",
                        "Regional",
                        "30-60 sec recruitment spots during commute hours",
                    ),
                    (
                        "Country / Classic Rock Station Sponsorships",
                        "Blue collar demographics",
                        "Regional",
                        "Recruitment spots on trade-friendly formats",
                    ),
                    (
                        "iHeartRadio Local Targeting",
                        "Regional workers by zip code",
                        "Regional",
                        "Digital audio ads targeted by location",
                    ),
                ],
            },
            "logistics_supply_chain": {
                ...
                "trade_publications": [
                    (
                        ...
                        "SCM directors, VP Operations",
                        "US + Global",
                        "Executive recruitment, employer profiles",
                    ),
                    (
                        "Logistics Management",
                        "Logistics managers, warehouse directors",
                        "US",
                        "Career section, recruitment display ads",
                    ),
                    (
                        "Transport Topics",
                        "Trucking, fleet operators, CDL drivers",
                        "US",
                        "Driver recruitment ads, fleet hiring features",
                    ),
                    (
                        "Journal of Commerce (JOC)",
                        "Shipping, freight, customs professionals",
                        "Global",
                        "Trade recruitment, sponsored content",
                    ),
                    (
                        "Inbound Logistics",
                        "Supply chain, procurement professionals",
                        "US",
                        "Recruitment ads, workforce articles",
                    ),
                ],
                "digital_media": [
                    (
                        ...
                        "CDL truck drivers",
                        "US",
                        "Driver-specific job board, 500K+ monthly visitors",
                    ),
                    (
                        "FreightWaves",
                        "Freight, trucking, logistics professionals",
                        "US",
                        "News site, career section, employer brand",
                    ),
                    (
                        "SupplyChainBrain",
                        "SC professionals all levels",
                        "US + Global",
                        "Job board, webinar sponsorships",
                    ),
                    (
                        "Indeed CDL Driver Campaigns",
                        "CDL-A/B drivers, delivery drivers",
                        "US",
                        "Programmatic driver recruitment",
                    ),
                    (
                        "Facebook Driver Groups",
                        "OTR, local, regional drivers",
                        "US",
                        "Community-based driver recruiting",
                    ),
                ],
                "ooh_print": [
                    (
                        "Truck Stop Advertising (Pilot/Flying J, Love's)",
                        "Long-haul truck drivers",
                        "US Nationwide",
                        "Poster ads, digital screens at fuel islands",
                    ),
                    (
                        "Warehouse District Billboards",
                        "Warehouse workers, forklift operators",
                        "Regional",
                        "High-traffic logistics corridor visibility",
                    ),
                    (
                        "Distribution Center Break Rooms",
                        "Current warehouse employees (referrals)",
                        "Local",
                        "Internal referral flyers, QR code postings",
                    ),
                    (
                        "CDL School Campus Boards",
                        "New CDL holders, students",
                        "Regional",
                        "Direct pipeline from driving schools",
                    ),
                ],
                "broadcast_audio": [
                    (
                        ...
                        "OTR truck drivers",
                        "US Nationwide",
                        "Recruitment spots during long hauls",
                    ),
                    (
                        "TruckersMP / Trucker Path App Ads",
                        "Active truck drivers",
                        "US",
                        "In-app recruitment ads",
                    ),
                    (
                        "Spotify/Pandora Geo-targeted (Warehouse zones)",
                        "Warehouse workers by geography",
                        "Regional",
                        "Audio ads near fulfillment centers",
                    ),
                ],
            },
            "retail_consumer": {
                ...
                "trade_publications": [
                    (
                        "Retail Dive",
                        "Retail executives, buyers, merchandisers",
                        "US",
                        "Recruitment display ads, employer profiles",
                    ),
                    (
                        "NRF SmartBrief / Stores Magazine",
                        "NRF members, retail leaders",
                        "US",
                        "Career content, sponsored recruitment",
                    ),
                    (
                        "Chain Store Age",
                        "Chain retail management",
                        "US",
                        "Recruitment ads, workforce features",
                    ),
                    (
                        "Progressive Grocer",
                        "Grocery retail managers, buyers",
                        "US",
                        "Supermarket industry recruitment",
                    ),
                ],
                "digital_media": [
                    (
                        "Snagajob / Snag",
                        "Hourly retail & service workers",
                        "US",
                        "Hourly job platform, 100M+ job seekers",
                    ),
                    (
                        "RetailCareers.com",
                        "Retail management, corporate retail",
                        "US + Global",
                        "Retail-specific job board",
                    ),
                    (
                        "Facebook Local Job Posts",
                        "Local hourly job seekers",
                        "Regional",
                        "Free + paid job postings, high engagement",
                    ),
                    (
                        "TikTok Recruitment (Retail brands)",
                        "Gen Z retail workers",
                        "US + Global",
                        "Short-form employer brand content",
                    ),
                    (
                        "Google Local Jobs",
                        "In-store, hourly, seasonal workers",
                        "Regional",
                        "High-intent local job search",
                    ),
                ],
                "ooh_print": [
                    (
                        "In-Store 'Now Hiring' Signage",
                        "Walk-in candidates, shoppers",
                        "Local",
                        "Highest-converting retail recruitment channel",
                    ),
                    (
                        "Shopping Mall Directory Boards",
                        "Foot traffic job seekers",
                        "Local",
                        "Multi-store hiring event ads",
                    ),
                    (
                        "Local Community Bulletin Boards",
                        "Neighborhood residents",
                        "Local",
                        "Free/low-cost community reach",
                    ),
                    (
                        "Bus Stop / Transit Shelter Ads",
                        "Commuters, entry-level workers",
                        "Regional",
                        "High-visibility in retail corridors",
                    ),
                    (
                        "College Campus Boards (Part-time)",
                        "Students seeking part-time work",
                        "Local",
                        "Campus partnerships for seasonal hires",
                    ),
                ],
                "broadcast_audio": [
                    (
                        "Local Radio (Pop, Urban, Top 40)",
                        "Younger workforce 18-35",
                        "Regional",
                        "Drive-time recruitment spots",
                    ),
                    (
                        "Spotify Free Tier Ads",
                        "Younger demographics",
                        "Regional",
                        "Audio/display ads between songs",
                    ),
                    (
                        "Retail podcast sponsorships",
            ...
            "hospitality_travel": {
                ...
                "trade_publications": [
                    (
                        "Hotel Management",
                        "Hotel GMs, front office, F&B directors",
                        "US",
                        "Career section, recruitment ads",
                    ),
                    (
                        "Restaurant Business / Nation's Restaurant News",
                        "Restaurant managers, chefs, FOH",
                        "US",
                        "Recruitment classifieds, workforce features",
                    ),
                    (
                        "Travel Weekly",
                        "Travel agents, tour operators",
                        "US + Global",
                        "Career ads, industry recruitment",
                    ),
                    (
                        "Lodging Magazine",
                        "Hospitality management, revenue mgrs",
                        "US",
                        "Recruitment display ads",
                    ),
                ],
                "digital_media": [
                    (
                        "Hcareers",
                        "Hotel, restaurant, casino, resort jobs",
                        "US + Global",
                        "Hospitality-specific job board",
                    ),
                    (
                        "Poached Jobs",
                        "Restaurant, bar, hotel staff",
                        "US",
                        "Culinary & hospitality talent marketplace",
                    ),
                    (
                        "Culinary Agents",
                        "Chefs, cooks, sommeliers, F&B",
                        "US",
                        "Food service recruitment platform",
                    ),
                    (
                        "Hosco",
                    ...
                    (
                        ...
                        "Younger hospitality workers",
                        "Global",
                        "Employer brand, day-in-the-life content",
                    ),
                ],
                "ooh_print": [
                    (
                        "Culinary School Campus Boards",
                        "Culinary students, externs",
                        "Local",
                        "Chef and kitchen pipeline",
                    ),
                    (
                        "Hotel Lobby / Employee Entrance Postings",
                        "Walk-in applicants, current referrals",
                        "Local",
                        "High-conversion referral channel",
                    ),
                    (
                        "Tourism District Billboards / Transit",
                        "Service workers in tourism areas",
                        "Regional",
                        "Seasonal hiring visibility",
                    ),
                    (
                        "Restaurant Supply Store Boards",
                        "Kitchen staff, restaurant workers",
                        "Local",
                        "Industry-adjacent community reach",
                    ),
                ],
                "broadcast_audio": [
                    (
                        "Food & restaurant podcasts (Bon Appétit, Eater)",
                        "Culinary professionals, foodies",
                        "US + Global",
                        "Sponsorship, employer spotlight",
                    ),
                    (
                        "Local radio — morning drive",
                        "Service industry shift workers",
                        "Regional",
                        "Recruitment spots before shift start",
                    ),
                ],
            },
            "finance_banking": {
                ...
                "trade_publications": [
                    (
                        "Financial Times",
                        "Finance professionals, banking executives",
                        "Global",
                        "Recruitment display ads, career section",
                    ),
                    (
                        "Wall Street Journal",
                        "Financial analysts, fund managers",
                        "Global",
                        "Career section, executive recruitment",
                    ),
                    (
                        "American Banker",
                        "Banking professionals, compliance, risk",
                        "US",
                        "Industry recruitment, employer profiles",
                    ),
                    (
                        "The Economist",
                        "Economists, policy analysts, finance leaders",
                        "Global",
                        "Career ads, sponsored employer content",
                    ),
                    (
                        "Barron's",
                        "Investment professionals, wealth managers",
                        "US",
                        "Career section, premium recruitment ads",
                    ),
                ],
                "digital_media": [
                    (
                        ...
                        "Global",
                        "Finance-specific job board, 2M+ monthly visitors",
                    ),
                    (
                        "Wall Street Oasis",
                        "IB analysts, PE/VC professionals",
                        "Global",
                        "Community + job board, employer AMAs",
                    ),
                    (
                        "Bloomberg Careers",
                        "Financial data, analytics professionals",
                        "Global",
                        "Premium finance recruitment",
                    ),
                    (
                        "Investopedia Career Center",
                        "Financial advisors, analysts",
                        "US",
                        "Career content, job listings",
                    ),
                    (
                        "The Muse / Vault Finance",
                        "Early-career finance professionals",
                        "US",
                        "Employer profiles, career guides",
                    ),
                ],
                "ooh_print": [
                    (
                        "Financial District Transit Ads (NYC, London, HK)",
                        "Finance commuters",
                        "Regional",
                        "High-visibility in financial hubs",
                    ),
                    (
                        "CFA/CPA Exam Prep Center Boards",
                        "Finance candidates pursuing credentials",
                        "US + Global",
                        "Targeted credential-track talent",
                    ),
                    (
                        "University Business School Boards",
                        "MBA, finance students",
                        "Regional",
                        "On-campus recruitment, career fair ads",
                    ),
                ],
                "broadcast_audio": [
                    (
                        ...
                        "Global",
                        "Recruitment sponsorships, career segments",
                    ),
                    (
                        "Financial podcasts (Odd Lots, Money Stuff)",
                        "Finance enthusiasts, professionals",
                        "Global",
                        "Sponsor reads, employer features",
                    ),
                    (
                        "CNBC / Fox Business Digital",
                        "Finance decision-makers",
                        "US",
                        "Recruitment display + video ads",
                    ),
                ],
            },
            "aerospace_defense": {
                ...
                "trade_publications": [
                    (
                        ...
                        "Aerospace engineers, program managers",
                        "Global",
                        "Career center, recruitment display ads",
                    ),
                    (
                        "Defense News",
                        "Defense contractors, military-transition",
                        "US + Global",
                        "Recruitment features, career section",
                    ),
                    (
                        "Jane's Defence Weekly",
                        "Defense & security professionals",
                        "Global",
                        "Premium recruitment listings",
                    ),
                    (
                        "SpaceNews",
                        "Space industry engineers, scientists",
                        "Global",
                        "Space sector recruitment ads",
                    ),
                ],
                "digital_media": [
                    (
                        ...
                        "US",
                        "Cleared talent job board, virtual career fairs",
                    ),
                    (
                        "Military.com",
                        "Veterans, military-transition professionals",
                        "US",
                        "Largest military community, job board",
                    ),
                    (
                        "Defense One",
                        "Defense policy, technology professionals",
                        "US",
                        "Sponsored recruitment content",
                    ),
                    (
                        "AIAA Career Center",
                    ...
                    (
                        "AviationJobSearch",
                        "Pilots, aviation maintenance, aerospace",
                        "Global",
                        "Aviation-specific recruitment platform",
                    ),
                ],
                "ooh_print": [
                    (
                        "Defense Industry Conference Programs (AUSA, AAAA)",
                        "Defense professionals, military leaders",
                        "US + Global",
                        "Conference recruitment booths, program ads",
                    ),
                    (
                        "Military Base Transition Centers",
                        "Separating service members",
                        "US",
                        "High-quality veteran pipeline",
                    ),
                    (
                        "Aerospace Museum / Air Show Programs",
                        "Aviation enthusiasts, STEM talent",
                        "Regional",
                        "Brand visibility + passive recruitment",
                    ),
                ],
                "broadcast_audio": [
                    (
                        "The Aerospace Executive Podcast",
                        "Aerospace leaders, engineers",
                        "US",
                        "Recruitment sponsorships",
                    ),
                    (
            ...
            "pharma_biotech": {
                ...
                "trade_publications": [
                    (
                        "Nature",
                        "Scientists, researchers, PhD professionals",
                        "Global",
                        "Career section, recruitment ads, employer profiles",
                    ),
                    (
                        "The Lancet",
                        "Clinical researchers, physicians",
                        "Global",
                        "Medical recruitment, career features",
                    ),
                    (
                        "Science Magazine (AAAS)",
                        "STEM researchers, postdocs",
                        "Global",
                        "Career center, employer ads",
                    ),
                    (
                        "STAT News",
                        "Biopharma, health, science reporters & pros",
                        "US",
                        "Sponsored employer content, job listings",
                    ),
                    (
                        "Genetic Engineering & Biotechnology News (GEN)",
                        "Biotech lab professionals",
                        "US + Global",
                        "Niche biotech recruitment ads",
                    ),
                ],
                "digital_media": [
                    (
                        "BioSpace",
                        "Life science professionals all levels",
                        "US",
                        "Biotech/pharma job board, 1M+ monthly visitors",
                    ),
                    (
                        "PharmiWeb",
                        "Pharmaceutical, CRO, medical device",
                        "Global",
                        "Pharma-specific recruitment platform",
                    ),
                    (
                        "Endpoints News",
                        "Biopharma executives, clinical teams",
                        "US",
                        "Sponsored content, employer brand features",
                    ),
                    (
                        "ResearchGate Jobs",
                        "Academic researchers, PhD scientists",
                        "Global",
                        "Research community recruitment",
                    ),
                    (
                        "MedReps",
                        "Medical sales, device reps, pharma sales",
                        "US",
                        "Sales-focused life science job board",
                    ),
                ],
                "ooh_print": [
                    (
                        "Biotech Hub Transit Ads (Boston, SF, San Diego, RTP)",
                        "Life science commuters",
                        "Regional",
                        "High-visibility in biotech clusters",
                    ),
                    (
                        "Scientific Conference Programs (ASCO, ASH, JPM)",
                        "Researchers, clinical professionals",
                        "Global",
                        "Conference career fair ads, booth recruitment",
                    ),
                    (
                        "University Lab / Science Building Boards",
                        "Postdocs, PhD students, researchers",
                        "Local",
                        "Academic pipeline for industry positions",
                    ),
                ],
                "broadcast_audio": [
                    (
                        "This Week in Virology / Science podcasts",
                        "Scientists, researchers",
                        "Global",
                        "Recruitment sponsorships in science media",
                    ),
                    (
                        "STAT News Podcast",
                        "Biopharma professionals",
                        "US",
                        "Industry recruitment sponsor reads",
                    ),
                ],
            },
            "energy_utilities": {
                ...
                "trade_publications": [
                    (
                        "Power Engineering",
                        "Power plant operators, engineers",
                        "US",
                        "Career section, recruitment ads",
                    ),
                    (
                        "Oil & Gas Journal",
                        "Petroleum engineers, field operators",
                        "Global",
                        "O&G industry recruitment",
                    ),
                    (
                        "Renewable Energy World",
                        "Solar, wind, energy storage pros",
                        "Global",
                        "Clean energy recruitment ads",
                    ),
                    (
                        "Utility Dive",
                        "Utility executives, grid engineers",
                        "US",
                        "Career content, sponsored recruitment",
                    ),
                    (
                        "T&D World",
                        "Transmission & distribution professionals",
                        "US",
                        "Utility workforce recruitment",
                    ),
                ],
                "digital_media": [
                    (
                        "Rigzone",
                        "Oil & gas professionals globally",
                        "Global",
                        "O&G job board, 1.5M+ monthly visitors",
                    ),
                    (
                        "EnergyJobline",
                        "All energy sector professionals",
                        "Global",
                        "Cross-energy recruitment platform",
                    ),
                    (
                        "CleanTechnica Jobs",
                    ...
                    (
                        "Utility Jobs Online",
                        "Electric, gas, water utility workers",
                        "US",
                        "Utility-specific recruitment",
                    ),
                ],
                "ooh_print": [
                    (
                        "Energy Conference Programs (CERAWeek, Solar Power Intl)",
                        "Energy executives, engineers",
                        "Global",
                        "Conference recruitment, booth presence",
                    ),
                    (
                        "Oil Field / Pipeline Corridor Billboards",
                        "Field workers, operators",
                        "Regional",
                        "Reach workers in energy production areas",
                    ),
                    (
                        "Trade Union Halls (IBEW, UWUA)",
                        "Lineworkers, electricians, utility workers",
                        "US",
                        "Union-pathway recruitment",
                    ),
                ],
                "broadcast_audio": [
                    (
                        "Energy podcasts (The Energy Gang, Columbia Energy Exchange)",
                        "Energy professionals, policy makers",
                        "US + Global",
                        "Recruitment sponsorships",
                    ),
                    (
                        "Local radio in energy production regions (Permian, Bakken)",
                        "Oilfield workers, plant operators",
                        "Regional",
                        "Geo-targeted recruitment spots",
                    ),
                ],
            },
            "education": {
                ...
                "trade_publications": [
                    (
                        ...
                        "US",
                        "Premier higher ed recruitment publication",
                    ),
                    (
                        "Education Week",
                        "K-12 teachers, administrators, policy",
                        "US",
                        "K-12 recruitment ads, career section",
                    ),
                    (
                        "Inside Higher Ed",
                        "College faculty, staff, administrators",
                        "US",
                        "Job board, employer profiles",
                    ),
                    (
                        "Times Higher Education (THE)",
                        "International university faculty",
                        "Global",
                        "Global academic recruitment",
                    ),
                ],
                "digital_media": [
                    (
                        ...
                        "US",
                        "Largest higher ed job board",
                    ),
                    (
                        "SchoolSpring / Frontline Education",
                        "K-12 teachers, staff",
                        "US",
                        "K-12 school recruitment platform",
                    ),
                    (
                        "Teach Away",
                    ...
                    (
                        "Indeed Education Campaigns",
                        "All education roles",
                        "US",
                        "Broad education recruitment",
                    ),
                ],
                "ooh_print": [
                    (
                        "Education Conference Programs (ISTE, ASCD, AERA)",
                        "Educators, administrators",
                        "US + Global",
                        "Conference career fair recruitment",
                    ),
                    (
                        "College of Education Bulletin Boards",
                        "Student teachers, new graduates",
                        "Local",
                        "Pipeline for new teacher hires",
                    ),
                    (
                        "School District Office Boards",
                        "Current staff referrals, substitutes",
                        "Local",
                        "Community recruitment reach",
                    ),
                ],
                "broadcast_audio": [
                    (
                        "Education podcasts (Cult of Pedagogy, EdSurge)",
                        "Teachers, instructional designers",
                        "US",
                        "Recruitment sponsorships",
                    ),
                    (
                        "NPR / Public Radio Underwriting",
                        "Educated demographics, teachers",
                        "Regional",
                        "Premium audience, employer brand",
                    ),
                ],
            },
            "construction_real_estate": {
                ...
                "trade_publications": [
                    (
                        ...
                        "Construction executives, PMs, engineers",
                        "US + Global",
                        "Industry recruitment leader, career center",
                    ),
                    (
                        "Construction Dive",
                    ...
                    (
                        "Real Estate Forum / GlobeSt",
                        "CRE professionals, brokers, developers",
                        "US",
                        "Commercial RE recruitment",
                    ),
                ],
                "digital_media": [
                    (
                        "ConstructionJobs.com",
                        "All construction trades & management",
                        "US",
                        "Construction-specific job board",
                    ),
                    (
                        "iHireConstruction",
                        "Laborers, operators, supervisors",
                        "US",
                        "Niche construction recruitment",
                    ),
                    (
                        "Procore Community Jobs",
                        "Construction tech, project management",
                        "US",
                        "Construction software community",
                    ),
                    (
                        "Zillow / Realtor.com Careers",
                ...
                "ooh_print": [
                    (
                        "Construction Supply Store Boards (Home Depot Pro)",
                        "Contractors, construction workers",
                        "Local",
                        "Industry-adjacent community reach",
                    ),
                    (
                        "Job Site Fence Banners / QR Codes",
                        "Passersby, local trade workers",
                        "Local",
                        "High-visibility at active construction sites",
                    ),
                    (
                        "Union Halls (Carpenters, Laborers, Operating Engineers)",
                        "Union tradespeople",
                        "Regional",
                        "Union apprenticeship pipeline",
                    ),
                ],
                "broadcast_audio": [
                    (
                        "Construction-focused radio (AM talk, sports)",
                        "Construction worker commuters",
                        "Regional",
                        "Drive-time recruitment spots",
                    ),
                    (
                        "Construction podcasts (ConTech Crew)",
            ...
            "food_beverage": {
                ...
                "trade_publications": [
                    (
                        ...
                        "Food manufacturing, QA, plant managers",
                        "US",
                        "Industry recruitment, career section",
                    ),
                    (
                        "Beverage Industry",
                        "Beverage production, distribution",
                        "US",
                        "Beverage sector recruitment",
                    ),
                    (
                        "Food Engineering",
                        "Food plant engineers, operations",
                        "US",
                        "Engineering recruitment in food mfg",
                    ),
                    (
                        "QSR Magazine",
                        "Quick service restaurant management",
                        "US",
                        "Restaurant industry recruitment",
                    ),
                ],
                "digital_media": [
                    (
                        "Poached Jobs",
                        "Restaurant, bar, culinary talent",
                        "US",
                        "Hospitality talent marketplace",
                    ),
                    (
                        "FoodGrads",
                        "Food science, food safety graduates",
                        "US + Canada",
                        "Entry-level food industry recruitment",
                    ),
                    (
                        "CareerBuilder Food & Bev",
                        "Food manufacturing, plant workers",
                        "US",
                        "Broad food industry recruitment",
                    ),
                    (
                        "Facebook Local Restaurant Groups",
                        "Line cooks, servers, kitchen staff",
                        "Regional",
                ...
                "ooh_print": [
                    (
                        "Food Industry Trade Show Programs (IFT, NRA Show)",
                        "Food scientists, chefs, F&B execs",
                        "US + Global",
                        "Conference recruitment and brand visibility",
                    ),
                    (
                        "Restaurant Supply Store Boards",
                        "Kitchen staff, restaurant workers",
                        "Local",
                        "Industry community reach",
                    ),
                    (
                        "Food Plant Employee Entrance Postings",
                        "Current employee referrals",
                        "Local",
                        "Referral-based internal recruitment",
                    ),
                ],
                "broadcast_audio": [
                    (
                        "Food podcasts (Bon Appétit, Gastropod)",
                        "Culinary professionals, food scientists",
                        "US + Global",
                        "Recruitment sponsorships",
                    ),
                    (
                        "Local radio near food production facilities",
                        "Plant workers, line operators",
                        "Regional",
                        "Geo-targeted recruitment spots",
                    ),
                ],
            ...
            "telecommunications": {
                ...
                "trade_publications": [
                    (
                        ...
                        "Global",
                        "Carrier & vendor recruitment ads",
                    ),
                    (
                        "FierceTelecom / FierceWireless",
                        "Telecom executives, 5G professionals",
                        "US + Global",
                        "Recruitment features, career section",
                    ),
                    (
                ...
                "digital_media": [
                    (
                        ...
                        "All telecom roles",
                        "US",
                        "Telecom-specific job board",
                    ),
                    (
                        "Dice.com (Telecom/Network)",
                        "Network engineers, telecom IT",
                        "US",
                        "Tech job board with telecom filter",
                    ),
                    (
                        "LinkedIn Telecom Groups",
                        "Telecom professionals by specialty",
                        "Global",
                        "Group recruiting, sponsored posts",
                    ),
                ],
                "ooh_print": [
                    (
                        "MWC / CES Conference Programs",
                        "Telecom executives, engineers",
                        "Global",
                        "Event recruitment, booth hiring",
                    ),
                    (
                        "Telecom facility area billboards",
                        "Tower technicians, field engineers",
                        "Regional",
                        "Local recruitment near infrastructure",
                    ),
                ],
                "broadcast_audio": [
                    (
                        "Telecom podcasts (Light Reading, Fierce)",
            ...
            "media_entertainment": {
                ...
                "trade_publications": [
                    (
                        ...
                        "Global",
                        "Recruitment ads, career classifieds",
                    ),
                    (
                        "The Hollywood Reporter",
                        "Film, TV, media executives",
                        "Global",
                        "Entertainment career section",
                    ),
                    (
                        "Broadcasting & Cable",
                        "Broadcast, streaming professionals",
                        "US",
                        "Media industry recruitment",
                    ),
                    (
                        "AdAge / AdWeek",
                        "Marketing, advertising, creative pros",
                        "US + Global",
                        "Agency & brand recruitment",
                    ),
                ],
                "digital_media": [
                    (
                        "Mandy.com",
                        "Actors, crew, production staff",
                        "Global",
                        "Entertainment production job board",
                    ),
                    (
                        "ProductionHub",
                        "Film & video production professionals",
                        "US",
                        "Production crew recruitment",
                    ),
                    (
                        "Mediabistro",
                        "Media, journalism, content professionals",
                        "US",
                        "Media industry job board",
                    ),
                    (
                        "CreativePool / Behance Jobs",
                        "Designers, animators, creative talent",
                        "Global",
                ...
                "ooh_print": [
                    (
                        "Film/Media Festival Programs (Sundance, SXSW, NAB Show)",
                        "Creative & production professionals",
                        "Global",
                        "Event recruitment, networking",
                    ),
                    (
                        "Studio Lot / Production Office Boards",
                        "Production crew, assistants",
                        "Local",
                        "Industry insider recruitment",
                    ),
                ],
                "broadcast_audio": [
                    (
                        "Entertainment industry podcasts (The Business, Scriptnotes)",
                        "Writers, producers, industry pros",
                        "US + Global",
                        "Recruitment sponsorships",
                    ),
                ],
            ...
            "insurance": {
                ...
                "trade_publications": [
                    (
                        "Insurance Journal",
                        "Insurance agents, underwriters, adjusters",
                        "US",
                        "Insurance industry recruitment ads",
                    ),
                    (
                        "Best's Review (AM Best)",
                        "Insurance executives, actuaries",
                        "US + Global",
                        "Premium insurance career listings",
                    ),
                    (
                        "National Underwriter",
                        "P&C, life/health underwriters",
                        "US",
                        "Underwriting recruitment classifieds",
                    ),
                ],
                "digital_media": [
                    (
                        ...
                        "All insurance roles",
                        "US",
                        "Insurance-specific job board",
                    ),
                    (
                        "The Institutes Career Center",
                        "Certified insurance professionals (CPCU, ARM)",
                        "US",
                        "Credential-based recruitment",
                    ),
                    (
                        "Actuarial Outpost / GoActuary",
                        "Actuaries, actuarial students",
                        "US + Global",
                        "Actuarial recruitment platform",
                    ),
                ],
                "ooh_print": [
                    (
                        "Insurance Industry Conference Programs (RIMS, CPCU)",
                        "Insurance professionals",
                        "US + Global",
                        "Conference career fairs, program ads",
                    ),
                    (
                        "Actuarial exam prep center boards",
                        "Aspiring actuaries",
                        "US",
                        "Pipeline recruitment for actuarial talent",
                    ),
                ],
                "broadcast_audio": [
                    (
                        "Insurance podcasts (Insurance Journal, Carrier Mgmt)",
                        "Insurance professionals",
                        "US",
                        "Recruitment sponsorships",
                    ),
            ...
            "legal_services": {
                ...
                "trade_publications": [
                    (
                        "The American Lawyer",
                        "BigLaw attorneys, firm management",
                        "US",
                        "Legal recruitment ads, lateral classifieds",
                    ),
                    (
                        "National Law Journal",
                        "Attorneys, judges, legal professionals",
                        "US",
                        "Career section, recruitment display ads",
                    ),
                    (
                        "ABA Journal",
                        "All practicing attorneys (ABA members)",
                        "US",
                        "Bar association recruitment listings",
                    ),
                    (
                        "Law360",
                        "In-house counsel, litigators, transactional",
                        "US",
                        "Legal news + career center",
                    ),
                ],
                "digital_media": [
                    (
                        "LawCrossing",
                        "Attorney, paralegal, legal staff",
                        "US",
                        "Legal-specific job aggregator",
                    ),
                    (
                        "Robert Half Legal / Special Counsel",
                        "Legal professionals, temp-to-perm",
                        "US",
                        "Legal staffing + recruitment",
                    ),
                    (
                        "Above the Law",
                        "Associates, law students, in-house counsel",
                        "US",
                        "Legal career content, employer features",
                    ),
                    (
                        "NALP / Law School Career Centers",
                        "Law students, recent JD graduates",
                        "US",
                        "Pipeline from law schools",
                    ),
                ],
                "ooh_print": [
                    (
                        "Bar Association Event Programs (ABA, State Bars)",
                        "Practicing attorneys",
                        "US",
                        "CLE event career fairs, program ads",
                    ),
                    (
                        "Law School Campus Boards (Top 50)",
                        "Law students, 1L-3L",
                        "Regional",
                        "On-campus legal recruitment",
                    ),
                ],
                "broadcast_audio": [
                    (
                        "Legal podcasts (Strict Scrutiny, Lawyer 2 Lawyer)",
            ...
            "automotive": {
                ...
                "trade_publications": [
                    (
                        "Automotive News",
                        "Auto industry executives, dealers",
                        "US + Global",
                        "Automotive recruitment leader",
                    ),
                    (
                        "SAE International Publications",
                        "Automotive engineers, EV specialists",
                        "Global",
                        "Engineering recruitment, career center",
                    ),
                    (
                        "IndustryWeek",
                        "Manufacturing plant managers, directors",
                        "US",
                        "Manufacturing recruitment ads",
                    ),
                    (
                        "Assembly Magazine",
                        "Manufacturing, assembly professionals",
                        "US",
                        "Plant-level recruitment ads",
                    ),
                ],
                "digital_media": [
                    (
                        "AutoJobs.com",
                        "Dealership, automotive service, manufacturing",
                        "US",
                        "Auto industry job board",
                    ),
                    (
                        "iHireManufacturing",
                    ...
                    (
                        ...
                        "Global",
                        "Engineering recruitment platform",
                    ),
                    (
                        "DealerSocket / Hireology (Dealers)",
                        "Dealership staff recruitment",
                        "US",
                        "Automotive retail recruitment",
                    ),
                ],
                "ooh_print": [
                    (
                        "Auto Shows (NAIAS, LA Auto Show) Programs",
                        "Automotive professionals, enthusiasts",
                        "US + Global",
                        "Event recruitment, brand visibility",
                    ),
                    (
                        "Manufacturing Plant Corridor Boards",
                        "Current employees (referrals), temp workers",
                        "Local",
                        "Internal referral recruitment",
                    ),
                    (
                        "Vocational School Auto Programs",
                        "Auto tech students, mechanics",
                        "Local",
                        "Technician pipeline recruitment",
                    ),
                ],
                "broadcast_audio": [
                    (
                        "Automotive podcasts (Autoline, The Drive)",
                        "Auto industry professionals",
                        "US + Global",
                        "Recruitment sponsorships",
                    ),
                    (
                        "Local radio near manufacturing plants",
                        "Manufacturing workers, shift employees",
                        "Regional",
                        "Geo-targeted recruitment spots",
                    ),
                ],
            ...
            "military_recruitment": {
                ...
                "trade_publications": [
                    (
                        "Military Times (Army/Navy/Air Force/Marine Times)",
                        "Active duty, veterans, military families",
                        "US",
                        "Premier military recruitment publication",
                    ),
                    (
                        "Stars and Stripes",
                        "Overseas military, DoD civilians",
                        "Global",
                        "Overseas military community recruitment",
                    ),
                    (
                        "G.I. Jobs Magazine",
                        "Transitioning service members",
                        "US",
                        "Military-to-civilian career guide",
                    ),
                ],
                "digital_media": [
                    (
                        "Military.com",
                        "5M+ Veterans, active duty, families",
                        "US",
                        "Largest military job board & community",
                    ),
                    (
                        "Hire Heroes USA",
                        "Veteran job seekers",
                        "US",
                        "Nonprofit veteran career placement",
                    ),
                    (
                        "RecruitMilitary",
                        "Military-experienced professionals",
                        "US",
                        "Veteran job fairs + job board",
                    ),
                    (
                        "USAJOBS (Federal)",
                ...
                "ooh_print": [
                    (
                        "Military Base Transition Assistance (TAP) Centers",
                        "Separating service members",
                        "US + Global",
                        "Direct military-to-civilian pipeline",
                    ),
                    (
                        "VFW / American Legion Posts",
                        "Veterans community",
                        "US",
                        "Community-based veteran recruitment",
                    ),
                    (
                        "Military Installation Bulletin Boards",
                        "Active duty, dependents",
                        "US",
                        "On-base job postings",
                    ),
                ],
                "broadcast_audio": [
                    (
                        "Armed Forces Radio (AFN)",
                        "Active duty service members worldwide",
                        "Global",
                        "Overseas military recruitment",
                    ),
                    (
                        "Veteran podcasts (Borne the Battle, Jocko Podcast)",
            ...
            "maritime_marine": {
                ...
                "trade_publications": [
                    (
                        "Maritime Executive",
                        "Ship officers, port management, naval architects",
                        "Global",
                        "Maritime career section, recruitment ads",
                    ),
                    (
                        "Marine Log",
                    ...
                    (
                        "TradeWinds",
                        "Shipping, offshore, maritime executives",
                        "Global",
                        "Shipping industry career classifieds",
                    ),
                    (
                        "WorkBoat Magazine",
                        "Workboat operators, inland waterway pros",
                        "US",
                        "Domestic maritime recruitment",
                    ),
                ],
                "digital_media": [
                    (
                        "MarineLink / Maritime Jobs",
                        "All maritime roles globally",
                        "Global",
                        "Maritime-specific job board",
                    ),
                    (
                        "Crew4Yachts / dockwalk",
                        "Yacht crew, superyacht positions",
                        "Global",
                        "Luxury maritime recruitment",
                    ),
                    (
                        "GCAPTAIN",
                        "Maritime professionals, ship captains",
                        "Global",
                        "Maritime news + career section",
                    ),
                ],
                "ooh_print": [
                    (
                        "Port Authority / Seamen's Church Boards",
                        "Mariners, dockworkers, port staff",
                        "Regional",
                        "Direct maritime community recruitment",
                    ),
                    (
                        "Maritime Academy Campus Boards",
                        "Maritime cadets, new graduates",
                        "US",
                        "Maritime officer pipeline",
                    ),
                    (
                        "Ship Chandlery / Maritime Supply Stores",
                        "Active mariners",
                        "Regional",
                        "Industry community postings",
                    ),
                ],
                "broadcast_audio": [
                    (
                        "Maritime podcasts (Maritime Podcast, The Shipping Podcast)",
        ...
        media_platforms_db["general_entry_level"] = {
            ...
            "trade_publications": [
                (
                    ...
                    "General job seekers, all levels",
                    "Local",
                    "Traditional recruitment, broad reach",
                ),
                (
                    "Community Newspapers / Weeklies",
                    "Local residents, hourly workers",
                    "Local",
                    "Affordable, community-trusted channel",
                ),
            ],
            "digital_media": [
                (
                    "Indeed Sponsored Jobs",
                    "All job seekers, 250M+ monthly visitors",
                    "Global",
                    "Largest job site, broad programmatic reach",
                ),
                (
                    "Facebook Jobs",
                    "Hourly, entry-level, local job seekers",
                    "Global",
                    "Social recruiting, community groups",
                ),
                (
                    "Google for Jobs",
                    "High-intent job searchers",
                    "Global",
                    "Organic + paid local job visibility",
                ),
                (
                    "Craigslist Jobs",
                    "Local hourly, entry-level, gig workers",
                    "US",
                    "Low-cost local recruitment",
                ),
            ],
            "ooh_print": [
                (
                    "Community Bulletin Boards (Libraries, Rec Centers)",
                    "Local job seekers",
                    "Local",
                    "Free community reach",
                ),
                (
                    "Public Transit Ads (Bus, Subway, Shelters)",
                    "Commuting workers, entry-level",
                    "Regional",
                    "High-frequency visibility",
                ),
                (
                    "Grocery Store / Laundromat Boards",
                    "Neighborhood residents seeking work",
                    "Local",
                    "Hyperlocal community reach",
                ),
            ],
            "broadcast_audio": [
                (
                    "Local Radio (Mix of formats)",
                    "General audience by demographics",
                    "Regional",
                    "Broad recruitment reach, drive-time slots",
                ),
                (
                    "Spotify/Pandora Geo-targeted Ads",
                    "Younger demographics by location",
                    "Regional",
                    "Digital audio recruitment ads",
                ),
            ],
        }
        ...
        if ind_key not in media_platforms_db:
            ...
            alias_map = {
                "niche_industry": "general_entry_level",
                "mental_health": "healthcare_medical",
            }
            ind_key = alias_map.get(ind_key, "general_entry_level")
        ind_platforms = media_platforms_db.get(
            ind_key, media_platforms_db["general_entry_level"]
        )
        ...
        ws_media["B3"].value = (
            f"Recommended recruitment marketing channels for {ind_label} roles in {loc_context}. These platforms complement digital programmatic channels to reach passive candidates and build employer brand."
        )
        ...
        for cat_key, (cat_label, cat_color) in category_labels.items():
            ...
            for c in range(3, 8):
                ws_media.cell(row=row, column=c).fill = PatternFill(
                    start_color=cat_color, end_color=cat_color, fill_type="solid"
                )
            row += 1
            ...
            for idx, plat in enumerate(platforms):
                ...
                for ci, val in enumerate(
                    [name, type_label, audience, reach, use_case, rec]
                ):
                    ...
                    cell.font = Font(
                        ...
                        size=10,
                        bold=(ci == 0),
                        color="1B2A4A" if ci == 0 else "333333",
                    )
                    cell.fill = row_fill
        ...
        cell = ws_media.cell(
            row=row,
            column=2,
            value="💡 Recommendation: Combine 2-3 print/OOH channels with digital programmatic for optimal passive + active candidate coverage. Allocate 10-15% of total recruitment budget to media/print channels for employer brand lift.",
        )
    ...
    if isinstance(_ad_plat, dict) and _ad_plat:
        ...
        if _top_platform:
            ...
            _rec_cell = ws_adplat.cell(
                row=adp_row,
                column=2,
                value=f"Top Recommendation: {_top_platform} (Fit Score: {_top_score:.0%})",
            )
            _rec_cell.font = Font(name="Calibri", bold=True, size=12, color="FFFFFF")
            _rec_cell.fill = PatternFill(
                start_color="2E7D32", end_color="2E7D32", fill_type="solid"
            )
            _rec_cell.alignment = center_alignment
            for c in range(3, 10):
                ws_adplat.cell(row=adp_row, column=c).fill = PatternFill(
                    start_color="2E7D32", end_color="2E7D32", fill_type="solid"
                )
            adp_row += 2
        ...
        if isinstance(_platforms_list, dict):
            for pidx, (pname, pdata) in enumerate(_platforms_list.items()):
                ...
                _reach_val = pdata.get(
                    "estimated_reach", pdata.get("audience_reach", "N/A")
                )
                _roi_val = pdata.get("roi_projection", "N/A")
                _daily_val = pdata.get(
                    "daily_budget_range", pdata.get("recommended_daily_budget", "N/A")
                )
                ...
                if isinstance(_daily_val, (list, tuple)) and len(_daily_val) == 2:
                    ...
                    _daily_val = f"${_daily_val.get('min') or 0:,.0f} - ${_daily_val.get('max') or 0:,.0f}"
                ...
                for ci, val in enumerate(
                    [
                        ...
                        _cpa_val,
                        _reach_val,
                        f"{_fit:.0%}" if isinstance(_fit, (int, float)) else str(_fit),
                        _roi_val,
                        _daily_val,
                    ]
                ):
                ...
                if isinstance(_fit, (int, float)):
                    if _fit >= 0.7:
                        fit_cell.fill = PatternFill(
                            start_color="E8F5E9", end_color="E8F5E9", fill_type="solid"
                        )
                        fit_cell.font = Font(
                            name="Calibri", bold=True, size=10, color="2E7D32"
                        )
                    elif _fit >= 0.4:
                        fit_cell.fill = PatternFill(
                            start_color="FFF3E0", end_color="FFF3E0", fill_type="solid"
                        ...
                        fit_cell.font = Font(
                            name="Calibri", bold=True, size=10, color="F57C00"
                        )
                    else:
                        fit_cell.fill = PatternFill(
                        ...
                        fit_cell.font = Font(
                            name="Calibri", bold=True, size=10, color="C62828"
                        )
                adp_row += 1
        ...
        ws_adplat.cell(
            row=adp_row,
            column=2,
            value="Fit scores are computed from industry match, audience alignment, cost efficiency, and historical conversion benchmarks. Higher scores indicate better expected ROI.",
        ).font = Font(name="Calibri", italic=True, size=9, color="596780")
    ...
    if isinstance(_sal_intel, dict) and _sal_intel:
        ...
        ws_salary.cell(
            row=sal_row,
            column=2,
            value="Fused salary data from multiple API sources with confidence scoring. Use for competitive offer benchmarking and budget calibration.",
        ).font = Font(name="Calibri", italic=True, size=9, color="596780")
        sal_row += 2
        ...
        _sal_roles = (
            _sal_intel.get("roles", _sal_intel) if isinstance(_sal_intel, dict) else {}
        )
        if isinstance(_sal_roles, dict):
            for sidx, (role_name, role_data) in enumerate(_sal_roles.items()):
                ...
                _conf = role_data.get(
                    "confidence", role_data.get("confidence_score") or 0
                )
                ...
                _sal_vals = [
                    role_name,
                    (
                        f"${role_data.get('min', role_data.get('p10') or 0):,.0f}"
                        if isinstance(
                            role_data.get("min", role_data.get("p10") or 0),
                            (int, float),
                        )
                        else str(role_data.get("min", "N/A"))
                    ),
                    (
                        f"${role_data.get('p25') or 0:,.0f}"
                        if isinstance(role_data.get("p25") or 0, (int, float))
                        else str(role_data.get("p25", "N/A"))
                    ),
                    (
                        f"${role_data.get('median', role_data.get('p50') or 0):,.0f}"
                        if isinstance(
                            role_data.get("median", role_data.get("p50") or 0),
                            (int, float),
                        )
                        else str(role_data.get("median", "N/A"))
                    ),
                    (
                        f"${role_data.get('p75') or 0:,.0f}"
                        if isinstance(role_data.get("p75") or 0, (int, float))
                        else str(role_data.get("p75", "N/A"))
                    ),
                    (
                        f"${role_data.get('max', role_data.get('p90') or 0):,.0f}"
                        if isinstance(
                            role_data.get("max", role_data.get("p90") or 0),
                            (int, float),
                        )
                        else str(role_data.get("max", "N/A"))
                    ),
                    str(_srcs),
                ...
                if isinstance(_conf, (int, float)) and _conf < 0.5:
                    for c in range(2, 10):
                        ws_salary.cell(row=sal_row, column=c).fill = PatternFill(
                            start_color="FFF3E0", end_color="FFF3E0", fill_type="solid"
                        )
        ...
        ws_salary.cell(
            row=sal_row,
            column=2,
            value="Rows highlighted in amber indicate low confidence (< 50%). Consider supplementing with additional market research for those roles.",
        ).font = Font(name="Calibri", italic=True, size=9, color="596780")
    ...
    if isinstance(_mkt_demand, dict) and _mkt_demand:
        ...
        ws_demand.cell(
            row=dem_row,
            column=2,
            value="Job market demand signals fused from job posting APIs, search trend data, and talent pool analysis. Temperature indicates hiring difficulty.",
        ).font = Font(name="Calibri", italic=True, size=9, color="596780")
        dem_row += 2
        ...
        _dem_roles = (
            _mkt_demand.get("roles", _mkt_demand)
            if isinstance(_mkt_demand, dict)
            else {}
        )
        if isinstance(_dem_roles, dict):
            for didx, (role_name, role_data) in enumerate(_dem_roles.items()):
                ...
                _postings = role_data.get(
                    "total_postings",
                    role_data.get(
                        "job_postings", role_data.get("posting_count", "N/A")
                    ),
                )
                _search = role_data.get(
                    "search_volume_monthly",
                    role_data.get(
                        "search_interest", role_data.get("search_volume", "N/A")
                    ),
                )
                _pool = role_data.get(
                    "talent_pool_estimate",
                    role_data.get("talent_pool", role_data.get("talent_supply", "N/A")),
                )
                _comp = role_data.get(
                    "competition_index", role_data.get("competition", "N/A")
                )
                _temp = role_data.get(
                    "market_temperature", role_data.get("temperature", "N/A")
                )
                _trend = role_data.get(
                    "trend_direction",
                    role_data.get("trend", role_data.get("demand_trend", "N/A")),
                )
                ...
                if "hot" in _temp_lower:
                    ...
                    temp_cell.font = Font(
                        name="Calibri", bold=True, size=10, color="E65100"
                    )
                    ...
                    temp_cell.font = Font(
                        name="Calibri", bold=True, size=10, color="1565C0"
                    )
                elif "cold" in _temp_lower:
                    temp_cell.fill = PatternFill(
                        start_color="ECEFF1", end_color="ECEFF1", fill_type="solid"
                    )
                    temp_cell.font = Font(
                        name="Calibri", bold=True, size=10, color="78909C"
                    )
                dem_row += 1
        ...
        ws_demand.cell(
            row=dem_row,
            column=2,
            value="Temperature: Hot = high demand/low supply (hardest to hire), Cold = low demand/high supply (easiest). Competition Index reflects employer competition for the same talent.",
        ).font = Font(name="Calibri", italic=True, size=9, color="596780")
    ...
    if isinstance(_loc_profiles, dict) and _loc_profiles:
        ...
        ws_loc.cell(
            row=loc_row,
            column=2,
            value="Per-location economic and workforce profiles fused from Census, GeoNames, Teleport, REST Countries, IMF, and World Bank data sources.",
        ).font = Font(name="Calibri", italic=True, size=9, color="596780")
        loc_row += 2
        ...
        for lidx, (loc_name, loc_data) in enumerate(_loc_profiles.items()):
            ...
            if isinstance(_loc_col, dict):
                _col_idx = _loc_col.get("index", _loc_col.get("cost_index") or "")
                _col_display = (
                    f"{_col_idx:.1f}"
                    if isinstance(_col_idx, (int, float))
                    else str(_col_idx) if _col_idx else "N/A"
                )
            else:
            ...
            _loc_src_count = (
                _loc_meta.get("source_count") or 0 if isinstance(_loc_meta, dict) else 0
            )
            ...
            if (isinstance(_econ, dict) and _econ) or (
                isinstance(_wb_ind, dict) and _wb_ind
            ):
                _gdp_g = (
                    _econ.get("gdp_growth", _wb_ind.get("gdp_growth", None))
                    if isinstance(_econ, dict)
                    else (
                        _wb_ind.get("gdp_growth", None)
                        if isinstance(_wb_ind, dict)
                        else None
                    )
                )
                _unemp = (
                    _econ.get("unemployment", _wb_ind.get("unemployment_rate", None))
                    if isinstance(_econ, dict)
                    else (
                        _wb_ind.get("unemployment_rate", None)
                        if isinstance(_wb_ind, dict)
                        else None
                    )
                )
                _infl = (
                    _econ.get("inflation", None) if isinstance(_econ, dict) else None
                )
                econ_parts = []
                ...
                if econ_parts:
                    ...
                    ws_loc.cell(
                        row=loc_row,
                        column=3,
                        value=f"Economic: {' | '.join(econ_parts)}",
                    ).font = Font(name="Calibri", italic=True, size=9, color="596780")
                    loc_row += 1
            ...
            if isinstance(_country, dict) and _country:
                ...
                _region = _country.get("region") or ""
                country_parts = []
                if _region:
                ...
                if isinstance(_currencies, dict) and _currencies:
                    curr_names = [
                        ...
                        for k, v in list(_currencies.items())[:3]
                    ]
                    country_parts.append(f"Currency: {', '.join(curr_names)}")
                if isinstance(_languages, dict) and _languages:
                    lang_names = (
                        list(_languages.values())[:3]
                        if all(isinstance(v, str) for v in _languages.values())
                        else list(_languages.keys())[:3]
                    )
                    country_parts.append(
                        f"Languages: {', '.join(str(l) for l in lang_names)}"
                    )
                if country_parts:
                    ws_loc.merge_cells(f"C{loc_row}:I{loc_row}")
                    ws_loc.cell(row=loc_row, column=2, value="").fill = _row_fill
                    ws_loc.cell(
                        row=loc_row,
                        column=3,
                        value=f"Country: {' | '.join(country_parts)}",
                    ).font = Font(name="Calibri", italic=True, size=9, color="596780")
                    loc_row += 1
        ...
        ws_loc.cell(
            row=loc_row,
            column=2,
            value="Sources include US Census Bureau, DataUSA, GeoNames, Teleport QoL Index, REST Countries, IMF WEO, and World Bank. Source count indicates number of independent data sources contributing to each profile.",
        ).font = Font(name="Calibri", italic=True, size=9, color="596780")
    ...
    if isinstance(_comp_intel, dict) and _comp_intel:
        ...
        if isinstance(_comp_profile, dict) and _comp_profile:
            ...
            _profile_fields = [
                ...
                (
                    "SIC Industry",
                    _comp_profile.get("sec_sic_description", "N/A") or "N/A",
                ),
        ...
        if isinstance(_hiring, dict) and _hiring:
            ...
            if isinstance(_mkt_ctx, dict) and _mkt_ctx:
                ...
                ws_comp.cell(
                    row=comp_row,
                    column=2,
                    value=f"Market State: {_mkt_state} | AI Adoption in Recruiting: {_ai_adoption}",
                ).font = Font(name="Calibri", italic=True, size=10, color="596780")
                comp_row += 1
        ...
        if isinstance(_mkt_pos, dict) and _mkt_pos:
            ...
            _pos_items = [
                ...
                (
                    "Publicly Traded",
                    "Yes" if _mkt_pos.get("is_public_company") else "No",
                ),
                ("Known Competitors", str(_mkt_pos.get("competitor_count") or 0)),
                (
                    "SEC Filings Available",
                    "Yes" if _mkt_pos.get("has_sec_filings") else "No",
                ),
            ]
        ...
        if isinstance(_clearbit_co, dict) and _clearbit_co:
            comp_row += 2
            style_section_header(
                ws_comp, comp_row, 2, 7, "Company Intelligence (Clearbit)"
            )
            comp_row += 2
            _cb_fields = [
                ...
                (
                    "Employee Count",
                    (
                        f"{_clearbit_co.get('employee_count', 'N/A'):,}"
                        if isinstance(_clearbit_co.get("employee_count"), (int, float))
                        else str(_clearbit_co.get("employee_count", "N/A"))
                    ),
                ),
                (
                    "Annual Revenue",
                    (
                        f"${_clearbit_co.get('annual_revenue') or 0:,.0f}"
                        if isinstance(_clearbit_co.get("annual_revenue"), (int, float))
                        and _clearbit_co.get("annual_revenue")
                        else "N/A"
                    ),
                ),
            ]
        ...
        if isinstance(_wiki_co, dict) and _wiki_co:
            _wiki_desc = _wiki_co.get("description") or ""
            ...
            if _wiki_desc or _wiki_founded or _wiki_hq:
                comp_row += 2
                style_section_header(
                    ws_comp, comp_row, 2, 7, "Company Background (Wikipedia)"
                )
                comp_row += 2
                _wiki_fields = []
                if _wiki_desc:
                ...
                if isinstance(_sec_filings, list) and _sec_filings:
                    ...
                    for fidx, filing in enumerate(_sec_filings[:5]):
                        ...
                        _f_vals = [
                            ...
                            filing.get(
                                "description",
                                filing.get("primaryDocDescription", "N/A"),
                            ),
                        ]
                        for ci, val in enumerate(_f_vals):
                            cell = ws_comp.cell(
                                row=comp_row, column=2 + ci, value=str(val)[:100]
                            )
                            cell.font = Font(name="Calibri", size=10)
                            cell.fill = _row_fill
                            cell.border = thin_border
                            cell.alignment = (
                                wrap_alignment if ci == 2 else center_alignment
                            )
                        comp_row += 1
        ...
        ws_comp.cell(
            row=comp_row,
            column=2,
            value="Data sourced from Wikipedia, Clearbit, SEC EDGAR filings, BLS QCEW, and internal knowledge base. Competitor data based on industry classification and domain analysis.",
        ).font = Font(name="Calibri", italic=True, size=9, color="596780")
    ...
    if isinstance(_budget_alloc, dict) and _budget_alloc:
        ...
        _bstr_display = str(data.get("budget") or "" or "")
        try:
            _bnums_d = re.findall(
                r"[\d]+", _bstr_display.replace(",", "").replace("$", "").strip()
            )
            _total_budget_val = float(_bnums_d[0]) if _bnums_d else 0
        except (ValueError, IndexError):
            _total_budget_val = 0

        _proj_hires = _total_proj.get("hires") or 0
        _proj_cph = _total_proj.get("cost_per_hire") or 0

        _hero_items = [
            (
                "TOTAL BUDGET",
                (
                    f"${_total_budget_val:,.0f}"
                    if _total_budget_val > 0
                    else str(data.get("budget", "N/A"))
                ),
            ),
            (
                "PROJECTED HIRES",
                (
                    f"{_proj_hires:,.0f}"
                    if isinstance(_proj_hires, (int, float))
                    else str(_proj_hires)
                ),
            ),
            (
                "PROJECTED COST/HIRE",
                (
                    f"${_proj_cph:,.0f}"
                    if isinstance(_proj_cph, (int, float)) and _proj_cph > 0
                    else "N/A"
                ),
            ),
        ]
        for hidx, (hlabel, hval) in enumerate(_hero_items):
            ...
            hcell.fill = PatternFill(
                start_color=ACCENT, end_color=ACCENT, fill_type="solid"
            )
            hcell.alignment = Alignment(horizontal="center", vertical="center")
            for cc in range(col_start, col_end + 1):
                ws_ba.cell(row=ba_row, column=cc).fill = PatternFill(
                    start_color=ACCENT, end_color=ACCENT, fill_type="solid"
                )
            ws_ba.merge_cells(
                start_row=ba_row + 1,
            ...
            lcell.fill = PatternFill(
                start_color=NAVY, end_color=NAVY, fill_type="solid"
            )
            lcell.alignment = Alignment(horizontal="center", vertical="center")
            for cc in range(col_start, col_end + 1):
                ws_ba.cell(row=ba_row + 1, column=cc).fill = PatternFill(
                    start_color=NAVY, end_color=NAVY, fill_type="solid"
                )
        ...
        if isinstance(_ch_allocs, dict):
            for chidx, (ch_name, ch_data) in enumerate(_ch_allocs.items()):
                ...
                _pct = ch_data.get("percentage") or 0
                _amt = ch_data.get("dollar_amount", ch_data.get("amount") or 0)
                _clicks = ch_data.get("projected_clicks") or 0
                _apps = ch_data.get("projected_applications") or 0
                _hires = ch_data.get("projected_hires") or 0
                _cpc = ch_data.get("cpc", ch_data.get("cost_per_click") or 0)
                _cpa = ch_data.get("cpa", ch_data.get("cost_per_application") or 0)
                _cph = ch_data.get("cost_per_hire") or 0
                ...
                _ch_vals = [
                    ...
                    (
                        f"{_clicks:,.0f}"
                        if isinstance(_clicks, (int, float))
                        else str(_clicks)
                    ),
                    f"{_apps:,.0f}" if isinstance(_apps, (int, float)) else str(_apps),
                    (
                        f"{_hires:,.1f}"
                        if isinstance(_hires, (int, float))
                        else str(_hires)
                    ),
                    ...
                    (
                        f"${_cph:,.0f}"
                        if isinstance(_cph, (int, float)) and _cph > 0
                        else "N/A"
                    ),
        ...
        _total_vals = [
            ...
            f"{_total_clicks:,.0f}",
            f"{_total_apps:,.0f}",
            f"{_total_hires_ch:,.1f}",
            "",
            "",
            (
                f"${_total_spend / max(_total_hires_ch, 1):,.0f}"
                if _total_hires_ch > 0
                else "N/A"
            ),
            "",
        ]
        ...
        if isinstance(_suff, dict) and _suff:
            ...
            _suff_msg = _suff.get("message", _suff.get("assessment") or "")
            _suff_color = (
                "2E7D32"
                if str(_suff_grade).upper() in ("A", "B", "SUFFICIENT", "GOOD")
                else (
                    "F57C00"
                    if str(_suff_grade).upper() in ("C", "MODERATE", "MARGINAL")
                    else "C62828"
                )
            )
            ws_ba.merge_cells(f"B{ba_row}:K{ba_row}")
            _suff_cell = ws_ba.cell(
                row=ba_row, column=2, value=f"Grade: {_suff_grade}  |  {_suff_msg}"
            )
            _suff_cell.font = Font(
                name="Calibri", bold=True, size=11, color=_suff_color
            )
            ba_row += 1
        ...
        if _warnings and isinstance(_warnings, list):
            ...
            for w in _warnings:
                ws_ba.merge_cells(f"B{ba_row}:K{ba_row}")
                ws_ba.cell(row=ba_row, column=2, value=f"  Warning: {w}").font = Font(
        ...
        ba_reality = (
            _ba_suff.get("budget_reality_check", {})
            if isinstance(_ba_suff, dict)
            else {}
        )
        if ba_reality:
            tier = ba_reality.get("feasibility_tier") or ""
            if tier in ("impossible", "severely_underfunded", "underfunded"):
                ...
                for col_idx in range(2, 12):
                    ...
                    if _rc.value:
                        _rc.font = Font(
                            name="Calibri", bold=True, size=13, color="FF0000"
                        )
                ba_row += 2

                _rc_label = ba_reality.get("feasibility_label", "WARNING")
                _rc_msg = ba_reality.get("feasibility_message") or ""
                _rc_color = (
                    "C00000"
                    if tier == "impossible"
                    else "E65100" if tier == "severely_underfunded" else "F57C00"
                )
                ...
                _lbl_cell.fill = PatternFill(
                    start_color=_rc_color, end_color=_rc_color, fill_type="solid"
                )
                _lbl_cell.alignment = Alignment(horizontal="center", vertical="center")
                for _cc in range(2, 12):
                    ws_ba.cell(row=ba_row, column=_cc).fill = PatternFill(
                        start_color=_rc_color, end_color=_rc_color, fill_type="solid"
                    )
                ba_row += 1
                ...
                _rc_details = [
                    (
                        "Budget per Hire:",
                        f"${ba_reality.get('budget_per_hire') or 0:,.0f}",
                    ),
                    (
                        "Industry Avg CPH:",
                        f"${ba_reality.get('industry_avg_cph') or 0:,.0f}",
                    ),
                    (
                        "Realistic Hires at This Budget:",
                        str(ba_reality.get("realistic_hires", "N/A")),
                    ),
                    ("Target Hires:", str(ba_reality.get("target_hires", "N/A"))),
                    (
                        "Recommended Min Budget:",
                        f"${ba_reality.get('min_viable_budget') or 0:,.0f}",
                    ),
                ]
                for _dl, _dv in _rc_details:
                    ws_ba.cell(row=ba_row, column=2, value=f"  {_dl}").font = Font(
                        name="Calibri", bold=True, size=10, color=NAVY
                    )
                    ws_ba.merge_cells(f"D{ba_row}:F{ba_row}")
                    ws_ba.cell(row=ba_row, column=4, value=_dv).font = Font(
                        name="Calibri", size=10, color="333333"
                    )
                    ba_row += 1
                ba_row += 1
        ...
        if _recs and isinstance(_recs, list):
            ...
            for ridx, rec in enumerate(_recs):
                ...
                ws_ba.merge_cells(f"B{ba_row}:K{ba_row}")
                ws_ba.cell(
                    row=ba_row, column=2, value=f"  {ridx + 1}. {rec_text}"
    ...
    if isinstance(_hist_perf, dict) and _hist_perf.get("platforms"):
        try:
            ...
            ws_hist.cell(
                row=h_row,
                column=2,
                value="Historical Performance vs. Recommended Strategy",
            ).font = Font(name="Calibri", bold=True, size=14, color="E65100")
            h_row += 1
            ws_hist.merge_cells(f"B{h_row}:H{h_row}")
            ws_hist.cell(
                row=h_row,
                column=2,
                value=f"Based on uploaded data from: {', '.join(_hist_perf.get('source_files', ['client data']))}",
            ).font = Font(name="Calibri", italic=True, size=10, color="596780")
            h_row += 2
            ...
            hist_summary = [
                (
                    ...
                    f"${_hist_perf.get('total_spend') or 0:,.0f}",
                ),
                ("Total Hires", f"{_hist_perf.get('total_hires') or 0:,}"),
                (
                    "Total Applications",
                    f"{_hist_perf.get('total_applications') or 0:,}",
                ),
                ("Overall CPA", f"${_hist_perf.get('total_cpa') or 0:,.2f}"),
                ("Overall CPH", f"${_hist_perf.get('total_cph') or 0:,.2f}"),
                ("Platforms Used", str(_hist_perf.get("platform_count") or 0)),
            ]
            for label, value in hist_summary:
                ws_hist.cell(row=h_row, column=2, value=label).font = Font(
                    name="Calibri", size=10, color="596780"
                )
                ws_hist.cell(row=h_row, column=3, value=value).font = Font(
                    name="Calibri", bold=True, size=10, color="1B2A4A"
                )
                h_row += 1
            h_row += 1
            ...
            for platform, pdata in sorted(
                _hist_perf["platforms"].items(),
                key=lambda x: x[1].get("spend") or 0,
                reverse=True,
            ):
                ws_hist.cell(row=h_row, column=2, value=platform).font = Font(
                    name="Calibri", bold=True, size=10
                )
                ws_hist.cell(
                    row=h_row, column=3, value=f"${pdata.get('spend') or 0:,.0f}"
                ).font = Font(name="Calibri", size=10)
                ws_hist.cell(
                    row=h_row, column=4, value=f"{pdata.get('applications') or 0:,}"
                ).font = Font(name="Calibri", size=10)
                ws_hist.cell(
                    row=h_row, column=5, value=f"{pdata.get('hires') or 0:,}"
                ).font = Font(name="Calibri", size=10)
                cpa = pdata.get("cpa") or 0
                cph = pdata.get("cph") or 0
                ws_hist.cell(
                    row=h_row, column=6, value=f"${cpa:,.2f}" if cpa else "N/A"
                ).font = Font(name="Calibri", size=10)
                ws_hist.cell(
                    row=h_row, column=7, value=f"${cph:,.2f}" if cph else "N/A"
                ).font = Font(name="Calibri", size=10)
    ...
    if isinstance(_budget_alloc, dict) and _budget_alloc:
        try:
            ...
            _proj_navy_fill = PatternFill(
                start_color="08294A", end_color="08294A", fill_type="solid"
            )
            _proj_white_fill = PatternFill(
                start_color="FFFFFF", end_color="FFFFFF", fill_type="solid"
            )
            _proj_alt_fill = PatternFill(
            ...
            ws_proj["B3"].value = (
                "All projections generated by Joveo's budget allocation engine based on industry benchmarks, channel CPC data, and historical conversion rates."
            )
            ws_proj["B3"].font = Font(
                name="Calibri", italic=True, size=9, color="596780"
            )
            ...
            if not isinstance(_proj_meta, dict):
                _proj_meta = {}
            _proj_tb = _proj_meta.get("total_budget") or 0

            style_section_header(
                ws_proj, proj_row, 2, 6, "Campaign Summary Projections"
            )
            proj_row += 2
            ...
            _proj_clicks = _proj_total.get("clicks") or 0
            _proj_apps = _proj_total.get("applications") or 0
            _proj_hires = _proj_total.get("hires") or 0
            _proj_cpc_avg = _proj_total.get("cost_per_click") or 0
            _proj_cpa_avg = _proj_total.get("cost_per_application") or 0
            _proj_cph_avg = _proj_total.get("cost_per_hire") or 0
            _proj_util = (
                ((_proj_clicks * _proj_cpc_avg) / _proj_tb * 100)
                if _proj_tb > 0 and _proj_cpc_avg > 0 and _proj_clicks > 0
                else 100.0
            )
            ...
            _summary_rows = [
                (
                    "Total Budget",
                    f"${_proj_tb:,.0f}" if _proj_tb > 0 else "N/A",
                    "-",
                    "User Input",
                    "Campaign investment amount",
                ),
                (
                    "Projected Total Clicks",
                    f"{_proj_clicks:,.0f}" if _proj_clicks else "N/A",
                    "Medium",
                    "Budget Engine",
                ...
                (
                    ...
                    f"{_proj_apps:,.0f}" if _proj_apps else "N/A",
                    "Medium",
                    "Budget Engine",
                    "Based on channel apply rates",
                ),
                (
                    "Projected Total Hires",
                    f"{_proj_hires:,.0f}" if _proj_hires else "N/A",
                    "Medium",
                    "Budget Engine",
                    "Based on tier-blended hire rates",
                ),
                (
                    "Avg Cost Per Click",
                    f"${_proj_cpc_avg:,.2f}" if _proj_cpc_avg > 0 else "N/A",
                    "Medium",
                    "Budget Engine",
                ...
                (
                    ...
                    f"${_proj_cpa_avg:,.2f}" if _proj_cpa_avg > 0 else "N/A",
                    "Medium",
                    "Budget Engine",
                    "Total budget / projected applications",
                ),
                (
                    "Avg Cost Per Hire",
                    f"${_proj_cph_avg:,.0f}" if _proj_cph_avg > 0 else "N/A",
                    "Medium",
                    "Budget Engine",
                    "Total budget / projected hires",
                ),
                (
                    "Budget Utilization",
                    f"{min(_proj_util, 100):.1f}%",
                    "High",
                    "Budget Engine",
                    "Percentage of budget allocated to channels",
                ),
            ]
            ...
            if _proj_ch_allocs:
                style_section_header(
                    ws_proj, proj_row, 2, 12, "Channel-by-Channel Projections"
                )
                proj_row += 2
                ...
                for chidx, (ch_name, ch_data) in enumerate(_sorted_chs):
                    ...
                    _ch_dollars = ch_data.get(
                        "dollar_amount", ch_data.get("dollars") or 0
                    )
                    _ch_pct = ch_data.get("percentage") or 0
                    _ch_cpc = ch_data.get("cpc") or 0
                    _ch_clicks = ch_data.get("projected_clicks") or 0
                    _ch_apps = ch_data.get("projected_applications") or 0
                    _ch_hires = ch_data.get("projected_hires") or 0
                    _ch_cpa = ch_data.get(
                        "cpa", ch_data.get("cost_per_application") or 0
                    )
                    _ch_cph = ch_data.get("cost_per_hire") or 0
                    _ch_roi = ch_data.get("roi_score") or 0
                    _ch_conf = ch_data.get("confidence", "low").title()

                    _ch_total_spend += (
                        _ch_dollars if isinstance(_ch_dollars, (int, float)) else 0
                    )
                    _ch_total_clicks += (
                        _ch_clicks if isinstance(_ch_clicks, (int, float)) else 0
                    )
                    _ch_total_apps += (
                        _ch_apps if isinstance(_ch_apps, (int, float)) else 0
                    )
                    _ch_total_hires += (
                        _ch_hires if isinstance(_ch_hires, (int, float)) else 0
                    )
                    ...
                    _ch_vals = [
                        ...
                        (
                            f"${_ch_dollars:,.0f}"
                            if isinstance(_ch_dollars, (int, float))
                            else str(_ch_dollars)
                        ),
                        (
                            f"{_ch_pct:.1f}%"
                            if isinstance(_ch_pct, (int, float))
                            else str(_ch_pct)
                        ),
                        (
                            f"${_ch_cpc:.2f}"
                            if isinstance(_ch_cpc, (int, float)) and _ch_cpc > 0
                            else "N/A"
                        ),
                        (
                            f"{_ch_clicks:,.0f}"
                            if isinstance(_ch_clicks, (int, float))
                            else str(_ch_clicks)
                        ),
                        (
                            f"{_ch_apps:,.0f}"
                            if isinstance(_ch_apps, (int, float))
                            else str(_ch_apps)
                        ),
                        (
                            f"{_ch_hires:,.1f}"
                            if isinstance(_ch_hires, (int, float))
                            else str(_ch_hires)
                        ),
                        (
                            f"${_ch_cpa:.2f}"
                            if isinstance(_ch_cpa, (int, float)) and _ch_cpa > 0
                            else "N/A"
                        ),
                        (
                            f"${_ch_cph:,.0f}"
                            if isinstance(_ch_cph, (int, float)) and _ch_cph > 0
                            else "N/A"
                        ),
                        (
                            f"{_ch_roi:.1f}/10"
                            if isinstance(_ch_roi, (int, float))
                            else str(_ch_roi)
                        ),
                        _ch_conf,
                    ]
                ...
                _ch_totals_vals = [
                    ...
                    (
                        f"${_ch_total_spend / max(_ch_total_apps, 1):,.2f}"
                        if _ch_total_apps > 0
                        else "N/A"
                    ),
                    (
                        f"${_ch_total_spend / max(_ch_total_hires, 1):,.0f}"
                        if _ch_total_hires > 0
                        else "N/A"
                    ),
                    "",
                    "",
            ...
            if _proj_role_allocs:
                proj_row += 1
                style_section_header(
                    ws_proj, proj_row, 2, 8, "Role-by-Role Budget Distribution"
                )
                proj_row += 2
                ...
                _sorted_roles = sorted(
                    ...
                    key=lambda x: (
                        x[1].get("dollar_amount") or 0 if isinstance(x[1], dict) else 0
                    ),
                    reverse=True,
                )
                ...
                for ridx, (role_name, role_data) in enumerate(_sorted_roles):
                    if not isinstance(role_data, dict):
                        continue
                    _rfill = _proj_white_fill if ridx % 2 == 0 else _proj_alt_fill
                    _r_share = role_data.get("budget_share") or 0
                    _r_dollars = role_data.get("dollar_amount") or 0
                    _r_tier = role_data.get("tier", "default").replace("_", " ").title()
                    _r_openings = role_data.get(
                        "openings", role_data.get("headcount", 1)
                    )
                    _r_per_opening = (
                        _r_dollars / max(_r_openings, 1) if _r_dollars > 0 else 0
                    )
            ...
            if _proj_suff:
                proj_row += 1
                style_section_header(
                    ws_proj, proj_row, 2, 6, "Budget Feasibility Assessment"
                )
                proj_row += 2

                for ci, hdr in enumerate(
                    ["Assessment", "Value", "Details", "Status", "Action Required"]
                ):
                    cell = ws_proj.cell(row=proj_row, column=2 + ci, value=hdr)
                    cell.font = _proj_header_font
                    cell.fill = _proj_navy_fill
                ...
                _suff_bpo = _proj_suff.get("budget_per_opening") or 0
                _suff_avg_cph = _proj_suff.get("industry_avg_cost_per_hire") or 0
                _suff_gap = _proj_suff.get("gap_amount") or 0
                _suff_proj_hires = _proj_suff.get("total_projected_hires") or 0
                _suff_openings = _proj_meta.get("total_openings") or 0
                ...
                _assess_rows = [
                    (
                        "Feasibility Rating",
                        _feas_label,
                        f"Budget is {'sufficient' if _suff_sufficient else 'below recommended levels'} for target openings",
                        _feas_status,
                        "Review if WARNING",
                    ),
                    (
                        "Budget Per Opening",
                        f"${_suff_bpo:,.0f}" if _suff_bpo > 0 else "N/A",
                        (
                            f"Total budget divided by {_suff_openings} openings"
                            if _suff_openings > 0
                            else "Based on total budget allocation"
                        ),
                        "INFO",
                        "-",
                    ),
                    (
                        "Industry Avg CPH",
                        f"${_suff_avg_cph:,.0f}" if _suff_avg_cph > 0 else "N/A",
                        f"Average cost-per-hire for {_proj_meta.get('industry', 'general').replace('_', ' ').title()}",
                        "BENCHMARK",
                        "-",
                    ),
                    (
                        "Projected Hires",
                        f"{_suff_proj_hires:,.0f}" if _suff_proj_hires else "N/A",
                        (
                            f"Against target of {_suff_openings}"
                            if _suff_openings > 0
                            else "Budget engine projection"
                        ),
                        "INFO",
                    ...
                    (
                        "Budget Gap",
                        f"${_suff_gap:,.0f}" if _suff_gap > 0 else "$0 (No Gap)",
                        (
                            "Additional budget needed to meet all openings at industry avg CPH"
                            if _suff_gap > 0
                            else "Budget covers projected needs"
                        ),
                        "WARNING" if _suff_gap > 0 else "PASS",
                        "Consider increasing budget" if _suff_gap > 0 else "-",
                    ),
                ]

                for ridx, (assess, val, detail, status, action) in enumerate(
                    _assess_rows
                ):
                    _rfill = _proj_white_fill if ridx % 2 == 0 else _proj_alt_fill
                    ...
                    _status_color = (
                        "2E7D32"
                        if status == "PASS"
                        else "F57C00" if status == "WARNING" else "333333"
                    )

                    for ci, cell_val in enumerate(
                        [assess, val, detail, status, action]
                    ):
                        cell = ws_proj.cell(row=proj_row, column=2 + ci, value=cell_val)
                        if ci == 3:
                            cell.font = Font(
                                name="Calibri", bold=True, size=10, color=_status_color
                            )
                        elif ci == 0:
                ...
                if _proj_warnings and isinstance(_proj_warnings, list):
                    ...
                    ws_proj.cell(row=proj_row, column=2, value="Warnings").font = Font(
                        name="Calibri", bold=True, size=11, color="C62828"
                    )
                    proj_row += 1
                    for w in _proj_warnings:
                        ws_proj.merge_cells(f"B{proj_row}:L{proj_row}")
                        ws_proj.cell(row=proj_row, column=2, value=f"  {w}").font = (
                            Font(name="Calibri", size=10, color="C62828")
                        )
                ...
                if _proj_recs and isinstance(_proj_recs, list):
                    ...
                    ws_proj.merge_cells(f"B{proj_row}:F{proj_row}")
                    ws_proj.cell(
                        row=proj_row, column=2, value="Optimization Recommendations"
                    ).font = Font(name="Calibri", bold=True, size=11, color="2E7D32")
                    proj_row += 1
                    for ridx, rec in enumerate(_proj_recs):
                        if isinstance(rec, dict):
                            rec_text = rec.get(
                                "recommendation", rec.get("message", str(rec))
                            )
                        else:
                            rec_text = str(rec)
                        ws_proj.merge_cells(f"B{proj_row}:L{proj_row}")
                        ws_proj.cell(
                            row=proj_row, column=2, value=f"  {ridx + 1}. {rec_text}"
                        ).font = Font(name="Calibri", size=10, color="333333")
                        ws_proj.row_dimensions[proj_row].height = 30
                        ws_proj.cell(row=proj_row, column=2).alignment = wrap_alignment
                        proj_row += 1
            ...
            if _proj_opt_sugg and isinstance(_proj_opt_sugg, list):
                ...
                ws_proj.merge_cells(f"B{proj_row}:L{proj_row}")
                ws_proj.cell(
                    row=proj_row, column=2, value="Optimization Suggestions"
                ).font = Font(name="Calibri", bold=True, size=11, color=ACCENT)
                proj_row += 1
                for sidx, sugg in enumerate(_proj_opt_sugg):
                    ws_proj.merge_cells(f"B{proj_row}:L{proj_row}")
                    ws_proj.cell(
                        row=proj_row, column=2, value=f"  {sidx + 1}. {sugg}"
                    ).font = Font(name="Calibri", size=10, color="333333")
                    ws_proj.row_dimensions[proj_row].height = 30
                    ws_proj.cell(row=proj_row, column=2).alignment = wrap_alignment
                    proj_row += 1
            ...
            ws_proj.cell(
                row=proj_row,
                column=2,
                value="Projections are estimates based on industry benchmarks, historical channel performance data, and Joveo's ML models. Actual results may vary based on market conditions, job posting quality, and campaign optimization.",
            ).font = Font(name="Calibri", italic=True, size=9, color="596780")
            ws_proj.row_dimensions[proj_row].height = 30
            ws_proj.cell(row=proj_row, column=2).alignment = wrap_alignment
    ...
    if isinstance(_workforce, dict) and _workforce:
        try:
            ...
            ws_wf.cell(
                row=wf_row,
                column=2,
                value="Workforce intelligence derived from industry research, white papers, and knowledge base data. Use these insights to tailor messaging, channel selection, and employer brand positioning.",
            ).font = Font(name="Calibri", italic=True, size=9, color="596780")
            wf_row += 2
            ...
            if isinstance(_gen_z, dict) and _gen_z:
                ...
                if _gz_share:
                    ...
                    ws_wf.cell(
                        row=wf_row,
                        column=2,
                        value=f"Gen-Z Workforce Share: {_gz_share}",
                    ).font = Font(name="Calibri", bold=True, size=11, color=ACCENT)
                    wf_row += 2
                ...
                if isinstance(_gz_platforms, dict) and _gz_platforms:
                    ws_wf.cell(
                        row=wf_row, column=2, value="Job Search Platform Preferences"
                    ).font = Font(name="Calibri", bold=True, size=11, color=NAVY)
                    wf_row += 1
                    ...
                    for pidx, (pname, pval) in enumerate(_gz_platforms.items()):
                        ...
                        _pval_str = (
                            ...
                            else f"{pval:.0%}" if pval <= 1 else f"{pval}%"
                        )
                ...
                if isinstance(_mobile, dict) and _mobile:
                    ws_wf.cell(
                        row=wf_row, column=2, value="Mobile vs Desktop Behavior"
                    ).font = Font(name="Calibri", bold=True, size=11, color=NAVY)
                    wf_row += 1
                    for mkey, mval in _mobile.items():
                        ...
                        c1 = ws_wf.cell(
                            row=wf_row,
                            column=2,
                            value=str(mkey).replace("_", " ").title(),
                        )
                        c1.font = Font(name="Calibri", bold=True, size=10, color=NAVY)
                        c1.fill = _wf_mf
                        c1.border = thin_border
                ...
                if isinstance(_social, dict) and _social:
                    ws_wf.cell(
                        row=wf_row, column=2, value="Social Media Habits"
                    ).font = Font(name="Calibri", bold=True, size=11, color=NAVY)
                    wf_row += 1
                    for skey, sval in _social.items():
                        c1 = ws_wf.cell(
                            row=wf_row,
                            column=2,
                            value=str(skey).replace("_", " ").title(),
                        )
                        c1.font = Font(name="Calibri", bold=True, size=10, color=NAVY)
                        c1.border = thin_border
                ...
                if isinstance(_expectations, dict) and _expectations:
                    style_section_header(
                        ws_wf, wf_row, 2, 7, "Gen-Z Workplace Expectations"
                    )
                    wf_row += 2
                    for exp_key, exp_data in _expectations.items():
                        ...
                        ws_wf.cell(
                            row=wf_row,
                            column=2,
                            value=str(exp_key).replace("_", " ").title(),
                        ).font = Font(name="Calibri", bold=True, size=11, color=ACCENT)
                        wf_row += 1
                        for ekey, eval_val in exp_data.items():
                            ...
                            c1 = ws_wf.cell(
                                row=wf_row,
                                column=2,
                                value=str(ekey).replace("_", " ").title(),
                            )
                            ...
                            ws_wf.merge_cells(f"C{wf_row}:G{wf_row}")
                            c2 = ws_wf.cell(
                                row=wf_row, column=3, value=str(eval_val)[:300]
                            )
                            c2.font = Font(name="Calibri", size=10)
                            c2.fill = _wf_ef
                            c2.border = thin_border
                ...
                if isinstance(_salary_exp, dict) and _salary_exp:
                    ws_wf.cell(
                        row=wf_row, column=2, value="Gen-Z Salary Expectations"
                    ).font = Font(name="Calibri", bold=True, size=11, color=NAVY)
                    wf_row += 1
                ...
                if isinstance(_tenure, dict) and _tenure:
                    ws_wf.cell(
                        row=wf_row, column=2, value="Gen-Z Tenure & Job Hopping"
                    ).font = Font(name="Calibri", bold=True, size=11, color=NAVY)
                    wf_row += 1
                    for tkey, tval in _tenure.items():
                        c1 = ws_wf.cell(
                            row=wf_row,
                            column=2,
                            value=str(tkey).replace("_", " ").title(),
                        )
                        c1.font = Font(name="Calibri", bold=True, size=10, color=NAVY)
                        c1.border = thin_border
                        ws_wf.merge_cells(f"C{wf_row}:D{wf_row}")
            ...
            if isinstance(_eb, dict) and _eb:
                style_section_header(
                    ws_wf, wf_row, 2, 7, "Employer Branding Intelligence"
                )
                wf_row += 2
                _roi_data = _eb.get("roi_data", {})
                if isinstance(_roi_data, dict) and _roi_data:
                    ws_wf.cell(
                        row=wf_row, column=2, value="Employer Branding ROI"
                    ).font = Font(name="Calibri", bold=True, size=11, color=NAVY)
                    wf_row += 1
                    for rkey, rval in _roi_data.items():
                        c1 = ws_wf.cell(
                            row=wf_row,
                            column=2,
                            value=str(rkey).replace("_", " ").title(),
                        )
                        c1.font = Font(name="Calibri", bold=True, size=10, color=NAVY)
                        c1.border = thin_border
                        ws_wf.merge_cells(f"C{wf_row}:G{wf_row}")
                ...
                if isinstance(_bp, dict) and _bp:
                    ws_wf.cell(
                        row=wf_row, column=2, value="Employer Branding Best Practices"
                    ).font = Font(name="Calibri", bold=True, size=11, color=NAVY)
                    wf_row += 1
                    for bkey, bval in _bp.items():
                        c1 = ws_wf.cell(
                            row=wf_row,
                            column=2,
                            value=str(bkey).replace("_", " ").title(),
                        )
                        c1.font = Font(name="Calibri", bold=True, size=10, color=NAVY)
                        c1.border = thin_border
                        ws_wf.merge_cells(f"C{wf_row}:G{wf_row}")
                ...
                if isinstance(_ch_eff, dict) and _ch_eff:
                    ws_wf.cell(
                        row=wf_row,
                        column=2,
                        value="Employer Brand Channel Effectiveness",
                    ).font = Font(name="Calibri", bold=True, size=11, color=NAVY)
                    wf_row += 1
                    for ckey, cval in _ch_eff.items():
                        c1 = ws_wf.cell(
                            row=wf_row,
                            column=2,
                            value=str(ckey).replace("_", " ").title(),
                        )
                        c1.font = Font(name="Calibri", bold=True, size=10, color=NAVY)
                        c1.border = thin_border
                        ws_wf.merge_cells(f"C{wf_row}:G{wf_row}")
            ...
            if isinstance(_sp_trends, dict) and _sp_trends:
                ...
                for spkey, spval in _sp_trends.items():
                    c1 = ws_wf.cell(
                        row=wf_row, column=2, value=str(spkey).replace("_", " ").title()
                    )
                    c1.font = Font(name="Calibri", bold=True, size=10, color=NAVY)
                    c1.border = thin_border
                    ws_wf.merge_cells(f"C{wf_row}:G{wf_row}")
            ...
            if isinstance(_jt_trends, dict) and _jt_trends:
                ...
                for jtkey, jtval in _jt_trends.items():
                    c1 = ws_wf.cell(
                        row=wf_row, column=2, value=str(jtkey).replace("_", " ").title()
                    )
                    c1.font = Font(name="Calibri", bold=True, size=10, color=NAVY)
                    c1.border = thin_border
                    ws_wf.merge_cells(f"C{wf_row}:G{wf_row}")
            ...
            ws_wf.cell(
                row=wf_row,
                column=2,
                value="Data sourced from recruitment industry knowledge base, workforce trends intelligence, and employer branding research. Gen-Z data reflects 2024-2026 behavioral studies.",
            ).font = Font(name="Calibri", italic=True, size=9, color="596780")
    ...
    if isinstance(_relevant_research, list) and _relevant_research:
        try:
            ...
            ws_refs.cell(
                row=ref_row,
                column=2,
                value="Industry research reports and white papers referenced in this media plan. Reports are selected for relevance to your industry, target roles, and recruitment strategy.",
            ).font = Font(name="Calibri", italic=True, size=9, color="596780")
            ref_row += 2

            style_section_header(
                ws_refs, ref_row, 2, 7, "Research Reports & White Papers"
            )
            ref_row += 2
            ...
            for ridx, report in enumerate(_relevant_research):
                ...
                _r_vals = [
                    ...
                    str(_r_title),
                    str(_r_publisher),
                    str(_r_year) if _r_year else "N/A",
                    _findings_display,
                ...
                for ci, val in enumerate(_r_vals):
                    ...
                    cell.border = thin_border
                    cell.alignment = (
                        center_alignment if ci in (0, 3, 5) else wrap_alignment
                    )
            ...
            ws_refs.cell(
                row=ref_row,
                column=2,
                value="Reports are ranked by relevance to your industry and recruitment objectives. Findings are summarized from original publications.",
            ).font = Font(name="Calibri", italic=True, size=9, color="596780")
        ...
        if _has_regional:
            try:
                ...
                ws_reg.cell(
                    row=reg_row,
                    column=2,
                    value="Per-market hiring intelligence including top job boards, dominant industries, talent dynamics, hiring regulations, cultural norms, and CPA benchmarks. Data sourced from regional hiring knowledge base.",
                ).font = Font(name="Calibri", italic=True, size=9, color="596780")
                reg_row += 2
                ...
                for loc_name, loc_data in _loc_profiles.items():
                    ...
                    ws_reg.cell(
                        row=reg_row,
                        column=2,
                        value=f"Region: {_ri_region}  |  Market: {_ri_market}",
                    ).font = Font(name="Calibri", italic=True, size=10, color="596780")
                    reg_row += 2
                    ...
                    if isinstance(_top_boards, list) and _top_boards:
                        ...
                        for bidx, board in enumerate(_top_boards):
                            ...
                            if isinstance(board, dict):
                                _b_vals = [
                                    board.get("name", board.get("platform", "N/A")),
                                    board.get("specialty", board.get("focus", "N/A")),
                                    board.get(
                                        "pricing_model", board.get("pricing", "N/A")
                                    ),
                                    board.get(
                                        "monthly_traffic", board.get("traffic", "N/A")
                                    ),
                                    board.get("notes", board.get("description") or ""),
                                ]
                            ...
                            for ci, val in enumerate(_b_vals):
                                _val_str = (
                                    ...
                                    else str(val)[:150] if val else ""
                                )
                                cell = ws_reg.cell(
                                    row=reg_row, column=2 + ci, value=_val_str
                                )
                                cell.font = Font(
                                    name="Calibri", size=10, bold=(ci == 0)
                                )
                                cell.fill = _reg_rf
                                cell.border = thin_border
                                cell.alignment = wrap_alignment
                    ...
                    if isinstance(_dom_ind, list) and _dom_ind:
                        ...
                        _ind_names = [
                            (
                                ind.get("name", ind.get("industry", str(ind)))
                        ...
                        ws_reg.cell(
                            row=reg_row,
                            column=2,
                            value=", ".join(str(d) for d in _ind_names[:15]),
                        ).font = Font(name="Calibri", size=10, color="333333")
                    ...
                    if isinstance(_talent_dyn, dict) and _talent_dyn:
                        ws_reg.cell(
                            row=reg_row, column=2, value="Talent Dynamics"
                        ).font = Font(name="Calibri", bold=True, size=11, color=NAVY)
                        reg_row += 1
                        for tkey, tval in _talent_dyn.items():
                            ...
                            c2 = ws_reg.cell(
                                row=reg_row, column=3, value=str(tval)[:300]
                            )
                            c2.font = Font(name="Calibri", size=10)
                            c2.fill = _td_f
                            c2.border = thin_border
                    ...
                    if isinstance(_hire_regs, dict) and _hire_regs:
                        ws_reg.cell(
                            row=reg_row, column=2, value="Hiring Regulations"
                        ).font = Font(name="Calibri", bold=True, size=11, color=NAVY)
                        reg_row += 1
                        for rkey, rval in _hire_regs.items():
                            ...
                            ws_reg.merge_cells(f"C{reg_row}:H{reg_row}")
                            c2 = ws_reg.cell(
                                row=reg_row, column=3, value=str(rval)[:300]
                            )
                            c2.font = Font(name="Calibri", size=10)
                            c2.border = thin_border
                            c2.alignment = wrap_alignment
                    ...
                    if isinstance(_cult_norms, dict) and _cult_norms:
                        ws_reg.cell(
                            row=reg_row, column=2, value="Cultural Norms"
                        ).font = Font(name="Calibri", bold=True, size=11, color=NAVY)
                        reg_row += 1
                        for ckey, cval in _cult_norms.items():
                            ...
                            ws_reg.merge_cells(f"C{reg_row}:H{reg_row}")
                            c2 = ws_reg.cell(
                                row=reg_row, column=3, value=str(cval)[:300]
                            )
                            c2.font = Font(name="Calibri", size=10)
                            c2.border = thin_border
                            c2.alignment = wrap_alignment
                    ...
                    if isinstance(_cpa_bench, dict) and _cpa_bench:
                        ws_reg.cell(
                            row=reg_row, column=2, value="CPA Benchmarks"
                        ).font = Font(name="Calibri", bold=True, size=11, color=NAVY)
                        reg_row += 1
                        ...
                        for cbkey, cbval in _cpa_bench.items():
                            ...
                            c1 = ws_reg.cell(
                                row=reg_row,
                                column=2,
                                value=str(cbkey).replace("_", " ").title(),
                            )
                            c1.font = Font(
                                name="Calibri", bold=True, size=10, color="2E7D32"
                            )
                ...
                ws_reg.cell(
                    row=reg_row,
                    column=2,
                    value="Regional intelligence sourced from regional hiring knowledge base, local market research, and industry-specific talent data. CPA benchmarks are market-specific estimates.",
                ).font = Font(name="Calibri", italic=True, size=9, color="596780")

            except Exception as _reg_err:
                logger.warning(
                    "Regional Intelligence sheet generation failed: %s", _reg_err
                )
    ...
    if isinstance(_conf_scores, dict) and _conf_scores:
        ...
        _overall = _conf_scores.get("overall", _conf_scores.get("overall_score") or 0)
        if isinstance(_overall, (int, float)):
            ...
            _grade_color = (
                "2E7D32"
                if _grade in ("A", "B")
                else "F57C00" if _grade == "C" else "C62828"
            )
        else:
            _grade = str(_overall)
            _grade_color = "333333"
        ...
        _grade_cell = ws_conf.cell(
            row=conf_row, column=2, value=f"Overall Data Quality Grade: {_grade}"
        )
        _grade_cell.font = Font(name="Calibri", bold=True, size=18, color="FFFFFF")
        _grade_cell.fill = PatternFill(
            start_color=_grade_color, end_color=_grade_color, fill_type="solid"
        )
        _grade_cell.alignment = Alignment(horizontal="center", vertical="center")
        for cc in range(3, 4):
            ws_conf.cell(row=conf_row, column=cc).fill = PatternFill(
                start_color=_grade_color, end_color=_grade_color, fill_type="solid"
            )
        ws_conf.merge_cells(f"D{conf_row}:E{conf_row}")
        _score_cell = ws_conf.cell(
            row=conf_row,
            column=4,
            value=(
                f"Score: {_overall:.0%}"
                if isinstance(_overall, (int, float))
                else f"Score: {_overall}"
            ),
        )
        ...
        _sections = _conf_scores.get(
            "per_section", _conf_scores.get("sections", _conf_scores)
        )
        if isinstance(_sections, dict):
            for cidx, (sec_name, sec_data) in enumerate(_sections.items()):
                ...
                for ci, val in enumerate(_sec_vals):
                    ...
                    cell.border = thin_border
                    cell.alignment = (
                        center_alignment if ci in (1, 2) else wrap_alignment
                    )
        ...
        if _failed_apis and isinstance(_failed_apis, list):
            style_section_header(
                ws_conf, conf_row, 2, 5, "API Failures & Circuit Breakers"
            )
            conf_row += 2
            for api_name in _failed_apis:
                if isinstance(api_name, dict):
                    api_display = api_name.get(
                        "name", api_name.get("api", str(api_name))
                    )
                    api_reason = api_name.get("reason", api_name.get("error") or "")
                else:
                    api_display = str(api_name)
                    api_reason = ""
                ws_conf.cell(row=conf_row, column=2, value=f"  {api_display}").font = (
                    Font(name="Calibri", size=10, color="C62828")
                )
                if api_reason:
                    ws_conf.cell(row=conf_row, column=4, value=str(api_reason)).font = (
                        Font(name="Calibri", italic=True, size=9, color="999999")
                    )
                    ws_conf.merge_cells(f"D{conf_row}:E{conf_row}")
                conf_row += 1
        else:
            ...
            ws_conf.cell(
                row=conf_row,
                column=2,
                value="All APIs responded successfully. No circuit breakers triggered.",
            ).font = Font(name="Calibri", size=10, color="2E7D32")
        ...
        try:
            ...
            if _roles_list and isinstance(_roles_list, list) and len(_roles_list) >= 2:
                ...
                for r in _roles_list[:15]:
                    ...
                    try:
                        cr = _collar_intel_mod.classify_collar(
                            role=r_str, industry=_industry_key
                        )
                        _collar_results.append((r_str, cr))
                    except Exception:
                        _collar_results.append(
                            (r_str, {"collar_type": "white_collar", "confidence": 0.3})
                        )

                # Check if we have a collar mix (not all the same type)
                _collar_types_found = set(
                    cr.get("collar_type") or "" for _, cr in _collar_results
                )
                if (
                    len(_collar_types_found) >= 2
                    or "blue_collar" in _collar_types_found
                ):
                    ...
                    ws_collar["B2"].value = (
                        "Blue Collar vs White Collar Strategy Analysis"
                    )
                    ws_collar["B2"].font = Font(
                        name="Calibri", bold=True, size=16, color=NAVY
                    )
                    ws_collar["B2"].border = accent_bottom_border

                    ws_collar.merge_cells("B3:F3")
                    ws_collar["B3"].value = (
                        "Differentiated recruitment approach based on collar type classification"
                    )
                    ws_collar["B3"].font = Font(
                        name="Calibri", italic=True, size=10, color="596780"
                    )
                    ...
                    for ci, (r_str, cr) in enumerate(_collar_results):
                        ...
                        conf = cr.get("confidence") or 0
                        method = cr.get("method") or ""
                        strategy = cr.get("channel_strategy") or ""
                        if isinstance(strategy, dict):
                            strategy = ", ".join(
                                f"{k}: {v}" for k, v in list(strategy.items())[:3]
                            )
                        ...
                        if "blue" in ct:
                            ...
                            ct_cell.font = Font(
                                name="Calibri", bold=True, size=10, color="596780"
                            )
                        # Color confidence
                        ...
                        if conf >= 0.8:
                            ...
                            conf_cell.font = Font(
                                name="Calibri", size=10, color="F57C00"
                            )
                        else:
                            conf_cell.font = Font(
                                name="Calibri", italic=True, size=10, color="C62828"
                            )
                        _crow += 1
                    ...
                    ws_collar.cell(
                        row=_crow, column=2, value="Strategy Comparison by Collar Type"
                    ...
                    for ct_key in ["blue_collar", "white_collar"]:
                        if ct_key in _collar_intel_mod.COLLAR_STRATEGY:
                            ...
                            ws_collar.cell(
                                row=_crow,
                                column=2,
                                value=ct_key.replace("_", " ").title(),
                            ).font = Font(
                                name="Calibri",
                                bold=True,
                                size=12,
                                color="0A66C9" if "blue" in ct_key else NAVY,
                            )
                            _crow += 1
                            for sk in [
                                "preferred_platforms",
                                ...
                                if sv:
                                    ...
                                    ws_collar.cell(
                                        row=_crow, column=2, value=f"  {label}"
                                    ).font = Font(
                                        name="Calibri", size=10, color="596780"
                                    )
                                    ws_collar.cell(
                                        row=_crow, column=4, value=str(sv)
                    ...
                    logger.info(
                        "Collar Strategy worksheet created with %d roles",
                        len(_collar_results),
                    )
        except Exception as e:
    ...
    if _HAS_TREND_ENGINE:
        try:
            ...
            ws_trends_v3["B2"].value = (
                f"CPC/CPA Trend Analysis - {data.get('industry_label', _industry_key.replace('_', ' ').title())}"
            )
            ws_trends_v3["B2"].font = Font(
                name="Calibri", bold=True, size=16, color=NAVY
            )
            ws_trends_v3["B2"].border = accent_bottom_border

            ws_trends_v3.merge_cells("B3:H3")
            ws_trends_v3["B3"].value = (
                "4-year historical trend with seasonal and collar-type adjustments"
            )
            ws_trends_v3["B3"].font = Font(
                name="Calibri", italic=True, size=10, color="596780"
            )
            ...
            for plat in _platforms:
                ...
                ws_trends_v3.cell(row=_trow, column=2, value=plat_label).font = Font(
                ...
                for metric in ["cpc", "cpa"]:
                    try:
                        ...
                        yoy = trend.get("avg_yoy_change_pct") or 0

                        ws_trends_v3.cell(
                            row=_trow, column=2, value=metric.upper()
                        ).font = Font(name="Calibri", bold=True, size=10)
                        ...
                        for yi, year in enumerate([2022, 2023, 2024, 2025]):
                            ...
                            if isinstance(val, (int, float)) and val > 0:
                                cell = ws_trends_v3.cell(
                                    row=_trow, column=3 + yi, value=f"${val:.2f}"
                                )
                            else:
                                cell = ws_trends_v3.cell(
                                    row=_trow, column=3 + yi, value="-"
                                )
                            cell.font = Font(name="Calibri", size=10)
                            cell.border = thin_border
                            cell.alignment = center_alignment
                        ...
                        # Trend direction
                        trend_cell = ws_trends_v3.cell(
                            row=_trow, column=7, value=direction.title()
                        )
            ...
            ws_trends_v3.merge_cells(f"B{_trow}:H{_trow}")
            ws_trends_v3.cell(
                row=_trow, column=2, value="Monthly Seasonal CPC Multipliers"
            ).font = Font(name="Calibri", bold=True, size=14, color=NAVY)
            ws_trends_v3.cell(row=_trow, column=2).border = accent_bottom_border
            _trow += 2
            ...
            for _half, _m_start in [(0, 0), (1, 6)]:
                ...
                # Header
                ws_trends_v3.cell(row=_trow, column=2, value="Collar Type").font = Font(
                    name="Calibri", bold=True, size=9, color="FFFFFF"
                )
                ws_trends_v3.cell(row=_trow, column=2).fill = PatternFill(
                    start_color=NAVY, end_color=NAVY, fill_type="solid"
                )
                ws_trends_v3.cell(row=_trow, column=2).border = thin_border
                ...
                for collar in ["blue_collar", "white_collar"]:
                    ws_trends_v3.cell(
                        row=_trow, column=2, value=collar.replace("_", " ").title()
                    ).font = Font(name="Calibri", bold=True, size=10)
                    ws_trends_v3.cell(row=_trow, column=2).border = thin_border
                    for mi in range(6):
                        try:
                            sa = _trend_engine_mod.get_seasonal_adjustment(
                                collar, _m_start + mi + 1
                            )
                            mult = (
                                sa.get("multiplier", 1.0)
                                if isinstance(sa, dict)
                                else sa
                            )
                            cell = ws_trends_v3.cell(
                                row=_trow, column=3 + mi, value=f"{mult:.2f}x"
                            )
                            cell.border = thin_border
                            cell.alignment = center_alignment
                            if mult > 1.1:
    ...
    _enrichment_summary = (
        data.get("_enriched", {}).get("enrichment_summary", {})
        if isinstance(data.get("_enriched"), dict)
        else {}
    )
    _apis_used_list = (
...
class MediaPlanHandler(BaseHTTPRequestHandler):
    ...
    def _handle_GET(self):
        ...
        # ── VendorIQ Page ──
        elif path in ("/vendor-iq", "/vendor-iq/"):
            viq_html = os.path.join(BASE_DIR, "templates", "vendor-iq.html")
            if os.path.exists(viq_html):
                with open(viq_html, "r") as f:
                    html = f.read()
                self.send_response(200)
                self.send_header("Content-Type", "text/html; charset=utf-8")
                self.end_headers()
                ...
                self.send_error(404, "VendorIQ page not found")
        ...
        # ── PayScale Sync Page ──
            ...
            if os.path.exists(_html_path):
                ...
                self.send_error(404, "PayScale Sync page not found")
        ...
        # ── CreativeAI Page ──
        elif path in ("/creative-ai", "/creative-ai/", "/creativeai", "/creativeai/"):
            _html_path = os.path.join(BASE_DIR, "templates", "creative-ai.html")
            if os.path.exists(_html_path):
                with open(_html_path, "r") as f:
                    html = f.read()
                self.send_response(200)
                ...
                self.send_error(404, "CreativeAI page not found")
        ...
        # ── ComplianceGuard Page ──
            ...
            _html_path = os.path.join(BASE_DIR, "templates", "compliance-guard.html")
            if os.path.exists(_html_path):
                with open(_html_path, "r") as f:
                    html = f.read()
                self.send_response(200)
