...
OUTPUT_FILE = os.path.join(
    OUTPUT_DIR, "Media_Plan_Generator_Product_Showcase_v3.4.docx"
)
...
def build():
    ...
    body(
        doc,
        "When someone submits a plan request, the system executes a 5-stage pipeline:",
    )
    ...
    kb_data = [
        ...
        (
            "Publisher Network",
            "96 KB",
            "Publisher network (Indeed, ZipRecruiter, etc.)",
        ),
    ...
    api_groups = [
        (
            ...
            [
                ...
                "BLS JOLTS (openings/hires/quits)",
                "FRED (unemployment, CPI, wages)",
                "FRED Employment (hourly earnings, ECI)",
                "O*NET (skills, education, outlook)",
                "H-1B Visa Wages (prevailing wages)",
                "CareerOneStop (DOL certifications)",
                "IMF (international GDP, inflation)",
                "Eurostat (EU labor stats)",
                "ILO/ILOSTAT (global labor data)",
                "DataUSA (occupation wages)",
    ...
    make_table(
        ...
        [
            (
                ...
                "Gemini 2.0 Flash",
                "Free",
                "Structured data, JSON, verification",
            ),
            (
                "2",
                "Groq",
                "Llama 3.3 70B",
                "Free",
                "Conversational, tool calling, reasoning",
            ),
            (
                "3",
                "Cerebras",
                "Llama 3.3 70B",
                "Free",
                "Redundancy (same model, different infrastructure)",
            ),
            ...
            (
                ...
                "Llama 4 Maverick",
                "Free",
                "Gateway to multiple free models",
            ),
            (
                "6",
                "xAI",
                "Grok 3 Mini",
                "Free",
                "Strong reasoning, 128K context window",
            ),
            (
                "7",
                "SambaNova",
                "Llama 3.1 405B",
                "Free",
                "Largest open model (405 billion parameters)",
            ),
            (
                "8",
                "NVIDIA NIM",
                "Llama 3.1 70B",
                "Free",
                "Hardware-optimized fast inference",
            ),
            (
                "9",
                "Cloudflare",
            ...
            (
                ...
                "Claude Sonnet",
                "Paid",
                "Complex tool chains (last resort)",
            ),
            (
                "12",
                "Anthropic",
    ...
    make_table(
        ...
        [
            ...
            (
                "Multi-step analysis (3 tools)",
                "~$0.009 (Claude)",
                "$0 (free LLM)",
                "100%",
            ),
            (
                "Complex tool chain (8 tools)",
                "~$0.024 (Claude)",
    ...
    kv_line(
        ...
        "10 (cache eviction, module reload, circuit reset, etc.)",
    )
    kv_line(
        doc, "Test Generation", "Automatic weekly, based on production usage patterns"
    )
    ...
    make_table(
        ...
        [
            (
                ...
                "< 30 seconds (P99)",
                "Time to produce a complete media plan",
            ),
            (
                "Chat Response Latency",
                "< 8 seconds (P99)",
                "Time for Nova to answer a question",
            ),
    ...
    make_table(
        ...
        [
            (
                "Critical",
                "9",
                "Memory leaks, missing budget allocation in async path, "
                "thread explosion under load, CORS on errors, metrics accuracy, tool-call routing failure",
            ),
            (
                "High",
                "11",
                "Lock contention, disk cache growth, N+1 database queries, "
                "thread ID propagation, sequential API calls, paid-provider guard",
            ),
            (
                "Medium",
                "7",
                "Email retry storms, QC code validation, budget threshold, "
                "tool name drift, documentation sync",
            ),
            (
                "Low/Info",
                "35",
                "Logging improvements, code style, documentation, "
    ...
    endpoints_summary = [
        (
            "Plan Generation",
            "/api/generate, /api/jobs/{id}",
            "Create and poll media plans",
        ),
        ("AI Chat", "/api/chat, /api/nova/chat", "Nova conversational interface"),
        ("Slack", "/api/slack/events", "Slack bot webhook"),
        (
            "Health",
            "/api/health, /ready, /data-matrix, /auto-qc, /slos, /eval",
            "5 health endpoints",
        ),
        (
            "Admin",
            "/api/admin/usage, /admin/keys, /admin/nova, /metrics",
            "Usage analytics, key management",
        ),
    ...
    body(doc, "A summary of everything the platform comprises, as of v3.4:")
    ...
    numbers = [
        ...
        (
            ...
            "2",
            "openpyxl + python-pptx (everything else is stdlib)",
        ),
    ]
    ...
    bullet(
        doc, "Cost and usage dashboard showing real-time spend across all 12 providers."
    )
    bullet(doc, "A/B testing framework to systematically compare provider quality.")
    bullet(
        doc, "Expand dynamic test generation to analyze production error logs with AI."
    )
