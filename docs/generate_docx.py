...
meta.add_run(
    "Stack: Python stdlib HTTP server | openpyxl + python-pptx | Render.com Standard"
).font.size = Pt(11)
...
add_body(
    "Total KB size: ~1.0 MB of curated recruitment intelligence. All files validated as parseable JSON."
)
...
kb_data = [
    (
        ...
        "100.9 KB",
        "Industry-specific hiring patterns, funnel rates",
    ),
    (
        "2",
        "platform_intelligence_deep.json",
        "84.2 KB",
        "Deep platform data (CPC/CPA/CTR by platform)",
    ),
    (
        "3",
    ...
    (
        ...
        "channels_db.json",
        "58.5 KB",
        "Channel taxonomy, capabilities, pricing models",
    ),
    (
        "5",
        "joveo_2026_benchmarks.json",
        "11.2 KB",
        "Joveo internal CPA/CPC benchmarks by occupation",
    ),
    ...
    (
        ...
        "75.3 KB",
        "Industry intelligence, strategic insights",
    ),
    (
        "8",
        "joveo_publishers.json",
        "96.4 KB",
        "Joveo publisher network (Indeed, ZR, etc.)",
    ),
    (
        "9",
        "linkedin_guidewire_data.json",
        "27.9 KB",
        "LinkedIn audience size and targeting data",
    ),
    (
        "10",
        "recruitment_strategy_intelligence.json",
        "72.0 KB",
        "Strategy playbooks by collar/tier",
    ),
    (
        "11",
    ...
    (
        ...
        "89.7 KB",
        "Talent pipeline ecosystem data",
    ),
    (
        "13",
        "workforce_trends_intelligence.json",
        "72.4 KB",
        "Macro workforce trend data",
    ),
]
...
add_body(
    "YES = Direct import | ORCH = Via data_orchestrator.py (lazy-loaded) | PART = Receives pre-computed data | -- = Intentionally excluded"
)
...
product_details = [
    (
        ...
        "POST /api/generate",
        "ZIP (Excel + PPT + HTML)",
        "Direct imports -- full pipeline",
    ),
    (
        "Nova Chat",
        "POST /api/chat",
        "JSON (response + sources + confidence)",
        "Via data_orchestrator.py lazy loaders",
    ),
    (
        "Slack Bot",
        "Slack Events API",
        "Slack messages",
        "Via nova.py -> data_orchestrator.py",
    ),
    (
        "PPT Generator",
        "Called by app.py",
        "PowerPoint (.pptx)",
        "Receives pre-computed data from caller",
    ),
]
add_table(
...
add_body(
    "All results cached with TTL via _api_result_cache (thread-safe). Request ID traced through all calls."
)
...
enrich_data = [
    (
        ...
        "enrich_salary",
        "BLS OES, Adzuna, H-1B",
        "Salary ranges, percentiles, prevailing wage",
    ),
    (
        "2",
        "enrich_location",
        "Census, GeoNames, Teleport, DataUSA",
        "Demographics, cost of living, quality of life",
    ),
    (
        "3",
        "enrich_market_demand",
        "JOLTS, FRED, O*NET, Eurostat, ILO",
        "Job openings, unemployment, skills demand",
    ),
    (
        "4",
        "enrich_competitive",
        "Wikipedia, SEC EDGAR",
        "Company info, revenue, competitors",
    ),
    ("5", "enrich_employer_brand", "Clearbit, Logo.dev", "Company logos, brand assets"),
    (
        "6",
        "enrich_ad_benchmarks",
        "Google, Meta, Bing, TikTok, LinkedIn",
        "CPC, CPM, audience sizes, competition",
    ),
    (
        "7",
        "enrich_collar_intelligence",
        "collar_intelligence.py",
        "Blue/white/pink collar classification",
    ),
    (
        "8",
        "enrich_hiring_trends",
        "trend_engine.py + FRED",
        "CPC trends, seasonal patterns, momentum",
    ),
    (
        "9",
        "enrich_budget",
        "budget_engine.py",
        "Channel allocation, CPA, hire projections",
    ),
    (
        "10",
        "enrich_hiring_regulations",
        "KB files",
        "Location-specific hiring regulations",
    ),
    (
        "11",
        "enrich_seasonal",
        "KB + trend_engine",
        "Seasonal hiring patterns by industry",
    ),
    ...
    (
        "15",
        "enrich_global_supply",
        "KB + World Bank",
        "Global talent supply indicators",
    ),
    (
        "16",
        "enrich_educational_partners",
        "KB files",
        "Educational institution partnerships",
    ),
    (
        "17",
        "enrich_radio_podcasts",
        "KB files",
        "Radio/podcast advertising opportunities",
    ),
]
add_table(
    ["#", "Function", "API Sources", "Data Produced"], enrich_data, [0.3, 2.0, 2.2, 2.3]
)
...
tier2_data = [
    ...
    (
        "data_contracts.py",
        "validate_kb_file",
        "KB JSON schema validation, enrichment output contracts",
    ),
    (
        "regression_detector.py",
        "run_regression_check",
        "10 reference scenarios, baseline drift detection",
    ),
    ("llm_router.py", "call_llm", "Multi-provider LLM routing with circuit breakers"),
    (
        "monitoring.py",
        "MetricsCollector",
        "Structured JSON logging, SLO monitoring, request tracing",
    ),
]
...
add_body(
    "Entry point: app.py receives user input (industry, roles, locations, budget, client_name, competitors)."
)
stage1 = [
    (
        "standardizer.normalize_industry()",
        "Map free-text industry to canonical taxonomy",
    ),
    ("standardizer.normalize_role()", "Map role to standardized title + SOC code"),
    (
        "collar_intelligence.classify_collar()",
        "Classify each role as blue/white/pink collar",
    ),
...
add_body(
    "data_orchestrator.py calls 17 enrich_* functions, each pulling from 1-5 APIs. All results cached with TTL. Request ID propagated for tracing."
)
...
add_body(
    "data_synthesizer.py merges all 17 enrichment outputs into a unified structure with structured confidence scoring (per-field, per-source), source attribution, and conflict resolution."
)

# Stage 4
doc.add_heading("Stage 4: Budget Allocation Engine", level=2)
add_body(
    "budget_engine.py performs channel selection (collar-weighted), dynamic CPC pricing (market + seasonal + regional adjustments), seniority decomposition, channel quality scoring, budget distribution, hire projections, ROI calculation, and what-if simulation."
)
...
outputs = [
    (
        "Excel/PPT Report",
        "ZIP containing: media_plan.xlsx + strategy_deck.pptx + report.html",
        "Full synthesized + allocated data",
    ),
    (
        "Nova Chat",
        "JSON with response, sources, confidence, tools_used",
        "26 tools + LLM via llm_router",
    ),
    (
        "Slack Bot",
        "Slack messages with formatted responses",
        "Same 26 tools via Slack Events API",
    ),
    (
        "PPT Generator",
        "PowerPoint deck with exec summary, strategy, benchmarks",
        "Pre-computed data from app.py",
    ),
]
...
add_body(
    "53 static tests + dynamic test generation + 10 self-healing strategies + weekly self-upgrade cycle."
)
...
test_cats = [
    (
        "01-10",
        "Infrastructure",
        "Health endpoint, KB files, Nova import, orchestrator, Claude API, data matrix, cache, tool count, env vars, module compilation",
    ),
    (
        "11-14",
        "Chat Quality",
        "Learned answers, response structure, empty message handling, ask-before-answer",
    ),
    (
        "15-20",
        "Orchestrator & Data",
        "Lazy loaders, enrichment context, ad benchmarks, insights, employer brand, fallback telemetry",
    ),
    ("21-23", "Trend Engine", "Benchmarks, seasonal patterns, data freshness"),
    (
        "24-28",
        "v3.0 Core",
        "Collar classification, budget engine params, ad benchmarks v3, collar intelligence, hiring trends",
    ),
    (
        "29-33",
        "v3.0 Integration",
        "Nova v3 tools, PPT v3 features, synthesizer, structured confidence, data matrix v3 layers",
    ),
    (
        "34-44",
        "v3.1 Tier 1-3",
        "KB data contracts, enrichment contracts, regression check, eval budget/collar, structured logging, OpenAPI, role decomposition, channel quality, dynamic CPC, what-if",
    ),
    (
        "45-53",
        "v3.1 Coverage",
        "LLM router health/classify, provider availability, async jobs, API key tiers, Joveo benchmarks, audit trail, SLO compliance, tier2 modules",
    ),
]
...
heal_data = [
    (
        ...
        "Reload data_orchestrator module",
    ),
    ("2", "reimport_nova", "Nova import failure", "Reload nova module"),
    (
        "3",
        "force_matrix_recheck",
        "Matrix health degraded",
        "Force immediate data matrix re-check",
    ),
    ("4", "cache_eviction", "Stale API cache", "Evict expired cache entries"),
    (
        "5",
        "reset_lazy_sentinels",
        "Lazy loader stuck in failed state",
        "Reset _IMPORT_FAILED sentinels",
    ),
    (
        "6",
        "reset_llm_circuit_breakers",
        "LLM providers all tripped",
        "Reset circuit breaker states",
    ),
    (
        "7",
        "async_job_cleanup",
        "Job queue overflow (>100)",
        "Evict completed/expired jobs",
    ),
    (
        "8",
        "tier2_reimport",
        "Tier2 module import failure",
        "Reimport eval/contracts/regression/llm/monitoring",
    ),
    (
        "9",
        "audit_log_persist",
        "Audit log exceeds max entries",
        "Truncate to bounded size",
    ),
    (
        "10",
        "joveo_kb_validated",
        "Joveo KB file missing/corrupt",
        "Validate + attempt recovery",
    ),
]
...
add_body(
    "47 total health probes running every 12 hours with self-healing. 60-second initial delay after startup."
)
...
probe_summary = [
    ...
    (
        "Tier2 module health",
        "5",
        "eval_framework, data_contracts, regression_detector, llm_router, monitoring",
    ),
    (
        "Extended health indicators",
        "6",
        "LLM providers, async jobs, API keys, KB freshness, eval scores, regression baseline",
    ),
    ("TOTAL", "47", ""),
]
...
endpoints = [
    (
        ...
        "/api/generate",
        "--",
        "Generate media plan (sync or async via X-Async header)",
    ),
...
add_table(
    ["Method", "Endpoint", "Auth", "Description"], endpoints, [0.5, 2.0, 0.5, 3.8]
)
...
core_files = [
    ...
    (
        ...
        "~2,800",
        "17 enrichment functions, lazy loading, caching",
    ),
...
intel_files = [
    (
        "budget_engine.py",
        "~2,500",
        "Budget allocation, quality scoring, what-if simulation",
    ),
    (
        "collar_intelligence.py",
        "~1,500",
        "Collar classification, role decomposition, skills gap",
    ),
...
checks = [
    ...
    (
        "8",
        "Generate (async)",
        "POST /api/generate + X-Async: true",
        "Job ID, poll until complete",
    ),
    ...
    (
        "11",
        "Role decomp",
        'Chat: "Break down 50 engineers"',
        "Uses query_role_decomposition",
    ),
