...
def _pct_sum_of_channels(channel_allocs: Dict[str, Dict]) -> float:
    ...
    if not channel_allocs:
        return 0.0
    total_dollars = sum(ch.get("dollar_amount", 0) for ch in channel_allocs.values())
...
def _build_budget_cases() -> List[Dict[str, Any]]:
    ...
    def _alloc_input(budget: float, roles: List[Dict], locations: List[Dict],
                     industry: str, channels: Dict[str, float],
                     collar: str = "") -> Dict[str, Any]:
        return {
            "total_budget": budget,
            "roles": roles,
            "locations": locations,
    ...
    def _run_budget(inp: Dict) -> Any:
        ...
        return _budget_engine.calculate_budget_allocation(
            ...
            collar_type=inp.get("collar_type", ""),
        )
    ...
    def _check_alloc_sum(result: Any) -> Tuple[bool, str]:
        ...
        total_dollars = sum(ch.get("dollar_amount", 0) for ch in allocs.values())
        budget = result.get("metadata", {}).get("total_budget", 0)
        if budget <= 0:
    ...
    for budget_val in [5000, 10000, 25000, 50000, 100000, 250000, 500000]:
        cases.append({
            ...
            "input": _alloc_input(
                budget=budget_val,
                roles=[{"title": "Software Engineer", "count": 3, "tier": "Professional / White-Collar"}],
    ...
    def _check_no_zero_channel(result: Any) -> Tuple[bool, str]:
        ...
        exempt = {"referral", "events", "staffing", "Referral Programs",
                  "Recruitment Events", "Staffing Agencies"}
        for ch_name, ch_data in allocs.items():
            if ch_name in exempt:
                continue
            if ch_data.get("dollar_amount", 0) <= 0:
    ...
    for industry in ["tech_engineering", "healthcare_medical", "retail_consumer", "logistics_supply_chain"]:
        cases.append({
            "name": f"no_zero_channel_{industry}_50k",
            "input": _alloc_input(
                budget=50000,
                roles=[{"title": "General Worker", "count": 5, "tier": "Hourly / Entry-Level"}],
    ...
    def _check_small_budget_channels(result: Any) -> Tuple[bool, str]:
        if result is None:
            return False, "budget_engine not available"
        allocs = result.get("channel_allocations", {})
        active = [ch for ch, d in allocs.items() if d.get("dollar_amount", 0) > 0]
        count = len(active)
        ...
        if count > 6:
            return False, f"Small budget spread across {count} channels (max ~5 expected)"
    ...
    for small_budget in [1000, 2000, 3000, 4000, 4500]:
        cases.append({
            ...
            "input": _alloc_input(
                budget=small_budget,
                roles=[{"title": "Warehouse Worker", "count": 2, "tier": "Hourly / Entry-Level"}],
                locations=[{"city": "Dallas", "state": "TX", "country": "US"}],
                industry="logistics_supply_chain",
                channels=_STANDARD_4CH,
            ),
            "check": lambda out, _r=_run_budget: _check_small_budget_channels(_r(out)),
        })
    ...
    def _check_cpa_reasonable(result: Any, industry: str) -> Tuple[bool, str]:
        ...
        cpa = tp.get("cost_per_application", 0)
        if cpa <= 0:
    ...
    for ind, budget in [("tech_engineering", 30000), ("healthcare_medical", 40000),
                        ("retail_consumer", 15000), ("hospitality_travel", 10000),
                        ("finance_banking", 60000), ("blue_collar_trades", 20000)]:
        cases.append({
            "name": f"cpa_positive_{ind}",
            "input": _alloc_input(
                budget=budget,
                roles=[{"title": "General Role", "count": 3, "tier": "Professional / White-Collar"}],
                locations=[{"city": "Los Angeles", "state": "CA", "country": "US"}],
                industry=ind,
                channels=_STANDARD_6CH,
            ),
            "check": lambda out, _i=ind, _r=_run_budget: _check_cpa_reasonable(_r(out), _i),
        })
    ...
    def _check_positive_hires(result: Any) -> Tuple[bool, str]:
        if result is None:
            return False, "budget_engine not available"
        hires = result.get("total_projected", {}).get("hires", 0)
        if hires > 0:
    ...
    for budget_val in [1500, 5000, 10000, 50000, 100000]:
        cases.append({
            ...
            "input": _alloc_input(
                budget=budget_val,
                roles=[{"title": "Warehouse Associate", "count": 5, "tier": "Hourly / Entry-Level"}],
    ...
    def _check_cph_positive(result: Any) -> Tuple[bool, str]:
        if result is None:
            return False, "budget_engine not available"
        cph = result.get("total_projected", {}).get("cost_per_hire", 0)
        if cph > 0:
    ...
    cases.append({
        ...
        "input": _alloc_input(
            budget=50000,
            roles=[{"title": "Data Analyst", "count": 4, "tier": "Professional / White-Collar"}],
    ...
    def _check_zero_budget(result: Any) -> Tuple[bool, str]:
        ...
        return False, f"Zero budget unexpectedly produced {len(allocs)} channel allocations"

    cases.append({
        "name": "zero_budget_empty",
        "input": _alloc_input(
            budget=0,
            roles=[{"title": "Engineer", "count": 1, "tier": "Professional / White-Collar"}],
    ...
    cases.append({
        ...
        "input": _alloc_input(
            budget=1000000,
            roles=[{"title": "Regional Manager", "count": 10, "tier": "Professional / White-Collar"}],
            locations=[{"city": "New York", "state": "NY", "country": "US"}],
...
def _build_collar_cases() -> List[Dict[str, Any]]:
    ...
    def _run_classify(inp: Dict) -> Any:
        ...
        return _collar_intel.classify_collar(
            ...
            soc_code=inp.get("soc_code", ""),
        )

    # --- B1: Blue collar role classification ---
    blue_collar_roles = [
        "warehouse worker", "CDL driver", "forklift operator", "truck driver",
        "construction laborer", "electrician", "plumber", "welder",
        "machine operator", "dock worker", "delivery driver", "janitor",
        "dishwasher", "line cook", "security guard",
    ]
    ...
    white_collar_roles = [
        ...
        "project manager", "attorney", "accountant", "director of marketing",
        "VP of Engineering", "CEO", "consultant", "research scientist",
    ]
    ...
    grey_collar_roles = [
        ...
        "EMT paramedic", "medical assistant", "respiratory therapist",
    ...
    pink_collar_roles = [
        "receptionist", "administrative assistant", "data entry clerk",
    ...
    def _check_blue_channel_mix(out: Any) -> Tuple[bool, str]:
        ...
        indeed_fb = mix.get("global_job_boards", 0) + mix.get("social_media", 0) + mix.get("programmatic", 0)
        if indeed_fb >= 0.40:
            return True, f"Blue collar Indeed/Facebook/Programmatic share = {indeed_fb:.0%} (>= 40%)"
        return False, f"Blue collar Indeed/Facebook/Programmatic share = {indeed_fb:.0%} (< 40%)"
    ...
    def _check_white_linkedin(out: Any) -> Tuple[bool, str]:
        ...
        mix = strategy.get("channel_mix", {})
        linkedin_pct = mix.get("linkedin", 0)
    ...
    for role, expected_collar in unambiguous_roles:
        def _check_conf(out: Any, _role: str = role, _ec: str = expected_collar) -> Tuple[bool, str]:
            ...
            return False, f"'{_role}' confidence {conf:.2f} < 0.5 (expected >= 0.5 for unambiguous role)"
        cases.append({
    ...
    def _check_strategy_field(out: Any) -> Tuple[bool, str]:
        ...
        if result is None:
            return False, "collar_intelligence not available"
        strat = result.get("channel_strategy", "")
...
def _build_geo_cases() -> List[Dict[str, Any]]:
    ...
    for city, state in us_cities:
        def _check_us(out: Any, _city: str = city, _st: str = state) -> Tuple[bool, str]:
        ...
        cases.append({
            ...
            "input": {
                "total_budget": 30000,
                "roles": [{"title": "Warehouse Worker", "count": 5, "tier": "Hourly / Entry-Level"}],
    ...
    uk_cities = [("London", "England"), ("Manchester", "England"), ("Edinburgh", "Scotland")]
    for city, region in uk_cities:
        def _check_uk(out: Any, _city: str = city) -> Tuple[bool, str]:
            ...
            return True, f"UK location {_city} produced {len(allocs)} channel allocations"
        cases.append({
    ...
    for city, region, country in intl_locations:
        def _check_intl(out: Any, _city: str = city, _country: str = country) -> Tuple[bool, str]:
            result = _run_budget(out)
            if result is None:
                return False, "budget_engine not available"
            allocs = result.get("channel_allocations", {})
            ...
            return True, f"International location {_city}, {_country} handled ({len(allocs)} channels)"

        cases.append({
            "name": f"intl_location_{city.replace(' ', '_')}_{country}",
            "input": {
                "total_budget": 30000,
                "roles": [{"title": "Marketing Manager", "count": 2, "tier": "Professional / White-Collar"}],
    ...
    def _check_trend_regional(out: Any) -> Tuple[bool, str]:
        ...
        nyc = _trend_engine.get_benchmark(
            platform="google_search", industry="tech_engineering",
            metric="cpc", location="New York",
        )
        rural = _trend_engine.get_benchmark(
            platform="google_search", industry="tech_engineering",
            metric="cpc", location="Boise",
        )
        nyc_val = nyc.get("value", 0)
        rural_val = rural.get("value", 0)
    ...
    def _check_location_normalization(out: Any) -> Tuple[bool, str]:
        ...
        nyc1 = _trend_engine.get_benchmark(
            platform="indeed", industry="retail_consumer",
            metric="cpc", location="New York",
        )
        nyc2 = _trend_engine.get_benchmark(
            platform="indeed", industry="retail_consumer",
            metric="cpc", location="NYC",
        )
        v1 = nyc1.get("value", 0)
        v2 = nyc2.get("value", 0)
        ...
        if v1 > 0:
            return True, f"'New York' resolved (CPC={v1:.2f}); 'NYC' may not be in lookup"
...
def _build_cpa_cases() -> List[Dict[str, Any]]:
    ...
    def _check_healthcare_cpa(out: Any) -> Tuple[bool, str]:
        ...
        bench = _trend_engine.get_benchmark(
            platform="indeed", industry="healthcare_medical",
            metric="cpa", collar_type="grey_collar",
        )
        cpa = bench.get("value", 0)
        if 5 <= cpa <= 200:
            return True, f"Healthcare CPA = ${cpa:.2f} (within $5-$200 range)"
        return False, f"Healthcare CPA = ${cpa:.2f} (outside $5-$200 range)"

    cases.append({"name": "healthcare_cpa_range", "input": {}, "check": _check_healthcare_cpa})
    ...
    for platform in ["indeed", "google_search", "meta_facebook", "linkedin"]:
        def _check_hc_plat(out: Any, _p: str = platform) -> Tuple[bool, str]:
            if not _HAS_TREND:
                return False, "trend_engine not available"
            bench = _trend_engine.get_benchmark(
                platform=_p, industry="healthcare_medical",
                metric="cpa", collar_type="grey_collar",
            )
            cpa = bench.get("value", 0)
            if cpa > 0:
    ...
    for platform in ["linkedin", "indeed", "google_search"]:
        def _check_tech_cpa(out: Any, _p: str = platform) -> Tuple[bool, str]:
            if not _HAS_TREND:
                return False, "trend_engine not available"
            bench = _trend_engine.get_benchmark(
                platform=_p, industry="tech_engineering",
                metric="cpa", collar_type="white_collar",
            )
            cpa = bench.get("value", 0)
            if 5 <= cpa <= 300:
    ...
    for platform in ["indeed", "meta_facebook", "programmatic"]:
        def _check_retail_cpa(out: Any, _p: str = platform) -> Tuple[bool, str]:
            if not _HAS_TREND:
                return False, "trend_engine not available"
            bench = _trend_engine.get_benchmark(
                platform=_p, industry="retail_consumer",
                metric="cpa", collar_type="blue_collar",
            )
            cpa = bench.get("value", 0)
            if 2 <= cpa <= 80:
    ...
    def _check_exec_cpa(out: Any) -> Tuple[bool, str]:
        if not _HAS_TREND:
            return False, "trend_engine not available"
        bench = _trend_engine.get_benchmark(
            platform="linkedin", industry="finance_banking",
            metric="cpa", collar_type="white_collar",
        )
        cpa = bench.get("value", 0)
    ...
    cases.append({"name": "executive_cpa_linkedin", "input": {}, "check": _check_exec_cpa})

    # --- D5: Blue collar CPA < white collar CPA in same industry ---
    comparison_industries = [
        "logistics_supply_chain", "retail_consumer", "hospitality_travel",
        "construction_real_estate", "automotive",
    ]
    for ind in comparison_industries:
        def _check_collar_cpa_order(out: Any, _ind: str = ind) -> Tuple[bool, str]:
            ...
            blue_cpa = blue.get("value", 0)
            white_cpa = white.get("value", 0)
            if blue_cpa <= 0 or white_cpa <= 0:
                return False, f"Non-positive CPA: blue=${blue_cpa:.2f}, white=${white_cpa:.2f}"
    ...
    if _HAS_COLLAR:
        for collar, strat in _collar_intel.COLLAR_STRATEGY.items():
            cpa_range = strat.get("avg_cpa_range", [])
            if len(cpa_range) == 2:
                def _check_strat_range(out: Any, _c: str = collar,
                                       _lo: float = cpa_range[0],
                                       _hi: float = cpa_range[1]) -> Tuple[bool, str]:
    ...
    major_platforms = ["google_search", "meta_facebook", "linkedin", "indeed", "programmatic"]
    for plat in major_platforms:
        def _check_cpc_positive(out: Any, _p: str = plat) -> Tuple[bool, str]:
            if not _HAS_TREND:
                return False, "trend_engine not available"
            bench = _trend_engine.get_benchmark(
                platform=_p, industry="general_entry_level", metric="cpc",
            )
            cpc = bench.get("value", 0)
            if cpc > 0:
