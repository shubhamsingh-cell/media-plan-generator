...
def test_copilot_suggest_brief_ok_list(monkeypatch):
    ...
    )  # 3rd skipped (no text), 4th over cap
...
def test_analyze_compliance_ok(monkeypatch):
    res = {
        ...
        "data": {
            "score": 72,
            "issues": [{"phrase": "young"}],
            "recommendations": ["fix age language"],
        },
        "provider": "gemini",
        "error": "",
    }
...
def test_post_campaign_summary_ok(monkeypatch):
    res = {
        "ok": True,
        "data": {
            "executive_summary": "ES with $5000",
            "channel_analysis": "CA",
            "recommendations": "REC",
        },
        "provider": "gemini",
...
def test_audit_complianceguard_ok(monkeypatch):
    res = {
        ...
        "data": {
            "score": 60,
            "risk_level": "High",
            "findings": [{"category": "age"}],
            "recommendations": ["reword"],
            "rewritten_description": "Seeking an engineer.",
        },
        "provider": "gemini",
        "error": "",
    }
...
def test_creative_ads_ok_list(monkeypatch):
    res = {
        "ok": True,
        "data": [{"variant_name": "A", "headline": "h"}],
        "provider": "gemini",
        "error": "",
    }
    monkeypatch.setattr(app, "_lazy_llm_router", lambda: _fake_router(res))
...
def test_ab_test_router_success(monkeypatch):
    res = {
        "ok": True,
        "data": {
            "variant_a": {"headline": "Benefits", "description": "d", "cta": "Apply"},
    ...
    out = app._generate_ab_test_with_claude(
        "Nurse", "Acme", "healthcare", "RN", "Indeed", 5000.0, ""
    )
