...
(function () {
  ...
  var STAGES_ORDERED = ['greeting', 'qualification', 'experience', 'skills', 'contact', 'confirmation', 'complete'];
  ...
  function injectStyles() {
    ...
    var css = ''
      ...
      + '.af-msg-user {'
      + '  align-self: flex-end;'
      + '  background: ' + CONFIG.bgUserMessage + ';'
      + '  color: #e0f0ff;'
  ...
  function buildWidget() {
    ...
    btn.innerHTML = ICONS.briefcase + '<span class="applyflow-btn-label">Apply Now</span>';
    ...
    panel.innerHTML = ''
      ...
      + '      <div class="af-header-icon">' + ICONS.briefcase + '</div>'
      + '      <div class="af-header-text">'
      + '        <div class="af-header-role" id="af-role" title="' + escapeHtml(role) + '">' + escapeHtml(role) + '</div>'
      + '        <div class="af-header-company" id="af-company">' + escapeHtml(company) + '</div>'
      + '      </div>'
      + '    </div>'
      + '    <button class="af-close-btn" aria-label="Close application chat" tabindex="0">' + ICONS.close + '</button>'
      + '  </div>'
      ...
      + '  <button class="af-send-btn" id="af-send" aria-label="Send message" tabindex="0">' + ICONS.send + '</button>'
      + '</div>';
  ...
  function updateProgressBar() {
    ...
    for (var i = 0; i < STAGES_ORDERED.length; i++) {
      ...
      html += '<div class="' + cls + '" title="' + STAGE_LABELS[STAGES_ORDERED[i]] + '"></div>';
    }
    ...
    if (label) {
      label.textContent = STAGE_LABELS[state.currentStage] || state.currentStage;
    }
  }
  ...
  function showTyping() {
    ...
    typing.innerHTML = '<div class="af-typing-dot"></div><div class="af-typing-dot"></div><div class="af-typing-dot"></div>';
  ...
  function initSession() {
    ...
    apiCall(payload, function (data) {
      ...
      hideTyping();
      setInputEnabled(true);
      addMessage('assistant', 'Welcome! I am here to help you apply. Please tell me a bit about yourself to get started.');
    });
  }
  ...
  function sendMessage() {
    ...
    apiCall(payload, function (data) {
      ...
      hideTyping();
      setInputEnabled(true);
      addMessage('assistant', 'I am sorry, there was a connection issue. Could you please try again?');
    });
  }
  ...
  function showCompleteBanner() {
    ...
    banner.innerHTML = '<div class="af-complete-icon">' + ICONS.check + '</div>'
